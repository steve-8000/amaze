...
async function cmdRelease(version: string): Promise<void> {
	...
	// 3b. Rename the natives version sentinel so any `.node` left on disk from
	...
	// `js_name = "__amazeNativesV…"` literal. `gen-enums.ts` regenerates the matching
	...
	console.log(`Bumping natives version sentinel to v${version}…`);
	const sentinelJsId = version.replace(/[^A-Za-z0-9]/g, "_");
	const sentinelName = `__amazeNativesV${sentinelJsId}`;
	const sentinelFiles = [
		"crates/natives/src/lib.rs",
		"packages/natives/native/index.d.ts",
		"packages/natives/native/index.js",
	];
	await $`sd '__amazeNativesV[A-Za-z0-9_]+' ${sentinelName} ${sentinelFiles}`;
	const libRs = await Bun.file("crates/natives/src/lib.rs").text();
	if (!libRs.includes(`js_name = "${sentinelName}"`)) {
		console.error(
			`Error: natives version sentinel did not move to ${sentinelName} in crates/natives/src/lib.rs. ` +
				"The `__amazeNativesV…` literal may have been removed or renamed; restore it before releasing.",
		);
		process.exit(1);
	}
