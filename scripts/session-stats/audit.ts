#!/usr/bin/env bun
/**
 * Token-usage audit over the local amaze session corpus (~/.amaze/agent/sessions/).
 *
 ...
 *
 * Auth: resolves an API key for the classifier provider through amaze's auth
 * storage (~/.amaze/agent/agent.db: stored key, OAuth, or env var fallback).
 */
...
const CACHE_PATH = path.join(os.homedir(), ".amaze", "stats-audit-cache.json");
...
function parseCli(argv: string[]): CliOptions {
	...
	if (values.help) {
		console.log(
			`session audit — token usage analysis over ~/.amaze/agent/sessions\n\n` +
				...
				`  --no-cache               skip the verdict cache (~/.amaze/stats-audit-cache.json)\n` +
...
async function openClassifier(modelSpec: string): Promise<Classifier> {
	...
	if (!apiKey) {
		throw new Error(`no credentials for provider "${provider}" (amaze login or env var required)`);
	}
	return { model, apiKey };
}
