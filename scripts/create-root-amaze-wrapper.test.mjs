...
import { createRootAmazeWrapper, shouldWriteGlobalShim } from "./create-root-amaze-wrapper.mjs";

describe("create-root-amaze-wrapper", () => {
	it("writes a launch-only wrapper when the root is a gitless snapshot", () => {
		// Given
		const root = mkdtempSync(join(tmpdir(), "amaze-wrapper-snapshot-"));
		const globalPrefix = mkdtempSync(join(tmpdir(), "amaze-wrapper-global-"));

		// When
		const result = createRootAmazeWrapper({ root, globalPrefix });
		...
		assert.equal(wrapper.includes("packages/coding-agent/dist/amaze"), true);
		assert.equal(wrapper.includes("scripts/build-all.mjs"), false);
		assert.equal(wrapper.includes("packages/ai/src"), false);
		assert.equal(wrapper.includes(".amaze-build-head"), false);
	...
	it("does NOT write a global shim by default in a git checkout (opt-in only)", () => {
		// Given
		const root = mkdtempSync(join(tmpdir(), "amaze-wrapper-git-default-"));
		mkdirSync(join(root, ".git"));

		// When
		const defaultDecision = shouldWriteGlobalShim(root, {});
		const ciDecision = shouldWriteGlobalShim(root, { CI: "true", AMAZE_WRITE_GLOBAL_SHIM: "1" });
	...
	it("writes a global shim only when AMAZE_WRITE_GLOBAL_SHIM=1 in a non-CI git checkout", () => {
		// Given
		const root = mkdtempSync(join(tmpdir(), "amaze-wrapper-git-optin-"));
		mkdirSync(join(root, ".git"));

		// When
		const optInDecision = shouldWriteGlobalShim(root, { AMAZE_WRITE_GLOBAL_SHIM: "1" });
	...
	it("replaces an existing global symlink instead of following it", () => {
		// Given
		const root = mkdtempSync(join(tmpdir(), "amaze-wrapper-root-"));
		const globalPrefix = mkdtempSync(join(tmpdir(), "amaze-wrapper-global-"));
		...
		symlinkSync(linkedTarget, join(globalBin, "amaze"));

		// When
		const result = createRootAmazeWrapper({ root, globalPrefix, writeGlobalShim: true });
