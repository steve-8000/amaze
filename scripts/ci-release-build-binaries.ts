...
const targets: BinaryTarget[] = [
	{
		...
		outfile: "packages/coding-agent/binaries/amaze-darwin-arm64",
	},
	{
		id: "darwin-x64",
		platform: "darwin",
		...
		outfile: "packages/coding-agent/binaries/amaze-darwin-x64",
	},
	{
		id: "linux-x64",
		platform: "linux",
		...
		outfile: "packages/coding-agent/binaries/amaze-linux-x64",
	},
	{
		id: "linux-arm64",
		platform: "linux",
		...
		outfile: "packages/coding-agent/binaries/amaze-linux-arm64",
	},
	{
		id: "win32-x64",
		platform: "win32",
		...
		outfile: "packages/coding-agent/binaries/amaze-windows-x64.exe",
	},
];
...
async function buildBinary(target: BinaryTarget): Promise<void> {
	...
	if (isDryRun) {
		console.log(`DRY RUN bun build --compile --no-compile-autoload-bunfig --no-compile-autoload-dotenv --no-compile-autoload-tsconfig --no-compile-autoload-package-json --keep-names --define process.env.AMAZE_COMPILED="true" --root . --external mupdf --target=${target.target} ${entrypoint} ${workerEntrypoints.join(" ")} --outfile ${target.outfile}`);
		return;
	}
	...
	await runCommand(
		[
			...
			"--keep-names",
			"--define",
			'process.env.AMAZE_COMPILED="true"',
			"--root",
			".",
			"--external",
			"mupdf",
