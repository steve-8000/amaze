...
const agentDirEnvName = "AMAZE_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "AMAZE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific AMAZE_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force AMAZE_OFFLINE=1 / AMAZE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.AMAZE_OFFLINE = "1";
		env.AMAZE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
