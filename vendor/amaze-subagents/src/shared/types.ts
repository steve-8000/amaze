...
export interface RunSyncOptions {
	...
	onUpdate?: (r: import("@steve-8000/amaze-agent-core").AgentToolResult<Details>) => void;
	...
	/** Explicit path-local memory packet for fresh child runtime injection. */
	memoryPacket?: PathMemoryPacketInput;
	/** Explicit path execution contract for child write-boundary enforcement. */
	pathContract?: PathContract;
	/** Fresh-only boot contract. When present, path memory and execution boundaries are derived from this contract. */
...
export const TEMP_ROOT_DIR = path.join(os.tmpdir(), `amaze-subagents-${resolveTempScopeId()}`);
