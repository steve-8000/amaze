...
import { type ExtensionAPI, type ExtensionContext, type ToolDefinition } from "amaze";
import { Box, Container, Spacer, Text, truncateToWidth, visibleWidth, wrapTextWithAnsi, type Component } from "@steve-8000/amaze-tui";
...
import { SUBAGENT_CHILD_ENV, SUBAGENT_FANOUT_CHILD_ENV } from "../runs/shared/amaze-args.ts";
...
class SubagentControlNoticeComponent implements Component {
	...
	render(width: number): string[] {
		...
		const borderChar = "─";
		// Subagent intercom/report notice: yellow box showing who reported to whom.
		const recipient = this.details.childIntercomTarget ?? "supervisor";
		const header = ` 💬 ${this.details.event.agent} → ${recipient} · ${eventLabel} `;
		const headerText = truncateToWidth(header, bodyWidth, "");
		const headerPadding = Math.max(0, bodyWidth - visibleWidth(headerText));
		const lines = [this.theme.fg("warning", `╭${headerText}${borderChar.repeat(headerPadding)}╮`)];

		for (const line of wrapTextWithAnsi(formatSubagentControlNotice(this.details), bodyWidth)) {
			const text = truncateToWidth(line, bodyWidth, "");
			const padding = Math.max(0, bodyWidth - visibleWidth(text));
			lines.push(this.theme.fg("warning", `│${text}${" ".repeat(padding)}│`));
		}
		lines.push(this.theme.fg("warning", `╰${borderChar.repeat(bodyWidth)}╯`));
		return lines;
	}
}
...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	const slashBridge = registerSlashSubagentBridge({
		...
		execute: (id, params, signal, onUpdate, ctx) =>
			executeSubagentCollapsed(id, params as SubagentParamsLike, signal, onUpdate, ctx),
	});
...
• {chain_dir} - Shared directory for chain files (e.g., <tmpdir>/amaze-subagents-<scope>/chain-runs/abc123/)
...
• { action: "doctor" } - read-only report for runtime paths, discovery, sessions, and intercom`,
		...
		renderResult(result, options, theme, context) {
			...
			const inner = renderSubagentResult(result, options, theme);
			const wrapper =
