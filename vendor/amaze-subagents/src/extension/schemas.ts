...
const MemoryPacketOverride = Type.Unsafe({
	type: "object",
	additionalProperties: true,
	description: "Path-local memory packet contract for fresh child boot. Supports memory_scope, memory_attachments, contract_id, and apply_updates_after_validation_pass.",
});

const PathContractOverride = Type.Unsafe({
	type: "object",
	additionalProperties: true,
	description: "Path execution contract for write-boundary enforcement. Supports owned_paths, write_allowed_paths, write_denied_paths, assigned_path, and contract_id.",
});

const BootContractOverride = Type.Unsafe({
	type: "object",
	additionalProperties: true,
	description: "FreshBootContract for fresh-only child execution. Supplies parent_context=false, specialist, context_packet, memory_attachments, and execution_contract.",
});
...
const TaskItem = Type.Object({
	...
	memoryPacket: Type.Optional(MemoryPacketOverride),
	pathContract: Type.Optional(PathContractOverride),
	bootContract: Type.Optional(BootContractOverride),
...
export const SubagentParams = Type.Object({
	...
	orchestrateOutput: Type.Optional(Type.String({
		enum: ["compact", "full"],
		description: "Deprecated for action='orchestrate'. Orchestrate now dispatches directly to one agent and does not compile intermediate FreshBootContract plans."
	})),
