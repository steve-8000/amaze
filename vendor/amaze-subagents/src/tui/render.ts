...
import { getMarkdownTheme, type ExtensionContext } from "amaze";
import { Container, Markdown, Spacer, Text, visibleWidth, type Component } from "@steve-8000/amaze-tui";
import {
	type AgentProgress,
	type AsyncJobState,
	type AsyncJobStep,
...
/**
 * Wraps a subagent result/progress component in an accent-bordered box so
 * orchestration output (parallel progress, agent list) reads as a card.
 */
export class SubagentBoxWrapper implements Component {
	inner: Component;
	private boxTheme: Theme;
	private header: string;

	constructor(inner: Component, theme: Theme, header: string) {
		this.inner = inner;
		this.boxTheme = theme;
		this.header = header;
	}

	setInner(inner: Component): this {
		this.inner = inner;
		return this;
	}
	...
	render(width: number): string[] {
		const theme = this.boxTheme;
		// boxSharp is provided by the host theme but not declared on the extension type.
		const bs = (theme as unknown as { boxSharp: Record<string, string> }).boxSharp;
		const fg = (s: string): string => theme.fg("accent", s);
		const interior = Math.max(1, width - 2);
		const contentWidth = Math.max(1, interior - 2);
		const innerLines = this.inner.render(contentWidth);
		const headerText = ` ${this.header} `;
		const topPad = Math.max(0, interior - 3 - visibleWidth(headerText));
		const lines = [fg(`${bs.topLeft}${bs.horizontal.repeat(3)}${headerText}${bs.horizontal.repeat(topPad)}${bs.topRight}`)];
		for (const line of innerLines) {
			const truncated = truncLine(line, contentWidth);
			const pad = Math.max(0, contentWidth - visibleWidth(truncated));
			lines.push(`${fg(bs.vertical)} ${truncated}${" ".repeat(pad)} ${fg(bs.vertical)}`);
		}
		lines.push(fg(`${bs.bottomLeft}${bs.horizontal.repeat(interior)}${bs.bottomRight}`));
		return lines;
	}
}
