...
export function buildPiArgs(input: BuildPiArgsInput): BuildPiArgsResult {
	...
	env[SUBAGENT_PARENT_CAPABILITY_TOKEN_ENV] = fanoutAuthorized
		...
		: "";
	env.PI_SUBAGENT_INHERIT_PROJECT_CONTEXT = input.bootContract ? "0" : input.inheritProjectContext ? "1" : "0";
	env.PI_SUBAGENT_INHERIT_SKILLS = input.bootContract ? "0" : input.inheritSkills ? "1" : "0";
	...
	if (effectiveMemoryPacket) {
		...
		const rendered = renderPathMemoryPacket(effectiveMemoryPacket, input.cwd ?? process.cwd());
		const memoryPacketPath = path.join(tempDir, "path-memory.md");
