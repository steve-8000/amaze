export interface RunnerSubagentStep {
	...
	memoryPacket?: import("../../harness/path-memory.ts").PathMemoryPacketInput;
	pathContract?: import("../../harness/path-contract.ts").PathContract;
	bootContract?: import("../../harness/fresh-boot-contract.ts").FreshBootContract;
}
...
export function resolveScoutSafeConcurrency(
	tasks: ReadonlyArray<{ agent?: string }>,
	concurrency: number,
): number {
	let scoutCount = 0;
	for (const task of tasks) {
		if (task.agent !== "scout") continue;
		scoutCount++;
		if (scoutCount > 1) return 1;
	}
	return concurrency;
}
