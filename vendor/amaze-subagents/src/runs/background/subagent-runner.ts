...
import { AMAZE_PACKAGE, getAmazeSpawnCommand, resolveInstalledAmazePackageRoot } from "../shared/amaze-spawn.ts";
...
import {
	...
	resolveScoutSafeConcurrency,
	MAX_PARALLEL_CONCURRENCY,
} from "../shared/parallel-utils.ts";
import { buildPiArgs, cleanupTempDir } from "../shared/amaze-args.ts";
...
import { freshBootContractToPathContract, freshBootContractToPathMemoryPacket } from "../../harness/fresh-boot-contract.ts";
import { outputSchemaForFreshBootContract, validateFreshBootOutputContract } from "../../harness/output-contract.ts";
import { evaluateActivityBudgetSnapshot, type PathContract } from "../../harness/path-contract.ts";
import { appendPathMemoryUpdates, extractMemoryUpdates } from "../../harness/path-memory.ts";
...
function runPiStreaming(
	...
	activityBudgetContract?: PathContract,
): Promise<RunPiStreamingResult> {
	return new Promise((resolve) => {
		const outputStream = fs.createWriteStream(outputFile, { flags: "w" });
		const spawnEnv = { ...process.env, ...(env ?? {}), ...getSubagentDepthEnv(maxSubagentDepth) };
		const spawnSpec = getAmazeSpawnCommand(args, {
		...
		function stopForActivityBudget(now: number): boolean {
			if (settled) return false;
			const decision = evaluateActivityBudgetSnapshot(activityBudgetContract, {
				tool_uses: toolCount,
				tokens: usage.input + usage.output,
				elapsed_ms: now - startTime,
			});
			if (decision.allowed) return false;
			error = decision.reason ?? "Path execution activity budget exceeded.";
			forcedTerminationSignal = trySignalChild(child, "SIGTERM") || forcedTerminationSignal;
			setTimeout(() => {
				if (settled) return;
		...
		const processStdoutLine = (line: string) => {
			...
			if (event.type === "tool_execution_start" && event.toolName) {
				toolCount += 1;
				if (stopForActivityBudget(Date.now())) return;
		...
		let budgetElapsedTimer: NodeJS.Timeout | undefined;
		let settled = false;
		...
		if (typeof activityBudgetContract?.activity_budget?.max_elapsed_ms === "number") {
			...
			}, activityBudgetContract.activity_budget.max_elapsed_ms + 1);
			budgetElapsedTimer.unref?.();
		}
		...
		const clearDrainTimers = () => {
			...
			if (budgetElapsedTimer) {
				clearTimeout(budgetElapsedTimer);
				budgetElapsedTimer = undefined;
			}
		};
...
function resolveAmazePackageRootFallback(): string {
	const root = resolveInstalledAmazePackageRoot();
	if (root) return root;
	throw new Error(`Could not resolve ${AMAZE_PACKAGE} package root`);
}

async function exportSessionHtml(sessionFile: string, outputDir: string, piPackageRoot?: string): Promise<string> {
	const pkgRoot = piPackageRoot ?? resolveAmazePackageRootFallback();
...
async function runSingleStep(
	...
	structuredOutputSchemaPath?: string;
	acceptance?: import("../../shared/types.ts").AcceptanceLedger;
	pathMemoryUpdates?: StepResult["pathMemoryUpdates"];
}> {
	const bootStructuredOutputSchema = step.bootContract
		? outputSchemaForFreshBootContract(step.bootContract, step.structuredOutput?.schema ?? step.structuredOutputSchema)
		: undefined;
	const effectiveStructuredOutput = step.bootContract
		? createStructuredOutputRuntime(bootStructuredOutputSchema!, path.join(path.dirname(ctx.outputFile), "structured-output"))
		: step.structuredOutput ?? (step.structuredOutputSchema
	...
	for (let index = 0; index < candidates.length; index++) {
		...
		const { args, env, tempDir } = buildPiArgs({
			...
			memoryPacket: step.memoryPacket,
			pathContract: step.pathContract,
		...
		const run = await runPiStreaming(
			...
			ctx.onChildEvent,
			step.bootContract ? freshBootContractToPathContract(step.bootContract) : step.pathContract,
		);
		...
		if (effectiveStructuredOutput && run.exitCode === 0 && !run.error && !hiddenError?.hasError) {
			...
			else if (step.bootContract) {
				const validation = validateFreshBootOutputContract(step.bootContract, structured.value);
				if (validation.status === "invalid") structuredError = `Output contract validation failed: ${validation.message}`;
	...
	const effectiveFinalError = acceptanceCanFailRun
		...
		: finalResult?.error;
	let pathMemoryUpdates: StepResult["pathMemoryUpdates"];
	if (!acceptanceFailure && effectiveFinalExitCode === 0 && !finalResult?.interrupted) {
		const structuredOutput = (finalResult as (RunPiStreamingResult & { structuredOutput?: unknown }) | undefined)?.structuredOutput;
		const updates = extractMemoryUpdates(structuredOutput, outputForAcceptance);
		const memoryPacket = step.bootContract ? freshBootContractToPathMemoryPacket(step.bootContract) : step.memoryPacket;
		const pathContract = step.bootContract ? freshBootContractToPathContract(step.bootContract) : step.pathContract;
		const appendResult = appendPathMemoryUpdates(memoryPacket, updates, step.cwd ?? ctx.cwd, Date.now, { pathContract });
		if (appendResult.written > 0 || appendResult.skipped > 0 || appendResult.warnings.length > 0) {
			pathMemoryUpdates = appendResult;
		}
	}
...
async function runSubagent(config: SubagentRunConfig): Promise<void> {
	...
	for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
		...
		if (isDynamicRunnerGroup(step)) {
			...
			const dynamicSteps = materialized.parallel.map((task) => ({
				...step.parallel,
			...
			const concurrency = resolveScoutSafeConcurrency(
				dynamicSteps,
			...
			for (const pr of normalizedParallelResults) {
				results.push({
					agent: pr.agent,
					output: pr.output,
					error: pr.error,
					...
					pathMemoryUpdates: pr.pathMemoryUpdates,
				});
			}
			...
			const failures = normalizedParallelResults.filter((result) => result.exitCode !== 0 && result.exitCode !== -1);
		...
		if (isParallelGroup(step)) {
			...
			const concurrency = resolveScoutSafeConcurrency(
				group.parallel,
				group.concurrency ?? MAX_PARALLEL_CONCURRENCY,
			);
			...
			try {
				...
				for (let t = 0; t < group.parallel.length; t++) {
					...
					const sessionTokens = config.sessionDir
						...
						: null;
					const taskTokens = sessionTokens ?? tokenUsageFromAttempts(normalizedParallelResults[t]?.modelAttempts);
				...
				for (let t = 0; t < group.parallel.length; t++) {
					...
					if (outputName) outputs[outputName] = outputEntryFromAsyncResult({
						agent: normalizedParallelResults[t]!.agent,
						output: normalizedParallelResults[t]!.output,
						structuredOutput: normalizedParallelResults[t]!.structuredOutput,
					}, stepIndex);
				}
				statusPayload.outputs = outputs;

				previousOutput = aggregateParallelOutputs(
					normalizedParallelResults.map((r) => ({
					agent: r.agent,
					output: r.output,
					exitCode: r.exitCode,
					error: r.error,
				...
				appendJsonl(eventsPath, JSON.stringify({
					...
					runId: id,
					stepIndex,
					success: normalizedParallelResults.every((r) => r.exitCode === 0 || r.exitCode === -1),
				}));

				if (normalizedParallelResults.some((r) => r.exitCode !== 0 && r.exitCode !== -1)) {
					break;
				}
			} finally {
			...
			results.push({
				...
				pathMemoryUpdates: singleResult.pathMemoryUpdates,
			});
	...
	try {
		writeAtomicJson(resultPath, {
			...
			results: results.map((r) => ({
				...
				pathMemoryUpdates: r.pathMemoryUpdates,
			})),
			outputs,
