...
import { applyThinkingSuffix } from "../shared/amaze-args.ts";
import { injectSingleOutputInstruction, normalizeSingleOutputOverride, resolveSingleOutputPath, validateFileOnlyOutputMode } from "../shared/single-output.ts";
import { buildChainInstructions, isDynamicParallelStep, isParallelStep, resolveStepBehavior, suppressProgressForReadOnlyTask, writeInitialProgressFile, type ChainStep, type ResolvedStepBehavior, type SequentialStep, type StepOverrides } from "../../shared/settings.ts";
import type { RunnerStep } from "../shared/parallel-utils.ts";
import { resolveAmazePackageRoot } from "../shared/amaze-spawn.ts";
...
const piPackageRoot = resolveAmazePackageRoot();
...
function spawnRunner(cfg: object, suffix: string, cwd: string): { pid?: number; error?: string } {
	...
	proc.on("error", (error) => {
		console.error(`[amaze-subagents] async spawn failed: ${error.message}`);
	});
...
const UNAVAILABLE_SUBAGENT_SKILL_ERROR = "Skills not found: amaze-subagents";
...
export function executeAsyncChain(
	...
	const buildSeqStep = (s: SequentialStep, sessionFile?: string, behaviorCwd?: string, progressPrecreated = false, resolvedBehavior?: ResolvedStepBehavior) => {
		...
		if (missingSkills.includes("amaze-subagents")) throw new UnavailableSubagentSkillError(UNAVAILABLE_SUBAGENT_SKILL_ERROR);
		...
		return {
			...
			pathContract: withDefaultScoutActivityBudget(s.agent, s.pathContract, s.bootContract),
			bootContract: s.bootContract,
...
export function executeAsyncSingle(
	...
	if (missingSkills.includes("amaze-subagents")) return formatAsyncStartError("single", UNAVAILABLE_SUBAGENT_SKILL_ERROR);
				...
				steps: [
					{
						...
						pathContract: withDefaultScoutActivityBudget(agent, params.pathContract, params.bootContract),
