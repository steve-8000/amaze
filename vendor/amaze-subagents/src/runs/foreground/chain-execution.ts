...
import type { ExtensionContext } from "amaze";
...
async function runParallelChainTasks(input: ParallelChainRunInput): Promise<SingleResult[]> {
	const concurrency = resolveScoutSafeConcurrency(
		input.step.parallel,
		input.step.concurrency ?? MAX_CONCURRENCY,
	);
	...
	for (let stepIndex = 0; stepIndex < chainSteps.length; stepIndex++) {
		...
		if (isParallelStep(step)) {
			...
			const r = await runSync(ctx.cwd, agents, seqStep.agent, stepTask, {
				...
				skills: behavior.skills === false ? [] : behavior.skills,
				structuredOutput: structuredRuntime,
				memoryPacket: seqStep.memoryPacket,
				pathContract: withDefaultScoutActivityBudget(seqStep.agent, seqStep.pathContract, seqStep.bootContract),
				bootContract: seqStep.bootContract,
