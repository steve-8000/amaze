...
import { evaluateActivityBudgetSnapshot } from "../../harness/path-contract.ts";
import { appendPathMemoryUpdates, extractMemoryUpdates } from "../../harness/path-memory.ts";
import { beginPathContractRun, finalizePathContractRun } from "../../harness/path-state.ts";
import {
	ensureArtifactsDir,
	getArtifactPaths,
	writeArtifact,
...
import { getAmazeSpawnCommand } from "../shared/amaze-spawn.ts";
...
function snapshotResult(result: SingleResult, progress: AgentProgress): SingleResult {
	return {
		...result,
		messages: result.outputMode === "file-only" && result.savedOutputPath ? [] : result.messages ? [...result.messages] : [],
		usage: { ...result.usage },
...
async function runSingleAttempt(
	...
	const effectiveStructuredOutput = options.bootContract
		? createStructuredOutputRuntime(outputSchemaForFreshBootContract(options.bootContract, options.structuredOutput?.schema))
	...
	const { args, env: sharedEnv, tempDir } = buildPiArgs({
		...
		parentEventSink: options.nestedRoute?.eventSink,
			...
			memoryPacket: options.memoryPacket,
			pathContract: options.pathContract,
	...
	const activityBudgetContract = options.bootContract ? freshBootContractToPathContract(options.bootContract) : options.pathContract;
	if (effectiveStructuredOutput) {
		try {
			if (existsSync(effectiveStructuredOutput.outputPath)) unlinkSync(effectiveStructuredOutput.outputPath);
		} catch {
	...
	if (effectiveStructuredOutput && result.exitCode === 0 && !result.error) {
		const structured = readStructuredOutput({
			schema: effectiveStructuredOutput.schema,
			schemaPath: effectiveStructuredOutput.schemaPath,
			outputPath: effectiveStructuredOutput.outputPath,
		});
		result.structuredOutputSchemaPath = effectiveStructuredOutput.schemaPath;
		result.structuredOutputPath = effectiveStructuredOutput.outputPath;
		if (structured.error) {
			result.exitCode = 1;
			result.error = structured.error;
		} else if (options.bootContract) {
			const validation = validateFreshBootOutputContract(options.bootContract, structured.value);
			if (validation.status === "invalid") {
				result.exitCode = 1;
				result.error = `Output contract validation failed: ${validation.message}`;
			} else {
	...
	result.progressSummary = {
		...
		const acceptanceOutput = getFinalOutput(messages);
...
export async function runSync(
	...
	const contractRun = beginPathContractRun(pathContract, cwd, {
		runId: options.runId,
		agent: agentName,
		task,
	});
	...
	try {
		result = await runSyncUnlocked(runtimeCwd, agents, agentName, task, options);
		finalizePathContractRun(contractRun, result.exitCode === 0 && !result.error ? "completed" : "failed", {
			exitCode: result.exitCode,
			error: result.error,
		});
		return result;
	} catch (error) {
		finalizePathContractRun(contractRun, "failed", {
...
async function runSyncUnlocked(
	...
	if (skillNames.some((skill) => skill.trim() === "amaze-subagents") && missingSkills.includes("amaze-subagents")) {
		return {
			agent: agentName,
			task,
			exitCode: 1,
			messages: [],
			usage: emptyUsage(),
			error: "Skills not found: amaze-subagents",
		};
	}
	...
	if (options.sessionFile && (existsSync(options.sessionFile) || resultMessages.length)) {
	...
	if (!acceptanceFailure && result.exitCode === 0 && !result.detached && !result.interrupted) {
		...
		if (appendResult.written > 0 || appendResult.skipped > 0 || appendResult.warnings.length > 0) {
			result.pathMemoryUpdates = appendResult;
		}
	}
