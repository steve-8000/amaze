...
import { resolveScoutSafeConcurrency } from "../shared/parallel-utils.ts";
import { withDefaultScoutActivityBudget } from "../shared/default-activity-budget.ts";
import {
...
import { applyRuntimeArchitecturePolicy } from "../shared/runtime-architecture-policy.ts";
...
interface ExecutionContextData {
	...
	chainTaskContexts?: SubagentExecutionContext[];
	parallelTaskContexts?: SubagentExecutionContext[];
}
...
function resolveRememberedForegroundRun(state: SubagentState, requested: string | undefined): ForegroundResumeRun | undefined {
	if (!state.foregroundRuns?.size) return undefined;
	if (!requested) {
...
function compactRememberedForegroundText(text: string | undefined, maxLength = 700): string | undefined {
	const compact = text
		?.split(/\r?\n/)
		.map((line) => line.trim())
		.filter(Boolean)
...
function rememberedForegroundStatusResult(run: ForegroundResumeRun): AgentToolResult<Details> {
	const failed = run.children.filter((child) => child.status === "failed");
	const state = failed.length > 0 ? "failed" : "completed";
	const lines = [
		`Run: ${run.runId}`,
		`State: ${state}`,
		`Mode: ${run.mode}`,
	...
	for (const child of run.children) {
		lines.push(`- #${child.index + 1} ${child.agent}: ${child.status}`);
		const reason = compactRememberedForegroundText(child.error) ?? compactRememberedForegroundText(child.finalOutput, 350);
...
function applyAgentDefaultContext(params: SubagentParamsLike, agents: AgentConfig[]): SubagentParamsLike {
	...
	if ((params.tasks?.length ?? 0) > 0) return params;
	if ((params.chain?.length ?? 0) > 0) return params;
...
function freshBootTaskText(contract: FreshBootContract): string {
	return [
		`Execute FreshBootContract ${contract.contract_id}.`,
		"",
		`Goal: ${contract.execution_contract.goal}`,
		`Assigned path: ${contract.execution_contract.assigned_path}`,
		`Context packet: ${contract.context_packet.path}`,
...
function syntheticPathSpecialistAgent(contract: FreshBootContract): AgentConfig {
	const name = contract.execution_contract.assigned_specialist;
	return {
		name,
		localName: name,
		description: `Contract-bound path specialist for ${contract.execution_contract.assigned_path}`,
		systemPromptMode: "replace",
		inheritProjectContext: false,
		inheritSkills: false,
		defaultContext: "fresh",
		systemPrompt: [
			"You are a fresh path specialist runtime.",
			"Do not infer parent conversation or role prompts.",
			"Follow the injected FreshBootContract and execution contract exactly.",
		].join("\n"),
		source: "project",
		filePath: `<fresh-boot-contract:${contract.contract_id}>`,
		completionGuard: false,
	};
}

export function normalizeContractFirstExecution(
	params: SubagentParamsLike,
	agents: AgentConfig[],
): { params: SubagentParamsLike; agents: AgentConfig[]; error?: string } {
	if (!params.bootContract || (params.chain?.length ?? 0) > 0 || (params.tasks?.length ?? 0) > 0) {
		return { params, agents };
	}
	const contract = parseFreshBootContract(params.bootContract);
	if (!contract) {
		return { params, agents, error: "Invalid FreshBootContract: bootContract must be fresh-only and include specialist, context_packet, and execution_contract." };
	}
	const agentName = params.agent ?? contract.execution_contract.assigned_specialist;
	const task = params.task ?? freshBootTaskText(contract);
	...
	if (agents.some((agent) => agent.name === agentName)) return { params: normalizedParams, agents };
	return {
		params: normalizedParams,
		agents: [...agents, syntheticPathSpecialistAgent({ ...contract, execution_contract: { ...contract.execution_contract, assigned_specialist: agentName } })],
	};
}

function resolveAgentExecutionContext(agent: AgentConfig | undefined, explicitContext: SubagentParamsLike["context"]): SubagentExecutionContext {
...
function wrapChainTasksForContexts(chain: ChainStep[] | undefined, contexts: SubagentExecutionContext[] | undefined): ChainStep[] | undefined {
	if (!chain || !contexts) return chain;
	let flatIndex = 0;
	return chain.map((step, stepIndex) => {
		if (isParallelStep(step)) {
			return {
				...
				parallel: step.parallel.map((task) => {
					const context = contexts[flatIndex++];
					return context === "fork" && !task.memoryPacket && !task.pathContract && !task.bootContract ? { ...task, task: wrapForkTask(task.task ?? "{previous}") } : task;
				}),
			};
		}
		const dynamic = step as DynamicParallelStep;
		if (dynamic.expand && dynamic.parallel) {
			const context = contexts[flatIndex++];
			return context === "fork" && !dynamic.parallel.memoryPacket && !dynamic.parallel.pathContract && !dynamic.parallel.bootContract
				? { ...dynamic, parallel: { ...dynamic.parallel, task: wrapForkTask(dynamic.parallel.task ?? "{previous}") } }
				: step;
		}
		const sequential = step as SequentialStep;
		const context = contexts[flatIndex++];
		return context === "fork" && !sequential.memoryPacket && !sequential.pathContract && !sequential.bootContract ? { ...sequential, task: wrapForkTask(sequential.task ?? (stepIndex === 0 ? "{task}" : "{previous}")) } : step;
	});
}

function shouldUseChildArtifactOutput(input: {
	task: TaskParam;
	agentConfig: AgentConfig | undefined;
	artifactConfig: ArtifactConfig;
}): boolean {
	return input.artifactConfig.enabled !== false
		&& input.task.output === undefined
		&& Boolean(input.agentConfig?.output);
}

function resolveParallelDefaultArtifactOutput(
	artifactsDir: string,
	runId: string,
	task: TaskParam,
	index: number,
	...
	if (!shouldUseChildArtifactOutput({ task, agentConfig, artifactConfig })) return undefined;
	return getArtifactPaths(artifactsDir, runId, task.agent, index).outputPath;
}
...
function runAsyncPath(data: ExecutionContextData, deps: ExecutorDeps): AgentToolResult<Details> | null {
	...
	if (!isAsyncAvailable()) {
		return {
			content: [{ type: "text", text: "Async mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the amaze-subagents package dependencies are installed." }],
			isError: true,
			details: { mode: "single" as const, results: [] },
		};
	}
	...
	if (hasTasks && params.tasks) {
		...
		const parallelTasks = params.tasks.map((task, index) => ({
			agent: task.agent,
			task: data.parallelTaskContexts?.[index] === "fork" ? wrapForkTask(task.task) : task.task,
			cwd: task.cwd,
			...(modelOverrides[index] ? { model: modelOverrides[index] } : {}),
			...(skillOverrides[index] !== undefined ? { skill: skillOverrides[index] } : {}),
			...(task.output === true
				? (agentConfigs[index]?.output ? { output: agentConfigs[index]!.output } : {})
				: task.output !== undefined
					? { output: task.output }
					: resolveParallelDefaultArtifactOutput(artifactsDir, id, task, index, agentConfigs[index], artifactConfig)
						? { output: resolveParallelDefaultArtifactOutput(artifactsDir, id, task, index, agentConfigs[index], artifactConfig)! }
						: {}),
		...
		return executeAsyncChain(id, {
			chain: [{
				...
				concurrency: resolveScoutSafeConcurrency(
					parallelTasks,
					resolveTopLevelParallelConcurrency(params.concurrency, deps.config.parallel?.concurrency),
				),
	...
	if (hasChain && params.chain) {
		...
		const chain = wrapChainTasksForContexts(params.chain as ChainStep[], data.chainTaskContexts) ?? (params.chain as ChainStep[]);
		return executeAsyncChain(id, {
			chain,
			task: params.task,
			agents,
	...
	if (hasSingle) {
		...
		return executeAsyncSingle(id, {
			agent: params.agent!,
			task: params.context === "fork" && !params.memoryPacket && !params.pathContract && !params.bootContract ? wrapForkTask(params.task ?? "") : (params.task ?? ""),
			agentConfig: a,
			ctx: asyncCtx,
			availableModels,
			cwd: effectiveCwd,
...
async function runChainPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	if (chainResult.requestedAsync) {
		if (!isAsyncAvailable()) {
			return {
				content: [{ type: "text", text: "Background mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the amaze-subagents package dependencies are installed." }],
				isError: true,
				details: { mode: "chain" as const, results: [] },
			};
		}
		...
		const asyncChain = wrapChainTasksForContexts(chainResult.requestedAsync.chain, data.chainTaskContexts)
...
async function runForegroundParallelTasks(input: ForegroundParallelRunInput): Promise<SingleResult[]> {
	return mapConcurrent(input.tasks, input.concurrencyLimit, async (task, index) => {
		...
		return runSync(input.ctx.cwd, input.agents, task.agent, taskText, {
			...
			pathContract: withDefaultScoutActivityBudget(task.agent, task.pathContract, task.bootContract),
...
async function runParallelPath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	const maxParallelTasks = resolveTopLevelParallelMaxTasks(deps.config.parallel?.maxTasks);
	const parallelConcurrency = resolveScoutSafeConcurrency(
		tasks,
		resolveTopLevelParallelConcurrency(params.concurrency, deps.config.parallel?.concurrency),
	);
	...
	if (params.clarify === true && ctx.hasUI) {
		...
		if (result.runInBackground) {
			...
			const parallelTasks = tasks.map((t, i) => {
				const taskText = data.parallelTaskContexts?.[i] === "fork" ? wrapForkTask(taskTexts[i]!) : taskTexts[i]!;
				const progress = taskDisallowsFileUpdates(taskText) ? false : behaviorOverrides[i]?.progress;
				const defaultArtifactOutput = resolveParallelDefaultArtifactOutput(artifactsDir, id, t, i, agentConfigs[i], artifactConfig);
				return {
					agent: t.agent,
					task: taskText,
					cwd: t.cwd,
					...
					...(behaviorOverrides[i]?.output !== undefined ? { output: behaviorOverrides[i]!.output } : defaultArtifactOutput ? { output: defaultArtifactOutput } : {}),
	...
	try {
		...
		for (let i = 0; i < behaviors.length; i++) {
			const defaultArtifactOutput = resolveParallelDefaultArtifactOutput(artifactsDir, runId, tasks[i]!, i, agentConfigs[i], artifactConfig);
			if (defaultArtifactOutput) behaviors[i] = { ...behaviors[i]!, output: defaultArtifactOutput };
		}

		if (params.context === "fork" || data.parallelTaskContexts?.some((context) => context === "fork")) {
			for (let i = 0; i < taskTexts.length; i++) {
				if (data.parallelTaskContexts?.[i] === "fork") taskTexts[i] = wrapForkTask(taskTexts[i]!);
			}
		}
...
async function runSinglePath(data: ExecutionContextData, deps: ExecutorDeps): Promise<AgentToolResult<Details>> {
	...
	if (params.clarify === true && ctx.hasUI) {
		...
		if (result.runInBackground) {
			...
			return executeAsyncSingle(id, {
				agent: params.agent!,
				task: params.context === "fork" && !params.memoryPacket && !params.pathContract && !params.bootContract ? wrapForkTask(task) : task,
				agentConfig,
				ctx: asyncCtx,
				availableModels,
				cwd: effectiveCwd,
	...
	if (params.context === "fork" && !params.memoryPacket && !params.pathContract && !params.bootContract) {
	...
	const r = await runSync(ctx.cwd, agents, params.agent!, task, {
		...
		memoryPacket: params.memoryPacket,
		pathContract: withDefaultScoutActivityBudget(params.agent!, params.pathContract, params.bootContract),
		bootContract: params.bootContract,
	});
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		let paramsWithResolvedCwd = params.cwd === undefined ? params : { ...params, cwd: requestCwd };
		if (params.action === "harness_run_contract") {
			if (!params.bootContract) {
				return {
					content: [{ type: "text", text: "harness_run_contract requires bootContract." }],
					isError: true,
					details: { mode: "single" as const, results: [] },
				};
			}
		...
		if (params.action) {
			...
			if (action === "harness_validate_contract") {
				...
				if (!contract) {
					return {
						content: [{ type: "text", text: "Invalid FreshBootContract: bootContract must be fresh-only and include specialist, context_packet, memory_attachments, and execution_contract." }],
						isError: true,
						details: { mode: "management" as const, results: [] },
					};
				}
				const report = validateHarnessValidatorContract(contract);
				return {
					content: [{
						type: "text",
						text: JSON.stringify({
							...
							validator_contract: report.validator_contract,
			...
			if (action === "harness_status") {
				action = "status";
				params = { ...params, action: "status" };
				paramsWithResolvedCwd = { ...paramsWithResolvedCwd, action: "status" };
			}
			if (action === "doctor") {
			...
			if (action === "status") {
				...
				if (targetRunId) {
					...
					const remembered = resolveRememberedForegroundRun(deps.state, targetRunId);
					if (remembered) return rememberedForegroundStatusResult(remembered);
				} else {
					const foreground = getForegroundControl(deps.state, undefined);
					if (foreground) return foregroundStatusResult(foreground);
					const remembered = resolveRememberedForegroundRun(deps.state, undefined);
					if (remembered) return rememberedForegroundStatusResult(remembered);
				}
		...
		let discoveredAgents = deps.discoverAgents(effectiveCwd, scope).agents;
		const contractFirst = normalizeContractFirstExecution(effectiveParams, discoveredAgents);
		if (contractFirst.error) return toExecutionErrorResult(effectiveParams, new Error(contractFirst.error));
		effectiveParams = contractFirst.params;
		discoveredAgents = contractFirst.agents;
		const architecturePolicy = applyRuntimeArchitecturePolicy(effectiveParams, discoveredAgents);
		...
		const parallelTaskContexts = resolveTopLevelParallelTaskContexts(effectiveParams, discoveredAgents);
		...
		try {
			const needsForkContext = effectiveParams.context === "fork"
				|| (chainTaskContexts?.some((context) => context === "fork") ?? false)
				|| (parallelTaskContexts?.some((context) => context === "fork") ?? false);
			const forkContextResolver = needsForkContext
				? createForkContextResolver(ctx.sessionManager, "fork")
				: { sessionFileForIndex: () => undefined };
			sessionFileForIndex = (index = 0): string | undefined => {
				if (parallelTaskContexts) {
					return parallelTaskContexts[index] === "fork" ? forkContextResolver.sessionFileForIndex(index) : undefined;
				}
				if (chainTaskContexts) {
					return chainTaskContexts[index] === "fork" ? forkContextResolver.sessionFileForIndex(index) : undefined;
				}
				return forkContextResolver.sessionFileForIndex(index);
			};
		} catch (error) {
