...
/**
 * amaze-subagents installer
 * 
 * Usage:
 *   npx amaze-subagents          # Install to ~/.pi/agent/extensions/subagent
 *   npx amaze-subagents --remove # Remove the extension
 */
...
const REPO_URL = "https://github.com/nicobailon/amaze-subagents.git";
...
if (isHelp) {
	console.log(`
amaze-subagents - Pi extension for delegating tasks to subagents

Usage:
  npx amaze-subagents          Install the extension
  npx amaze-subagents --remove Remove the extension
  npx amaze-subagents --help   Show this help
...
if (isRemove) {
	if (fs.existsSync(EXTENSION_DIR)) {
		...
		console.log("amaze-subagents removed");
	} else {
		console.log("amaze-subagents is not installed");
	}
	process.exit(0);
}

// Install
console.log("Installing amaze-subagents...\n");
...
if (fs.existsSync(EXTENSION_DIR)) {
	...
	if (isGitRepo) {
		...
		try {
			...
			console.log("\namaze-subagents updated");
		} catch (err) {
			console.error("Failed to update. Try removing and reinstalling:");
			console.error("  npx amaze-subagents --remove && npx amaze-subagents");
			process.exit(1);
		}
	} else {
		console.log(`Directory exists but is not a git repo: ${EXTENSION_DIR}`);
		console.log("Remove it first with: npx amaze-subagents --remove");
		process.exit(1);
	}
} else {
	// Fresh install
