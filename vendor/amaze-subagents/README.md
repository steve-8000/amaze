<p>
  <img src="https://raw.githubusercontent.com/nicobailon/amaze-subagents/main/banner.png" alt="amaze-subagents" width="1100">
</p>

# amaze-subagents

`amaze-subagents` lets Pi delegate work to focused child agents. Use it for code review, scouting, implementation, parallel audits, saved workflows, background jobs, and anything else that benefits from a second or third set of model eyes.
...
```bash
pi install npm:amaze-subagents
```
...
## FreshBootContract harness mode

For path-based persistent specialist execution, prefer a contract-first run instead of `agent + task + context`:
...
Useful harness actions:

- `harness_validate_contract` validates the FreshBootContract and returns a `validator_contract` report without spawning a child process.
- `harness_run_contract` runs the contract as a fresh-only child process. The runtime derives `agent` and `task` from the execution contract if they are omitted.
- `harness_status` is the contract-aware status alias.

Harness runs always boot fresh, disable parent context inheritance, inject only the execution contract/context packet/read-only path memory, enforce read/write/budget boundaries, validate `output_required`, and append `memory_updates` only after validation. Path memory packets include a deterministic Xenonite namespace such as `path:packages/coding-agent/src/runtime`.

## Optional pi-intercom companion

`amaze-subagents` works without `pi-intercom`. Install `pi-intercom` only if you want child agents to talk back to the parent Pi session while they are running.

```bash
pi install npm:pi-intercom
```

Most users do not call `intercom` directly. After `pi-intercom` is installed, `amaze-subagents` can automatically give child agents a private coordination channel back to the parent session. The bridge recognizes the normal `pi install npm:pi-intercom` package install as well as legacy local extension checkouts.
...
The package bundles a `amaze-subagents` skill that is automatically available to the parent agent when the extension is installed. It is for the orchestrating parent only: child subagents never receive it, and their context is explicitly filtered to strip parent-only orchestration instructions.
...
`amaze-subagents` reads optional JSON config from `~/.pi/agent/extensions/subagent/config.json`.
...
```text
<tmpdir>/amaze-subagents-<scope>/chain-runs/{runId}/
```
...
```text
<tmpdir>/amaze-subagents-<scope>/async-subagent-runs/<id>/
  status.json
  events.jsonl
  output-<n>.log
  subagent-log-<id>.md
