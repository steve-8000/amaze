...
`amaze-subagents` works without `pi-intercom`. When `pi-intercom` is installed and enabled, the intercom bridge can automatically give child agents a private coordination channel back to the parent session.
...
`amaze-subagents`. This is useful when a slash command should always run through a
