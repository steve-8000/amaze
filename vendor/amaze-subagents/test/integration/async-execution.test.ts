...
import { createEventBus, createMockAmaze, createTempDir, events, makeAgent, makeMinimalCtx, removeTempDir, tryImport } from "../support/helpers.ts";
...
function readLastMockPiArgs(mockPi: MockAmaze): string[] {
...
function readMockPiArgs(mockPi: MockAmaze, index: number): string[] {
	const callFile = fs.readdirSync(mockPi.dir)
		.filter((name) => name.startsWith("call-") && name.endsWith(".json"))
		.sort()
		.at(index);
...
describe("async execution utilities", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("background single runs report unavailable amaze-subagents skill requests", () => {
		const id = `async-amaze-subagents-skill-${Date.now().toString(36)}`;
		const result = executeAsyncSingle(id, {
			...
			shareEnabled: false,
			sessionRoot: path.join(tempDir, "sessions"),
			skills: ["amaze-subagents"],
			maxSubagentDepth: 2,
		});

		assert.equal(result.isError, true);
		assert.match(result.content[0]?.text ?? "", /Skills not found: amaze-subagents/);
	});

	it("background chains report unavailable amaze-subagents skill requests", () => {
		const id = `async-chain-amaze-subagents-skill-${Date.now().toString(36)}`;
		const result = executeAsyncChain(id, {
			chain: [{ agent: "worker", task: "Do work", skill: ["amaze-subagents"] }],
			agents: [makeAgent("worker")],
			ctx: { pi: { events: { emit() {} } }, cwd: tempDir, currentSessionId: "session-1" },
			cwd: tempDir,
			artifactConfig: {
