...
import { createEventBus, createMockAmaze, createTempDir, events, removeTempDir, tryImport } from "../support/helpers.ts";
...
describe("fork context execution wiring", { skip: !available ? "subagent executor not importable" : undefined }, () => {
	...
	it("uses agent defaultContext fork per child for top-level parallel when launch context is omitted", async () => {
		const parentSessionFile = path.join(tempDir, "parent.jsonl");
		const { manager, openedPaths, branchedLeafIds } = makeForkingSessionManagerRecorder({ sessionFile: parentSessionFile, leafId: "leaf-current" });
		const executor = makeExecutorWithDiscoverAgents(() => ({
			agents: [
		...
		assert.equal(sessionArgs.filter((sessionFile) => sessionFile === path.join(tempDir, "fork-1.jsonl")).length, 1);
		assert.equal(sessionArgs.some((sessionFile) => sessionFile.includes(`${path.sep}run-1${path.sep}session.jsonl`)), true);
		const taskArgs = readAllCallArgs().map((args) => args[args.length - 1] ?? "");
		assert.equal(taskArgs.some((task) => task.includes("Task:\none")), true);
		assert.equal(taskArgs.some((task) => task.startsWith("Task: two\n\n## Acceptance Contract")), true);
	});
	...
	it("uses agent defaultContext fork per step for chain runs when launch context is omitted", async () => {
		const parentSessionFile = path.join(tempDir, "parent.jsonl");
		const { manager, openedPaths, branchedLeafIds } = makeForkingSessionManagerRecorder({ sessionFile: parentSessionFile, leafId: "leaf-current" });
		const executor = makeExecutorWithDiscoverAgents(() => ({
			agents: [
		...
		assert.deepEqual(branchedLeafIds, ["leaf-current"]);
		const sessionArgs = readSessionArgsFromCalls();
		assert.equal(sessionArgs.some((sessionFile) => sessionFile.includes(`${path.sep}run-0${path.sep}session.jsonl`)), true);
		assert.equal(sessionArgs.filter((sessionFile) => sessionFile === path.join(tempDir, "fork-1.jsonl")).length, 1);
		const taskArgs = readAllCallArgs().map((args) => args[args.length - 1] ?? "");
		assert.equal(taskArgs.some((task) => task.startsWith("Task: scan")), true);
		assert.equal(taskArgs.some((task) => task.includes("Task:\nwrite")), true);
	});
