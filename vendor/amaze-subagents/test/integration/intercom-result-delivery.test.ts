...
interface ExecutorModule {
	createSubagentExecutor?: (...args: unknown[]) => {
		execute: (
			...
			signal: AbortSignal,
			onUpdate: ((result: { details?: { progress?: Array<{ currentTool?: string }> } }) => void) | undefined,
			ctx: unknown,
...
describe("intercom result delivery cutover", { skip: !available ? "executor not importable" : undefined }, () => {
	...
	before(() => {
		...
		originalAgentDir = process.env.AMAZE_CODING_AGENT_DIR;
		homeDir = createTempDir("pi-subagent-intercom-home-");
		process.env.HOME = homeDir;
		process.env.USERPROFILE = homeDir;
		process.env.AMAZE_CODING_AGENT_DIR = path.join(homeDir, ".pi", "agent");
	...
	after(() => {
		...
		if (originalAgentDir === undefined) delete process.env.AMAZE_CODING_AGENT_DIR;
		else process.env.AMAZE_CODING_AGENT_DIR = originalAgentDir;
	...
	it("mixed foreground outcomes produce failed grouped status and receipt counts", async () => {
		...
		assert.match(statusText, /State: failed/);
		assert.match(statusText, /#\d+ [ab]: failed/);
		assert.match(statusText, /reason: Parallel child failure/);
	});
});
