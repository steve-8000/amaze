...
import type { MockAmaze } from "../support/helpers.ts";
import {
	createEventBus,
	createMockAmaze,
	createTempDir,
...
describe("parallel agent execution", { skip: !piAvailable ? "pi packages not available" : undefined }, () => {
	let tempDir: string;
	let mockPi: MockAmaze;

	before(() => {
		mockPi = createMockAmaze();
		mockPi.install();
	});
	...
	function readAllCallArgs(): string[][] {
		return fs.readdirSync(mockPi.dir)
			.filter((name) => name.startsWith("call-"))
			.sort()
			.map((callFile) => JSON.parse(fs.readFileSync(path.join(mockPi.dir, callFile), "utf-8")).args as string[]);
	}

	function readAllCallStartTimes(): number[] {
		return fs.readdirSync(mockPi.dir)
			.filter((name) => name.startsWith("call-"))
			.sort()
			.map((callFile) => Number(callFile.split("-")[1]));
	}
	...
	it("uses unique artifact output for top-level parallel agent default output", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		mockPi.onCall({ output: "Scout result" });
		const executor = makeExecutor([
		...
		const result = await executor.execute(
			"parallel-default-output-artifact",
			{
				tasks: [
					{ agent: "scout", task: "Scan runtime" },
					{ agent: "echo", task: "Echo runtime" },
				],
				artifacts: true,
			},
			new AbortController().signal,
		...
		assert.equal(result.isError, undefined);
		const taskArgs = readAllCallArgs().map((args) => args[args.length - 1] ?? "");
		const scoutTask = taskArgs.find((task) => task.includes("Scan runtime")) ?? "";
		assert.match(scoutTask, /_scout_0_output\.md/);
		assert.doesNotMatch(scoutTask, /Write your findings to: .*context\.md/);
		const artifactOutputs: string[] = (result.details?.artifacts?.files ?? []).map((file: { outputPath: string }) => file.outputPath);
		assert.equal(artifactOutputs.some((outputPath) => outputPath.endsWith("_scout_0_output.md")), true);
	});
	...
	it("serializes duplicated scout agents in top-level parallel runs", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		mockPi.onCall({ output: "Scout done", delay: 150 });
		const executor = makeExecutor([makeAgent("scout")]);

		const result = await executor.execute(
			"parallel-scout-serialized",
			{
				tasks: [
					{ agent: "scout", task: "Scan A" },
					{ agent: "scout", task: "Scan B" },
				],
				concurrency: 2,
			},
			new AbortController().signal,
		...
		const startTimes = readAllCallStartTimes();
		assert.equal(result.isError, undefined);
		assert.equal(startTimes.length, 2);
		assert.ok(startTimes[1]! - startTimes[0]! >= 100, "second scout should start after the first scout completes");
	});
...
## Acceptance Contract`));
	...
	it("top-level parallel progress emits the existing progress instruction style", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		const args = readLastCallArgs();
		assert.ok((args[args.length - 1] ?? "").includes(`Update progress at: ${path.join(tempDir, "progress.md")}`));
