 ...
 *
 * Uses the local createMockAmaze() helper to simulate the pi CLI.
...
describe("single sync execution", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("fails foreground runs on explicit unavailable amaze-subagents skill requests without spawning", async () => {
		const agents = [makeAgent("worker")];

		const result = await runSync(tempDir, agents, "worker", "Task", { skills: ["amaze-subagents"] });

		assert.equal(result.exitCode, 1);
		assert.equal(result.error, "Skills not found: amaze-subagents");
		assert.equal(mockPi.callCount(), 0);
	});

	it("fails foreground runs when an agent default requests amaze-subagents skill", async () => {
		const agents = [makeAgent("worker", { skills: ["amaze-subagents"] })];
