...
export interface MockAmaze {
	readonly dir: string;
	install(): void;
	uninstall(): void;
	onCall(response: MockAmazeResponse): void;
	reset(): void;
	callCount(): number;
}

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const SCRIPT_PATH = path.join(__dirname, "mock-amaze-script.mjs");
...
export function createMockAmaze(): MockAmaze {
	...
	const shellScriptPath = path.join(binDir, "amaze");
