...
describe("registered subagent tool description", () => {
	it("does not advertise hardcoded builtin agent names", () => {
		...
		assert.match(description, /harness_validate_contract/);
		assert.match(description, /harness_run_contract/);
		assert.match(description, /FreshBootContract/);
		assert.match(description, /validator_contract/);
	});
});
