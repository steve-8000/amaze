...
function result(agent: string, output: string, overrides: Record<string, unknown> = {}) {
	return {
		agent,
		task: `${agent} task`,
		exitCode: 0,
...
test("parallel result rows include model and thinking", () => {
	const parallel = componentText(renderSubagentResult({
		content: [{ type: "text", text: "" }],
		details: {
			mode: "parallel",
			results: [
				result("scout", "a", { model: "gpt-5.3-codex-spark:xhigh", thinking: "xhigh" }),
				result("reviewer", "b", { model: "gpt-5.5" }),
			],
		},
	}, { expanded: false }, theme as any));

	assert.match(parallel, /Agent 1\/2: scout \(gpt-5\.3-codex-spark · thinking xhigh\)/);
	assert.match(parallel, /Agent 2\/2: reviewer \(gpt-5\.5\)/);
});
