...
describe("builtin agent overrides", () => {
	beforeEach(() => {
		tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-subagents-home-"));
		tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-subagents-project-"));
	...
	it("applies defaultThinkingLevel only to builtin subagents without explicit thinking", () => {
		writeJson(path.join(tempHome, ".pi", "agent", "settings.json"), {
