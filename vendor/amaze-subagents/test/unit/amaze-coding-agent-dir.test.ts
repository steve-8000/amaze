...
describe("AMAZE_CODING_AGENT_DIR runtime paths", () => {
	beforeEach(() => {
		oldAgentDir = process.env.AMAZE_CODING_AGENT_DIR;
		tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-dir-"));
	...
	it("resolves the agent dir dynamically and loads extension config from it", () => {
		...
		process.env.AMAZE_CODING_AGENT_DIR = "~";
		assert.equal(getAgentDir(), os.homedir());

		process.env.AMAZE_CODING_AGENT_DIR = "~/custom-agent-dir";
