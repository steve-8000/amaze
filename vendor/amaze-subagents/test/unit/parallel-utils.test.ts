...
function makeStep(agent: string, task: string): RunnerSubagentStep {
	return { agent, task, inheritProjectContext: true, inheritSkills: true };
}
...
describe("isParallelGroup", () => {
	...
	it("returns false for a sequential step", () => {
		const step = makeStep("a", "do stuff");
...
describe("flattenSteps", () => {
	...
	it("expands parallel groups into individual steps", () => {
		const steps: RunnerStep[] = [
			makeStep("scout", "find info"),
			{
				parallel: [
					makeStep("reviewer-a", "review part 1"),
					makeStep("reviewer-b", "review part 2"),
				],
			},
			makeStep("summarizer", "combine"),
		];
		const flat = flattenSteps(steps);
...
describe("resolveScoutSafeConcurrency", () => {
	it("keeps requested concurrency when fewer than two scout agents are present", () => {
		assert.equal(resolveScoutSafeConcurrency([{ agent: "scout" }, { agent: "reviewer" }], 4), 4);
		assert.equal(resolveScoutSafeConcurrency([{ agent: "reviewer" }, { agent: "worker" }], 3), 3);
	});

	it("serializes parallel groups with multiple scout agents", () => {
		assert.equal(resolveScoutSafeConcurrency([{ agent: "scout" }, { agent: "scout" }], 4), 1);
	});
});
