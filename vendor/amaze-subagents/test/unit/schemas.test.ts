...
describe("SubagentParams schema", { skip: !schemasAvailable ? "typebox not available" : undefined }, () => {
	...
	it("accepts contract-first bootContract input without agent or task", () => {
		const bootContractSchema = SubagentParams?.properties?.bootContract;
		assert.ok(bootContractSchema, "bootContract schema should exist");
		assert.equal(bootContractSchema.type, "object");
		assert.match(String(bootContractSchema.description ?? ""), /FreshBootContract/);
		if (!CompileSchema) return;
