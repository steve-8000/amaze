...
import { getAmazeSpawnCommand, resolveWindowsPiCliScript, type PiSpawnDeps } from "../../src/runs/shared/amaze-spawn.ts";
...
describe("getAmazeSpawnCommand", () => {
	it("uses plain pi on non-Windows even when argv1 is a runnable JS file", () => {
		...
		const result = getAmazeSpawnCommand(args, deps);
		assert.deepEqual(result, { command: "amaze", args });
	});
	...
	it("falls back to plain pi command on non-Windows when CLI script cannot be resolved", () => {
		const args = ["--mode", "json", "Task: check output"];
		const result = getAmazeSpawnCommand(args, { platform: "darwin" });
		assert.deepEqual(result, { command: "amaze", args });
	});
	...
	it("resolves CLI script from package bin when argv1 is not runnable JS", () => {
		...
		const result = getAmazeSpawnCommand(["-p", "Task: hello"], deps);
		assert.equal(result.command, "/usr/local/bin/node");
	...
	it("walks from package main entry to resolve package bin", () => {
		const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-spawn-package-root-"));
		try {
			const packageRoot = path.join(tempDir, "node_modules", "@steve-8000", "amaze");
			...
			fs.writeFileSync(path.join(packageRoot, "package.json"), JSON.stringify({ name: "amaze", bin: { pi: "dist/cli/index.js" } }));
			const result = getAmazeSpawnCommand(["-p", "Task: hello"], {
				platform: "win32",
				execPath: "/usr/local/bin/node",
				argv1: "/opt/pi/subagent-runner.ts",
...
describe("getAmazeSpawnCommand with piPackageRoot", () => {
