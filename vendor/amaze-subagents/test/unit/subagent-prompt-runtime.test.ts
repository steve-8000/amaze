...
import { SUBAGENT_FANOUT_CHILD_ENV } from "../../src/runs/shared/amaze-args.ts";
...
describe("subagent prompt runtime", () => {
	...
	it("injects explicit path memory packet without parent context inheritance", () => {
		const rewritten = rewriteSubagentPrompt(BASE_PROMPT, {
			inheritProjectContext: false,
			inheritSkills: false,
	...
	it("injects explicit path execution contract into the child prompt", () => {
		const rewritten = rewriteSubagentPrompt(BASE_PROMPT, {
			inheritProjectContext: false,
			inheritSkills: false,
			pathContractPacket: "# Path Execution Contract\n\nowned runtime only",
		});

		assert.ok(rewritten.includes("explicit path execution contract"));
		assert.ok(rewritten.includes("# Path Execution Contract"));
		assert.ok(rewritten.includes("owned runtime only"));
		assert.ok(!rewritten.includes("# Project Context"));
	});

	it("blocks mutating tool calls outside the path execution contract", () => {
		let toolCallHandler: ((event: unknown) => unknown) | undefined;
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-path-contract-runtime-"));
		const contractPath = path.join(dir, "path-contract.md");
		...
		process.env[PATH_CONTRACT_ENV] = contractPath;
		...
		assert.equal(toolCallHandler?.({
			toolName: "write",
			input: { path: "packages/coding-agent/src/runtime/state.ts" },
		}), undefined);
		assert.deepEqual(toolCallHandler?.({
			toolName: "write",
			input: { path: "packages/coding-agent/src/session/session.ts" },
		}), {
			block: true,
			reason: "Path contract runtime-state-model blocks writes outside allowed paths: packages/coding-agent/src/session/session.ts",
		});
		delete process.env[PATH_CONTRACT_ENV];
		fs.rmSync(dir, { recursive: true, force: true });
	});

	it("blocks reads and activity overages through the runtime path contract hook", () => {
		let toolCallHandler: ((event: unknown) => unknown) | undefined;
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-path-contract-runtime-"));
		const contractPath = path.join(dir, "path-contract.md");
		fs.writeFileSync(contractPath, [
			...
			JSON.stringify({
				...
				activity_budget: { max_tool_uses: 2 },
			}),
			"```",
		].join("\n"));
		process.env[PATH_CONTRACT_ENV] = contractPath;
		...
		assert.deepEqual(toolCallHandler?.({
			toolName: "read",
			input: { path: "packages/coding-agent/src/config/config.ts" },
		}), {
			block: true,
			reason: "Path contract runtime-state-model blocks reads outside allowed paths: packages/coding-agent/src/config/config.ts",
		});
		assert.deepEqual(toolCallHandler?.({
			toolName: "write",
			input: { path: "packages/coding-agent/src/runtime/state.ts" },
			...
			block: true,
			reason: "Path contract runtime-state-model exceeded activity budget max_tool_uses=2",
		});
		delete process.env[PATH_CONTRACT_ENV];
		fs.rmSync(dir, { recursive: true, force: true });
	});

	it("fails closed when a path execution contract file is invalid", () => {
		let toolCallHandler: ((event: unknown) => unknown) | undefined;
		const dir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-path-contract-runtime-"));
		const contractPath = path.join(dir, "path-contract.md");
		fs.writeFileSync(contractPath, [
			...
			"",
			"```json",
			JSON.stringify({ contract_id: "runtime-state-model" }),
			"```",
		].join("\n"));
		process.env[PATH_CONTRACT_ENV] = contractPath;
		...
		assert.deepEqual(toolCallHandler?.({
			...
			block: true,
			reason: "Path contract failed closed: Path execution contract is missing enforceable boundaries.",
		});
		delete process.env[PATH_CONTRACT_ENV];
		fs.rmSync(dir, { recursive: true, force: true });
	});
	...
	it("strips the subagent orchestration skill even when inherited skills remain", () => {
		...
		assert.ok(!rewritten.includes("<name>amaze-subagents</name>"));
		assert.ok(!rewritten.includes("delegate to subagents"));
	});

	it("strips explicit amaze-subagents skill injection from child prompts", () => {
		const prompt = "Before\n\n<skill name=\"amaze-subagents\">\nDo not keep this.\n</skill>\n\n<skill name=\"safe-bash\">\nKeep this.\n</skill>\nAfter";
