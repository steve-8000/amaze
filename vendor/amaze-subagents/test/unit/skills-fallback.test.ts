...
describe("skills filesystem fallback", () => {
	beforeEach(() => {
		...
		oldAgentDir = process.env.AMAZE_CODING_AGENT_DIR;
	...
	it("does not expose amaze-subagents as a child-injectable skill", () => {
		makeProjectSkill(tempDir, "amaze-subagents", "Parent orchestration only.");
		makeProjectSkill(tempDir, "safe-bash", "Use safe bash.");

		const available = discoverAvailableSkills(tempDir).map((skill) => skill.name);
		assert.equal(available.includes("amaze-subagents"), false);
		assert.equal(available.includes("safe-bash"), true);

		const { resolved, missing } = resolveSkills(["amaze-subagents", "safe-bash"], tempDir);
		assert.deepEqual(missing, ["amaze-subagents"]);
