...
[![ci](https://github.com/code-yeongyu/amaze-ast-grep/actions/workflows/ci.yml/badge.svg)](https://github.com/code-yeongyu/amaze-ast-grep/actions/workflows/ci.yml) [![license: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

AST-aware code search and rewrite for the [amaze coding agent](https://github.com/badlogic/amaze/tree/main/packages/coding-agent). Faithful port of the ast-grep tool stack from [oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent).
...
The same author re-licensed the ported source under MIT for distribution in the amaze ecosystem. omo itself remains under SUL-1.0; this package's MIT scope covers only the code that ships in this repository. See [LICENSE](LICENSE) and [NOTICE](NOTICE).
...
The package targets the [`pi`](https://github.com/badlogic/amaze/tree/main/packages/coding-agent) coding agent. Pi loads extensions from `~/.pi/agent/extensions/`, project `.pi/extensions/`, or via the `--extension` / `-e` CLI flag.

Pick whichever route fits:

```bash
# 1. From npm (once published)
pi install npm:@code-yeongyu/amaze-ast-grep

# 2. From git (once the repository is pushed)
pi install git:github.com/code-yeongyu/amaze-ast-grep

# 3. Manual placement (always works)
git clone https://github.com/code-yeongyu/amaze-ast-grep ~/.pi/agent/extensions/amaze-ast-grep
cd ~/.pi/agent/extensions/amaze-ast-grep && npm install

# 4. Dev / one-shot test
pi -e /path/to/amaze-ast-grep/src/index.ts
```
...
`amaze-ast-grep` resolves the `sg` binary in this order:

1. **Cached download** — `$XDG_CACHE_HOME/amaze-ast-grep/bin/sg` on Unix, `%LOCALAPPDATA%\amaze-ast-grep\bin\sg.exe` on Windows. Validated by existence and `>10000` byte size.
...
```bash
git clone https://github.com/code-yeongyu/amaze-ast-grep
cd amaze-ast-grep
...
- [amaze](https://github.com/codeamaze) — the fork/runtime these extensions are extracted from.
- [Ultraworkers Discord](https://discord.gg/PUwSMR9XNk) — community link from the amaze README.
- [Dori](https://sisyphuslabs.ai) — the product powered by amaze under the hood.
