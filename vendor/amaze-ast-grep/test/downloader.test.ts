...
describe("downloader helpers", () => {
	beforeEach(() => {
		process.env["XDG_CACHE_HOME"] = "/tmp/amaze-ast-grep-cache-test";
	});
	...
	it("#given cache environment override #when building cache dir #then returns pi ast grep cache path", () => {
		...
		// then
		expect(cacheDirectory).toContain("amaze-ast-grep");
		if (process.platform !== "win32") {
			expect(cacheDirectory).toBe(join("/tmp/amaze-ast-grep-cache-test", "amaze-ast-grep", "bin"));
		}
	});
