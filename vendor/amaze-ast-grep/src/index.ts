...
/**
 * amaze-ast-grep — AST-aware code search and replace for the amaze coding agent.
 *
...
export default function (pi: ExtensionAPI): void {
	...
	pi.registerCommand("ast-grep", {
		...
		handler: async (args, ctx) => {
			...
			if (wantsInstall) {
				ctx.ui.setStatus("amaze-ast-grep", "Downloading sg binary...");
				const path = await ensureAstGrepBinary();
				ctx.ui.setStatus("amaze-ast-grep", undefined);
				if (path) {
