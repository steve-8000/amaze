...
//!   `$XDG_STATE_HOME/amaze/logs/` on Linux / macOS when the user has migrated
...
const DEFAULT_CONFIG_DIR: &str = ".amaze";

/// App name used as the XDG-root subdirectory (`$XDG_STATE_HOME/amaze/`),
/// matching `APP_NAME` in `packages/utils/src/dirs.ts`.
#[cfg(any(target_os = "linux", target_os = "macos"))]
const APP_NAME: &str = "amaze";
...
fn resolve_logs_dir(
	...
	// XDG takes precedence so users who migrated to `$XDG_STATE_HOME/amaze/logs/`
...
/// `$XDG_STATE_HOME` set, `$XDG_STATE_HOME/amaze` exists on disk, and
...
/// `<xdg_state_home>/amaze` actually lives on disk.
#[cfg(any(target_os = "linux", target_os = "macos"))]
fn xdg_state_logs(
...
mod tests {
	...
	fn resolve_logs_dir_defaults_under_dot_omp() {
		...
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.amaze/logs"));
	}
	...
	fn resolve_logs_dir_honors_relative_pi_config_dir() {
		let dir = resolve_logs_dir(
			...
			Some(OsStr::new(".amaze-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.amaze-dev/logs"));
	}
	...
	fn resolve_logs_dir_normalizes_parent_components_like_path_join() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/pi-natives-test-home"),
			Some(OsStr::new("nested/../.amaze-dev")),
			None,
		);
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.amaze-dev/logs"));
	}
	...
	fn xdg_state_logs_ignores_empty_agent_dir_override() {
		...
		let dir = xdg_state_logs(
			...
			Some(OsStr::new("")),
			Path::new("/tmp/pi-natives-test-home/.amaze/agent"),
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/amaze/logs")));
	}
	...
	fn resolve_logs_dir_prefers_xdg_when_provided() {
		let dir = resolve_logs_dir(
			Path::new("/tmp/pi-natives-test-home"),
			None,
			Some(PathBuf::from("/xdg/state/amaze/logs")),
		);
		assert_eq!(dir, PathBuf::from("/xdg/state/amaze/logs"));
	}
	...
	fn xdg_state_logs_skipped_when_xdg_state_home_unset_or_empty() {
		let default_agent = Path::new("/tmp/pi-natives-test-home/.amaze/agent");
	...
	fn xdg_state_logs_honored_when_agent_override_matches_default() {
		let default_agent = std::path::absolute(Path::new("./.amaze/agent")).unwrap();
		let dir = xdg_state_logs(
			Some(OsStr::new("/xdg/state")),
			Some(OsStr::new("./.amaze/agent")),
			&default_agent,
			|_p| true,
		);
		assert_eq!(dir, Some(PathBuf::from("/xdg/state/amaze/logs")));
	...
	fn default_agent_dir_uses_dot_omp_by_default() {
		...
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.amaze/agent"));
	}
	#[cfg(any(target_os = "linux", target_os = "macos"))]
	#[test]
	fn default_agent_dir_respects_pi_config_dir() {
		let dir =
			default_agent_dir(Path::new("/tmp/pi-natives-test-home"), Some(OsStr::new(".amaze-dev")));
		assert_eq!(dir, PathBuf::from("/tmp/pi-natives-test-home/.amaze-dev/agent"));
	}

	#[test]
	fn build_crash_log_path_tags_kind_and_pid() {
		let dir = Path::new("/tmp/pi-natives-test-home/.amaze/logs");
		let panic_log = build_crash_log_path(dir, CrashKind::Panic, 4242, 1_700_000_000_000);
		assert_eq!(
			panic_log,
			PathBuf::from("/tmp/pi-natives-test-home/.amaze/logs/native-panic-4242-1700000000000.log")
		);
		let alloc_log = build_crash_log_path(dir, CrashKind::Alloc, 99, 1);
		assert_eq!(
			alloc_log,
			PathBuf::from("/tmp/pi-natives-test-home/.amaze/logs/native-alloc-99-1.log")
		);
	}
}
