# LSP configuration in Amaze

This guide explains how to configure language servers for the Amaze coding agent.
...
When no LSP config file is present, Amaze auto-detects servers by intersecting two conditions:
...
Amaze merges LSP config from multiple files, lowest to highest priority:

| Priority    | Location                                                                                                                    |
| ----------- | --------------------------------------------------------------------------------------------------------------------------- |
| 5 (lowest)  | `~/lsp.json`, `~/.lsp.json`, `~/lsp.yaml`, `~/.lsp.yaml`, `~/lsp.yml`, `~/.lsp.yml`                                         |
| 4           | Plugin LSP configs (marketplace / `--plugin-dir` roots)                                                                     |
| 3           | User config dirs: `~/.amaze/agent/lsp.*`, `~/.claude/lsp.*`, `~/.codex/lsp.*`, `~/.gemini/lsp.*`                              |
| 2           | Project config dirs: `<project>/.amaze/lsp.*`, `<project>/.claude/lsp.*`, `<project>/.codex/lsp.*`, `<project>/.gemini/lsp.*` |
...
- User-wide preferences → `~/.amaze/agent/lsp.json`
- Project-specific overrides → `<project>/.amaze/lsp.json`

> **Note:** Auto-detection is skipped only when at least one config file contributes server overrides. A config file that only sets `idleTimeoutMs` still lets Amaze auto-detect built-in servers. When server overrides exist, Amaze merges them with defaults and then loads servers that have matching `rootMarkers`, an available binary, and are not explicitly `disabled`.
...
The `capabilities` object enables optional server-specific features that Amaze supports on a per-server basis:
...
Place the override in `<project>/.amaze/lsp.json`:
...
The user-level config in `~/.amaze/agent/lsp.json` is unaffected; pylsp is only suppressed in this project.
