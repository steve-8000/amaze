...
This document explains how token/tool streaming is normalized in `@steve-z8k/pi-ai`, then propagated through `@steve-z8k/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@steve-z8k/pi-ai`
