...
This document explains how token/tool streaming is normalized in `@amaze/pi-ai`, then propagated through `@amaze/pi-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@amaze/pi-ai`
