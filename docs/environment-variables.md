...
3. Agent `.env` (`~/.amaze/agent/.env`, respecting `PI_CONFIG_DIR` / `PI_CODING_AGENT_DIR`) for keys not already set
4. Config-root `.env` (`~/.amaze/.env`, respecting `PI_CONFIG_DIR`) for keys not already set
...
Generic settings bridge: any schema-backed setting can also come from `.env` via `AMAZE_SETTING_<PATH>` (preferred) or `OMP_SETTING_<PATH>` (legacy alias). The `<PATH>` part is the setting path uppercased with dots/non-alphanumerics mapped to `_` and camelCase split with `_` (for example `task.maxConcurrency` → `AMAZE_SETTING_TASK_MAX_CONCURRENCY`). These values are parsed with the same type rules as `amaze config set`, loaded after global/project `config.yml`, and still yield to explicit `--config` overlays plus runtime overrides.
...
| Variable                | Used for                                                                                     | Required when                                                                                                             | Notes / precedence                                                                                                                                                                         |
| ----------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `OMP_AUTH_BROKER_URL`   | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`); selects broker mode | Resolving credentials through a broker; also required by `amaze auth-gateway serve` (the gateway is itself a broker client) | Wins over `auth.broker.url` in `config.yml`. When set with no resolvable token, `resolveAuthBrokerConfig()` hard-errors instead of falling back to local SQLite.                           |
...
| `OMP_AUTH_BROKER_SNAPSHOT_CACHE` | Path to the encrypted local broker snapshot cache | Optional in broker mode | Defaults to `~/.amaze/cache/auth-broker-snapshot.enc` (or XDG cache equivalent). Useful for tests, ephemeral hosts, or relocating the `0600` cache file. |

The gateway has no dedicated env vars — it inherits `OMP_AUTH_BROKER_*`. Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `amaze auth-gateway token`.
...
| `AMAZE_SETTING_<PATH>`       | Generic `.env` bridge for any schema-backed setting (example: `AMAZE_SETTING_TASK_MAX_CONCURRENCY=2` for `task.maxConcurrency`). Parsed with the same type rules as `amaze config set`; loaded after global/project `config.yml`, but before explicit `--config` overlays and runtime overrides.                                                                                                                                                                                                                                                                                                        |
| `OMP_SETTING_<PATH>`         | Legacy alias for `AMAZE_SETTING_<PATH>` with identical parsing and precedence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `PI_SUBPROCESS_CMD`          | Overrides subagent spawn command (`amaze` / `amaze.cmd` resolution bypass)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `OLLAMA_CONTEXT_LENGTH`      | Positive integer context-window override for implicit Ollama discovery; affects Amaze context budgeting only and does not change Ollama's runtime `num_ctx`                                                                                                                                                                                                                                                                                                                                                                                           |
...
| Variable              | Default / behavior                                                            |
| --------------------- | ----------------------------------------------------------------------------- |
| `PI_CONFIG_DIR`       | Config root dirname under home (default `.amaze`)                               |
| `PI_CODING_AGENT_DIR` | Full override for agent directory (default `~/<PI_CONFIG_DIR or .amaze>/agent`) |
