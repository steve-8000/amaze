...
Most runtime lookups use `$env` from `@amaze/utils` (`packages/utils/src/env.ts`).

`$env` loading order:

1. Existing process environment (`Bun.env`)
2. Project `.env` (`$PWD/.env`) for keys not already set
3. Agent `.env` (`~/.amaze/agent/.env`, respecting `AMAZE_CONFIG_DIR` / `AMAZE_CODING_AGENT_DIR`) for keys not already set
4. Config-root `.env` (`~/.amaze/.env`, respecting `AMAZE_CONFIG_DIR`) for keys not already set
5. Home `.env` (`~/.env`) for keys not already set

Additional rule inside each `.env` file: `AMAZE_*` keys are mirrored to `AMAZE_*` keys in that parsed file.
...
| `XAI_API_KEY`                   | xAI auth                                         | Using xAI models or X Search tools without stored xAI Grok OAuth credentials | Fallback after stored `xai-oauth` / xAI Grok OAuth credentials where supported                                |
...
| Variable                | Used for                                                                                          | Required when                                                                                                          | Notes / precedence                                                                                                                                                                                  |
| ----------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AMAZE_AUTH_BROKER_URL`   | Base URL of the remote auth-broker (e.g. `https://broker.tailnet:8765`); selects broker mode      | Resolving credentials through a broker; also required by `amaze auth-gateway serve` (the gateway is itself a broker client) | Wins over `auth.broker.url` in `config.yml`. When set with no resolvable token, `resolveAuthBrokerConfig()` hard-errors instead of falling back to local SQLite.                                    |
| `AMAZE_AUTH_BROKER_TOKEN` | Bearer token sent on every broker endpoint except `/v1/healthz`                                   | `AMAZE_AUTH_BROKER_URL` is set and no token is available from `auth.broker.token` or `<config-dir>/auth-broker.token`     | Resolution: this env → `auth.broker.token` (`$ENV_NAME` indirection supported) → `<config-dir>/auth-broker.token` (mode `0600`). `<config-dir>` is `~/.amaze/` (respecting `AMAZE_CONFIG_DIR`).         |

The gateway has no dedicated env vars — it inherits `AMAZE_AUTH_BROKER_*`. Its own inbound bearer token lives at `<config-dir>/auth-gateway.token` and is managed via `amaze auth-gateway token`.
...
| Variable                   | Default / behavior                                              |
| -------------------------- | --------------------------------------------------------------- |
| `AMAZE_AI_GEMINI_CLI_VERSION` | Overrides Gemini CLI user-agent version tag (`0.35.3` if unset) |

### OpenAI Codex responses (feature/debug controls)

| Variable                             | Behavior                                             |
| ------------------------------------ | ---------------------------------------------------- |
| `AMAZE_CODEX_DEBUG`                     | `1`/`true` enables Codex provider debug logging      |
| `AMAZE_CODEX_WEBSOCKET`                 | `1`/`true` enables websocket transport preference    |
| `AMAZE_CODEX_WEBSOCKET_V2`              | `1`/`true` enables websocket v2 path                 |
| `AMAZE_CODEX_WEBSOCKET_IDLE_TIMEOUT_MS` | Positive integer override (default 300000)           |
| `AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET`    | Non-negative integer override (default 5)            |
| `AMAZE_CODEX_WEBSOCKET_RETRY_DELAY_MS`  | Positive integer base backoff override (default 500) |
| `AMAZE_OPENAI_STREAM_IDLE_TIMEOUT_MS`   | Positive integer OpenAI stream idle timeout override |
...
| Variable             | Behavior                                                                                                          |
| -------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `AMAZE_CACHE_RETENTION` | If `long`, enables long retention where supported (`anthropic`, `openai-responses`, Bedrock retention resolution) |
...
| `AMAZE_CODEX_WEB_SEARCH_MODEL`                         | Codex search provider model override                          |
...
SearXNG also reads the equivalent `searxng.endpoint`, `searxng.token`, `searxng.basicUsername`, and `searxng.basicPassword` settings from `~/.amaze/agent/config.yml`; environment variables are fallbacks.

### xAI X Search tools

The built-in `x_search` and `x_search_deep` tools can use stored `xai-oauth` / xAI Grok OAuth credentials from the agent credential store. These OAuth credentials are managed by login/storage flows, not by environment variables. `XAI_API_KEY` remains the API-key fallback for the `xai` provider when no usable OAuth credential is available.

| Variable                      | Default / behavior                                                                 |
| ----------------------------- | ---------------------------------------------------------------------------------- |
| `AMAZE_X_SEARCH_MODEL`           | xAI model used for X Search requests; defaults to `grok-4.3`                       |
| `AMAZE_X_SEARCH_BASE_URL`        | xAI API base URL for X Search; defaults to `https://api.x.ai/v1`                   |
| `AMAZE_X_SEARCH_TIMEOUT_MS`      | Request timeout in milliseconds; defaults to `180000`                              |
...
| Variable            | Behavior                                                                        |
| ------------------- | ------------------------------------------------------------------------------- |
| `AMAZE_AUTH_NO_BORROW` | If set, disables macOS native-app token borrowing path in Perplexity login flow |
...
| Variable                  | Default / behavior                                                                                                  |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `AMAZE_PY`                   | Eval backend override: `0`/`bash`=JavaScript only, `1`/`py`=Python only, `mix`/`both`=both; invalid values ignored |
| `AMAZE_PYTHON_SKIP_CHECK`    | If `1`, skips Python interpreter availability checks (subprocess runner still starts on demand)                     |
| `AMAZE_PYTHON_INTEGRATION`   | If `1`, opts gated integration tests in (e.g. `python-runner.integration.test.ts`) into running against real Python |
| `AMAZE_PYTHON_IPC_TRACE`     | If `1`, logs NDJSON frames exchanged with the Python runner subprocess                                              |
...
- Python env filtering denies common API keys and allows safe base vars + `LC_`, `XDG_`, `AMAZE_` prefixes.

---

## 5) Built-in CUA computer-use tool

The built-in `cua` tool exposes CUA computer-use actions for sandbox lifecycle plus screenshot, click, type, key, and scroll control. Modes are configured as `local` (default sandbox), `localhost` (controls the host unsandboxed), or `cloud`. Prefer the local or cloud sandbox when possible; use `localhost` only when direct host control is intended. Project policy is read from `.amaze/cua.jsonc`; global policy is read from `~/.amaze/cua.json`. The Python CUA package must be available separately (`pip install cua`).
...
## 6) Agent/runtime behavior toggles

| Variable                     | Default / behavior                                                                                 |
| ---------------------------- | -------------------------------------------------------------------------------------------------- |
| `AMAZE_SMOL_MODEL`              | Ephemeral model-role override for `smol` (CLI `--smol` takes precedence)                           |
| `AMAZE_SLOW_MODEL`              | Ephemeral model-role override for `slow` (CLI `--slow` takes precedence)                           |
| `AMAZE_PLAN_MODEL`              | Ephemeral model-role override for `plan` (CLI `--plan` takes precedence)                           |
| `AMAZE_NO_TITLE`                | If set (any non-empty value), disables auto session title generation on first user message         |
| `NULL_PROMPT`                | If `true`, system prompt builder returns empty string                                              |
| `AMAZE_BLOCKED_AGENT`           | Blocks a specific subagent type in task tool                                                       |
| `AMAZE_SUBPROCESS_CMD`          | Overrides subagent spawn command (`amaze` / `amaze.cmd` resolution bypass)                             |
| `AMAZE_TASK_MAX_OUTPUT_BYTES`   | Max captured output bytes per subagent (default `500000`)                                          |
| `AMAZE_TASK_MAX_OUTPUT_LINES`   | Max captured output lines per subagent (default `5000`)                                            |
| `AMAZE_TIMING`                  | If set (any non-empty value), prints a hierarchical timing-span tree to **stderr** via `logger.printTimings()`. In interactive mode the tree prints once the agent is ready (before the TUI starts); in print mode it prints after the whole prompt batch completes. Print-mode prompts are wrapped in `print:prompt:initial` / `print:prompt:next` spans so each user message shows up as its own row. `AMAZE_TIMING=x` exits the process with code 0 right after printing in interactive mode (use to measure cold startup only). `AMAZE_TIMING=full` lists every module-load entry instead of just the top N. |
| `AMAZE_PACKAGE_DIR`             | Overrides package asset base dir resolution (docs/examples/changelog path lookup)                  |
| `AMAZE_DISABLE_LSPMUX`          | If `1`, disables lspmux detection/integration and forces direct LSP server spawning                |
| `AMAZE_RPC_EMIT_TITLE`          | Boolean-like flag enabling title events in RPC mode                                                |
...
| `AMAZE_EDIT_VARIANT`            | Forces edit tool variant when valid (`patch`, `replace`, `hashline`, `atom`, `vim`, `apply_patch`) |
| `AMAZE_FORCE_IMAGE_PROTOCOL`    | Forces supported image protocol (`kitty`, `iterm2`/`iterm`, `sixel`, `none`) where used            |
| `AMAZE_ALLOW_SIXEL_PASSTHROUGH` | Allows SIXEL passthrough when `AMAZE_FORCE_IMAGE_PROTOCOL=sixel`                                      |
| `AMAZE_NO_PTY`                  | If `1`, disables interactive PTY path for bash tool                                                |

`AMAZE_NO_PTY` is also set internally when CLI `--no-pty` is used.

---

## 7) Storage and config root paths

These are consumed via `@amaze/utils/dirs` and affect where coding-agent stores data.

| Variable              | Default / behavior                                                            |
| --------------------- | ----------------------------------------------------------------------------- |
| `AMAZE_CONFIG_DIR`       | Config root dirname under home (default `.amaze`)                               |
| `AMAZE_CODING_AGENT_DIR` | Full override for agent directory (default `~/<AMAZE_CONFIG_DIR or .amaze>/agent`) |
| `PWD`                 | Used when matching canonical current working directory in path helpers        |

---

## 8) Shell/tool execution environment

(From `packages/utils/src/procmgr.ts` and coding-agent bash tool integration.)

| Variable                   | Behavior                                                                       |
| -------------------------- | ------------------------------------------------------------------------------ |
| `AMAZE_BASH_NO_CI`            | Suppresses automatic `CI=true` injection into spawned shell env                |
| `CLAUDE_BASH_NO_CI`        | Legacy alias fallback for `AMAZE_BASH_NO_CI`                                      |
| `AMAZE_BASH_NO_LOGIN`         | Disables login-shell mode; shell args become `['-c']` instead of `['-l','-c']` |
| `CLAUDE_BASH_NO_LOGIN`     | Legacy alias fallback for `AMAZE_BASH_NO_LOGIN`                                   |
| `AMAZE_SHELL_PREFIX`          | Optional command prefix wrapper                                                |
| `CLAUDE_CODE_SHELL_PREFIX` | Legacy alias fallback for `AMAZE_SHELL_PREFIX`                                    |
| `VISUAL`                   | Preferred external editor command                                              |
| `EDITOR`                   | Fallback external editor command                                               |

Current implementation: `AMAZE_BASH_NO_LOGIN`/`CLAUDE_BASH_NO_LOGIN` are active; when either is set, `getShellArgs()` returns `['-c']`.

---

## 9) UI/theme/session detection (auto-detected env)
...
| Variable                  | Behavior                                                                              |
| ------------------------- | ------------------------------------------------------------------------------------- |
| `AMAZE_NOTIFICATIONS`        | `off` / `0` / `false` suppress desktop notifications                                  |
| `AMAZE_TUI_WRITE_LOG`        | If set, logs TUI writes to file                                                       |
| `AMAZE_HARDWARE_CURSOR`      | If `1`, enables hardware cursor mode                                                  |
| `AMAZE_CLEAR_ON_SHRINK`      | If `1`, clears empty rows when content shrinks                                        |
| `AMAZE_DEBUG_REDRAW`         | If `1`, enables redraw debug logging                                                  |
| `AMAZE_TUI_DEBUG`            | If `1`, enables deep TUI debug dump path                                              |
| `AMAZE_FORCE_IMAGE_PROTOCOL` | Forces terminal image protocol detection (`kitty`, `iterm2`/`iterm`, `sixel`, `none`) |
...
| Variable                  | Behavior                                                            |
| ------------------------- | ------------------------------------------------------------------- |
| `AMAZE_COMMIT_TEST_FALLBACK` | If `true` (case-insensitive), force commit fallback generation path |
| `AMAZE_COMMIT_NO_FALLBACK`   | If `true`, disables fallback when agent returns no proposal         |
| `AMAZE_COMMIT_MAP_REDUCE`    | If `false`, disables map-reduce commit analysis path                |
...
- Search/provider auth vars (`EXA_API_KEY`, `BRAVE_API_KEY`, `PERPLEXITY_API_KEY`, Anthropic search keys, `XAI_API_KEY`)
- CUA cloud credentials (`CUA_API_KEY`)
