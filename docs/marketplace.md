...
A **marketplace** is a Git repository (or local directory) containing a catalog file at `.amaze-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). The catalog lists available plugins with their sources, descriptions, and metadata.

A **plugin** is a directory containing Claude/Amaze plugin content such as skills, commands, agents, hooks, tools, MCP servers, or LSP servers. Extension modules (`package.json` `amaze.extensions` entry points) are not loaded from marketplace installs — they only load for npm-installed or `amaze plugin link`ed plugins. Plugins are identified by `name@marketplace` (e.g. `code-review@claude-plugins-official`).

**Scopes**: marketplace plugins can be installed at two scopes:

- **user** (default) -- available in all projects, stored in `~/.amaze/plugins/installed_plugins.json`
- **project** -- available only in the active project, stored in the nearest project `.amaze/plugins/installed_plugins.json`
...
```
amaze plugin marketplace add <source>
amaze plugin marketplace remove <name>
amaze plugin marketplace update [name]
amaze plugin marketplace list
amaze plugin discover [marketplace]
amaze plugin install [--force] [--scope user|project] name@marketplace
amaze plugin uninstall [--scope user|project] name@marketplace
amaze plugin upgrade [--scope user|project] [name@marketplace]
amaze plugin enable [--scope user|project] name@marketplace
amaze plugin disable [--scope user|project] name@marketplace
```
...
Git and local sources must contain a catalog at `.amaze-plugin/marketplace.json` (preferred) or `.claude-plugin/marketplace.json` (Claude Code-compatible fallback). Direct catalog URLs cache only the JSON catalog; plugins in URL-sourced catalogs cannot use relative string sources like `"./plugins/foo"`.

## Catalog format (marketplace.json)

A marketplace catalog lives at `.amaze-plugin/marketplace.json` in the repository root. When amaze is the only intended consumer, prefer this path. To remain Claude Code-compatible (amaze loads the same shape from either path), publish at `.claude-plugin/marketplace.json` instead — amaze uses it as a fallback when `.amaze-plugin/marketplace.json` is absent. A repository may ship both: amaze reads the `.amaze-plugin/` copy, Claude Code reads the `.claude-plugin/` copy. Same catalog format either way:
