...
Extensions are the primary way to add capabilities to `amaze-agent`. A single extension module can register tools the LLM can call, slash commands users can invoke, and event handlers that run throughout the session lifecycle — all from one TypeScript file.
...
amaze loads extension modules from these sources:

1. Native `.amaze` locations discovered through the capability system:
   - `<cwd>/.amaze/extensions/`
   - `~/.amaze/agent/extensions/`
   - legacy extension paths listed in `.amaze/settings.json#extensions` or `~/.amaze/agent/settings.json#extensions`
2. Installed plugins under `~/.amaze/plugins/node_modules` (`amaze plugin install` npm/git specs, or `amaze plugin link`) via their `amaze.extensions`/`pi.extensions` manifests. Marketplace cache installs do not feed extension modules — they surface skills/commands/hooks/tools/MCP only.
3. Explicit configured paths passed by the CLI (`amaze --extension ./my-ext.ts`, also `-e`; `--hook` is treated as an alias) and by the `extensions:` setting in config.
...
When scanning an `extensions/` directory, amaze also loads direct `*.ts`/`*.js` files and one-level subdirectories that have `index.ts`, `index.js`, or a manifest.

Extension packages can also bundle sibling capability directories. When a package is loaded through `extensions:` or `--extension`/`-e`, the `amaze-plugins` provider discovers its `skills/`, `hooks/pre|post/`, `tools/`, `commands/`, `rules/`, `prompts/`, and `.mcp.json`.
...
| Legacy hook module already exists | **Hook** (`HookAPI` from `@steve-z8k/pi-coding-agent/extensibility/hooks`) |
...
amaze writes structured logs to a rotating file under `~/.amaze/logs/` (debug level is always on; nothing is written to the console, which would corrupt the TUI). Tail today's log to see extension load diagnostics:

```
tail -f ~/.amaze/logs/amaze.$(date +%F).log
```
