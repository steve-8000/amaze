...
The SDK is the in-process integration surface for `@amaze/coding-agent`.
...
```bash
bun add @amaze/coding-agent
```

## Entry points

`@amaze/coding-agent` exports the SDK APIs from the package root (and also via `@amaze/coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@amaze/coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.amaze/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@amaze/coding-agent";
...
```ts
import { SessionManager } from "@amaze/coding-agent";
