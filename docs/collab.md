...
`/collab` shares your running session with other amaze instances in real time. Guests render the **same session natively in their own TUI** — streaming assistant text, tool-call cards, footer state (cwd, model, context %, cost), ctrl+o expansion, `/dump` — no terminal mirroring. Guests can prompt and interrupt the agent; the host machine runs the agent and all tools.
...
Collab session started!
 • Join from another terminal: amaze join "mgAYTZwEnpRQtca0CTgn-Q.gdJUbTovD94ofDaa8YvhY0-ty16w4fn8PgB6PLnoA30"
 • or any web browser: my.amaze/#mgAYTZwEnpRQtca0CTgn-Q.gdJUbTovD94ofDaa8YvhY0-ty16w4fn8PgB6PLnoA30
```

The browser line is click-to-join (an OSC 8 hyperlink to the full `https://` deep link): the relay serves the web guest client at `/`, and the room id + key ride in the URL fragment. From another amaze (any directory, any machine), either form works:
...
```
/join my.amaze/#mgAYTZwEnpRQtca0CTgn-Q.gdJU…
```
...
Accepted by `/join <link>` and `amaze join "<link>"`:

```
<roomId>.<key>                                                    → default relay (wss://my.amaze)
...
|---|---|---|
| `collab.relayUrl` | `wss://my.amaze` | Relay used by `/collab` when no relay is passed inline |
...
| `share.serverUrl` | `https://my.amaze/s` | Share viewer/upload base used by `/share` (links are `<base>/<id>#<key>`) |
