...
1. user level: `<active agent dir>/WATCHDOG.md` (`~/.amaze/agent/WATCHDOG.md` by default; relocated by `PI_CODING_AGENT_DIR`)
...
Candidates in hidden owner directories are ignored unless the file is inside an `.amaze` directory. This keeps unrelated dot-directory conventions from being picked up accidentally while still allowing `.amaze/WATCHDOG.md`.
...
- **Usage attribution.** `amaze stats` scans each session folder recursively, so advisor assistant turns (with their usage/cost) are attributed to the same project/session like any other subagent. Advisor "session update" prompts are persisted as `synthetic`, agent-attributed user messages so they never inflate user-message metrics.
