...
- `explore`, `plan`, `reviewer`, `oracle`, `researcher`, and `visual_qa` from prompt files
...
`discovery.ts` header comments still mention `.pi` and do not mention `.codex`/`.gemini`. Actual runtime order is driven by `src/config.ts` and currently uses `.amaze`, `.claude`, `.codex`, `.gemini`.
...
- Higher-priority source family overrides lower (`.amaze` before `.claude`, etc.).
...
`AMAZE_BLOCKED_AGENT` is read at tool construction. If request matches, execution is rejected with recursion-prevention message.
