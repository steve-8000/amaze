...
- Key collaborators:
  - `crates/natives/src/ast.rs` — native rewrite planning and file mutation
...
2. The wrapper reads `AMAZE_MAX_AST_FILES` via `$envpos(..., 1000)` and uses that as the native `maxFiles` cap for both preview and apply.
...
6. Native `ast_edit` in `crates/natives/src/ast.rs`:
...
## Limits & Caps
- File cap exposed by the wrapper: `AMAZE_MAX_AST_FILES`, default `1000`, in `packages/coding-agent/src/tools/ast-edit.ts`.
- Native `maxFiles` and `maxReplacements` are both clamped to at least `1` when provided in `crates/natives/src/ast.rs`.
