...
- `createReportBundle()` writes `amaze-report-<timestamp>.tar.gz` under the reports dir and returns the filesystem path to the UI handler.
...
- Subprocesses / native bindings
  ...
  - `getWorkProfile(30)` comes from `@steve-z8k/pi-natives`.
  - CPU profiling uses `node:inspector/promises`; heap snapshots use `Bun.generateHeapSnapshot("v8")`; raw/log viewers sanitize text via `sanitizeText()` from `@steve-z8k/pi-utils`.
...
- Raw SSE buffer caps in `packages/coding-agent/src/debug/raw-sse-buffer.ts`:
  ...
  - `MAX_RAW_SSE_EVENT_CHARS = 64_000` per event, with `: amaze-debug-truncated ...` marker appended on trim
