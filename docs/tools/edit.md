...
- Delete range: `- A..B`
- Replace range: `= A..B`
- Payload line: `~TEXT` by default; separator is `HL_EDIT_SEP` and can be overridden once at process start by `AMAZE_HL_SEP` (`packages/coding-agent/src/hashline/hash.ts`)
...
- Subprocesses / native bindings
  ...
  - `invalidateFsScanAfterWrite()` calls native `invalidateFsScanCache()` from `@amaze/natives`.
- Session state
