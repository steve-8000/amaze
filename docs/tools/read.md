...
- Key collaborators:
  ...
  - `packages/coding-agent/src/internal-urls/router.ts` — resolve `agent://`, `artifact://`, `local://`, `mcp://`, `memory://`, `amaze://`, `rule://`, `skill://`.
...
- Registered protocols are outside this file, but the router in `packages/coding-agent/src/internal-urls/router.ts` is built for `agent://`, `artifact://`, `issue://`, `local://`, `mcp://`, `memory://`, `amaze://`, `pr://`, `rule://`, and `skill://`.
