...
┌───────────────────────────────────────┐
│ 1) ~/.amaze/agent, ~/.claude, ...       │
│ 2) <cwd>/.amaze, <cwd>/.claude, ...     │
└───────────────────────────────────────┘
                    │
                    ▼
        capability providers enumerate items
 (native provider scans project .amaze before user .amaze;
...
`CONFIG_DIR_NAME` is `.amaze` (`packages/utils/src/dirs.ts`).

## Profiles

A named profile (`amaze --profile <name>`, the `--alias` shortcut, or `OMP_PROFILE` / `PI_PROFILE`) relocates the Amaze user base. When a profile is active, every Amaze-native user-level path written here as `~/.amaze/agent/...` resolves to `~/.amaze/profiles/<name>/agent/...` instead.

The relocation is uniform across the native provider (`builtin.ts`) and the generic `config.ts` helpers, so it covers slash commands, rules, prompts, instructions, hooks, tools, extensions, settings, skills, and MCP, plus the top-level `SYSTEM.md` / `RULES.md` / `AGENTS.md` files and runtime state (sessions, blobs, `agent.db`). A profile sees only its own Amaze config, never the default profile's `~/.amaze/agent`.

The other source bases are not profile-scoped and load identically under every profile: the external-tool bases (`~/.claude`, `~/.codex`, `~/.gemini`) belong to those tools, and the project-level bases (`<cwd>/.amaze`, `<cwd>/.claude`, ...) are keyed to the working directory. Throughout this document, read `~/.amaze/agent` as shorthand for the active profile's agent directory.
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.amaze`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
3. CLI config overlays: `amaze --config <path>` / repeated `--config` files, loaded as `config.yml`-style YAML for this process only
...
1. Migrate from `~/.amaze/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.amaze` provider behavior (`packages/coding-agent/src/discovery/builtin.ts`)
...
- `SYSTEM.md` and `AGENTS.md` read user-level files directly and use nearest-ancestor project `.amaze` lookup for project files, but the project `.amaze` directory must be non-empty. See [`docs/system-prompt-customization.md`](./system-prompt-customization.md) for the full `SYSTEM.md` / `APPEND_SYSTEM.md` contract (replace vs. append, templating).

### Scope-specific loading

- Skills: `<ancestor>/.amaze/skills/*/SKILL.md` and `~/.amaze/agent/skills/*/SKILL.md`
...
## For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.amaze` directory search (walk-up) and still requires the project `.amaze` dir to be non-empty.
...
```text
# ~/.amaze/agent/TITLE_SYSTEM.md
Generate a session name using lowercase `<type>:<primary-objective>`.
```

- Missing `TITLE_SYSTEM.md` keeps the bundled title prompts.
- Discovery uses the same project-then-user config directory pattern as `SYSTEM.md`: project `.amaze/TITLE_SYSTEM.md` first, then user `~/.amaze/agent/TITLE_SYSTEM.md` and the other supported config bases.
