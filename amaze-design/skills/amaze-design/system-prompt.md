...
**Multilingual type.** When the UI mixes non-Latin scripts with Latin:
- Use a system fallback stack with Latin first so each script gets correct glyphs.
- Give dense reading scripts enough line-height for comfortable reading.
- Tag content with the correct `lang` value so the browser picks appropriate fonts and line-breaking.
