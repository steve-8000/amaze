...
// `__ROCKY_CONFIG__` sentinel in `static/index.html` with a JSON blob so the
...
function readConfig(): AppConfig {
  const node = document.getElementById("rocky-config");
  const text = node?.textContent?.trim();
  if (!text || text === "__ROCKY_CONFIG__") {
...
export const AUTH_HEADERS: Readonly<Record<string, string>> = CONFIG.replayEnabled
  ? Object.freeze({ "X-Rocky-Replay-Token": CONFIG.replayToken })
  : Object.freeze({});
