...
from rocky.git_ops import GitCommandError
from rocky.sandbox import (
    SandboxManager,
    Workspace,
    _chown_workspace,
...
def test_rename_workspace_branch_refreshes_shared_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "rocky.sandbox._share_git_metadata_with_slots",
...
def test_rename_workspace_branch_runs_git_as_slot_when_permissions_active(
    ...
    monkeypatch.setattr("rocky.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("rocky.sandbox.subprocess.run", fake_run)
    monkeypatch.setattr("rocky.sandbox._share_git_metadata_with_slots", lambda _repo_dir, _slot_uid: None)
...
def test_delete_bad_refs_removes_worktree_holding_the_ref(tmp_path: Path) -> None:
    ...
    from rocky.git_ops import _delete_bad_refs
...
def test_chown_workspace_noops_when_not_root(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rocky.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 1000)
    monkeypatch.setattr(
        "rocky.sandbox.subprocess.run",
...
def test_chown_workspace_noops_off_linux(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rocky.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr(
        "rocky.sandbox.subprocess.run",
        lambda cmd, *, check: calls.append((cmd, check)),
...
def test_reap_slot_noops_when_permissions_inactive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rocky.sandbox.platform.system", lambda: "Darwin")
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("rocky.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_reap_slot_kills_slot_uid_on_linux_root(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rocky.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("rocky.sandbox._slot_pids", lambda _uid: (111, 222))
    monkeypatch.setattr("rocky.sandbox.os.kill", lambda pid, sig: calls.append((pid, sig)))
...
def test_prepare_slot_tmpdir_mkdirs_without_chown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rocky.sandbox.platform.system", lambda: "Linux")
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("rocky.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
...
def test_prepare_slot_runtime_env_returns_workspace_private_paths_without_chown(
    ...
    monkeypatch.setattr("rocky.sandbox.os.geteuid", lambda: 0)
    monkeypatch.setattr("rocky.sandbox.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
    monkeypatch.setattr("rocky.sandbox.subprocess.run", lambda cmd, **_kwargs: calls.append(cmd))
...
def test_ensure_workspace_refreshes_permissions_for_retry_slot_and_session(
    ...
    monkeypatch.setattr("rocky.sandbox._chown_workspace", record_chown)
    monkeypatch.setattr("rocky.sandbox._share_git_metadata_with_slots", record_share)
...
def test_redact_credentials_strips_userinfo() -> None:
    from rocky.sandbox import redact_credentials
...
def test_git_command_error_redacts_url_in_args_and_stderr(tmp_path: Path) -> None:
    ...
    from rocky.sandbox import _run
...
def test_push_force_with_lease_succeeds_after_local_amend(tmp_path: Path, upstream_repo: Path) -> None:
    ...
    from rocky.git_ops import push as git_push
...
def test_run_git_injects_safe_directory_and_subprocess_identity(
    ...
    from rocky.git_ops import _run_git
    ...
    monkeypatch.setattr("rocky.git_ops.subprocess.run", fake_run)
...
def test_run_git_kills_hung_child(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    timeout."""
    from rocky.git_ops import GitCommandError, _run_git
...
def test_ensure_workspace_populates_from_natives_cache(tmp_path: Path, upstream_repo: Path) -> None:
    from rocky.natives_cache import NativesCache, compute_key, target_triple
...
def test_ensure_workspace_cache_miss_is_silent_noop(tmp_path: Path, upstream_repo: Path) -> None:
    from rocky.natives_cache import NativesCache
