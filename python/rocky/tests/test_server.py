...
def test_api_status_reports_runtime_counts_and_inflight(settings: Settings) -> None:
    ...
    assert runtime["bot_login"] == "rocky-bot"
...
def test_api_logs_tails_jsonl_file(settings: Settings) -> None:
    log_path = settings.log_dir / "rocky.log.jsonl"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    payloads = [
        {"ts": "2026-05-14T21:28:28Z", "level": "INFO", "logger": "rocky.queue", "msg": "dispatch loop online"},
        {
        ...
        {"ts": "2026-05-14T21:30:00Z", "level": "WARNING", "logger": "rocky.queue", "msg": "tool_end", "ok": False},
    ]
...
def _enable_replay(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("ROCKY_REPLAY_TOKEN", token)
    reset_settings_cache()
    return token
...
def test_trigger_triage_fetches_and_enqueues(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        resp = client.post(
            ...
            headers={"X-Rocky-Replay-Token": token},
        )
    close_database()
...
def test_trigger_triage_rejects_repo_not_in_allowlist(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "ROCKY_REPO_ALLOWLIST" in resp.json()["detail"]
...
def rate_limited_settings(monkeypatch: pytest.MonkeyPatch, env: dict[str, str]) -> Settings:
    monkeypatch.setenv("ROCKY_RATE_LIMIT_DEFAULT", "2")
    monkeypatch.setenv("ROCKY_RATE_LIMIT_CONTRIBUTOR", "4")
    monkeypatch.setenv("ROCKY_RATE_LIMIT_WINDOW_SECONDS", "3600")
    monkeypatch.setenv("ROCKY_RATE_LIMIT_UNLIMITED", "can1357")
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    return cfg
...
def _allowlist(monkeypatch: pytest.MonkeyPatch, repos: str) -> None:
    monkeypatch.setenv("ROCKY_REPO_ALLOWLIST", repos)
    reset_settings_cache()
...
def test_browse_returns_401_with_replay_enabled_without_valid_token(env, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        wrong = client.get(
            "/api/github/issues",
            headers={"X-Rocky-Replay-Token": f"{token}-wrong"},
        )
    close_database()
...
def test_webhook_directive_on_unknown_issue_is_queued_with_metadata(env) -> None:
    ...
    with TestClient(app) as client:
        resp = _post_issue_comment(
            ...
            user="can1357",
            number=77,
            body="@rocky-bot please refactor X",
            association="OWNER",
        )
        assert resp.status_code == 202
    ...
    assert row is not None
    assert row.state == "queued"
    directive = row.payload.get("_rocky_directive")
...
def test_webhook_maintainer_bypasses_rate_limit(
    ...
    """Login in ROCKY_MAINTAINER_LOGINS is always unlimited, even with NONE association."""
    monkeypatch.setenv("ROCKY_MAINTAINER_LOGINS", "can1357")
    reset_settings_cache()
    cfg = Settings()  # type: ignore[call-arg]
    cfg.ensure_paths()
    ...
    with TestClient(app) as client:
        ...
        for i in range(4):
            resp = _post_issue_comment(
                ...
                user="can1357",
                number=300 + i,
                body=("@rocky-bot do X" if i == 3 else "comment"),
                association="NONE",
            )
            assert resp.status_code == 202
            states.append(resp.json()["state"])
...
def stub_run_task(monkeypatch: pytest.MonkeyPatch) -> list[dict]:
    ...
    from rocky import tasks as tasks_module
...
async def test_handle_pr_conversation_unmapped_bot_pr_uses_pr_branch(
    ...
    from rocky import tasks
    from rocky.github_client import IssueInfo, PullRequestInfo, RepoInfo
...
async def test_handle_comment_directive_bootstraps_untriaged_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_rocky_directive": {"body": "please refactor X", "author": "can1357"},
    }
...
async def test_handle_comment_directive_reopens_finalized_issue(
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_rocky_directive": {"body": "redo the fix", "author": "can1357"},
    }
    await tasks.handle_comment(
        settings=settings,
        db=db,
...
async def test_directive_handler_attaches_thread_from_github(
    ...
    from rocky import tasks
    from rocky.github_client import (
        CommentInfo,
        GitHubClient,
        IssueInfo,
        RepoInfo,
    ...
    payload = {
        ...
        "repository": {"full_name": "octo/widget"},
        "_rocky_directive": {"body": "do X", "author": "can1357"},
    }
...
async def test_triage_issue_skips_when_a_closing_pr_already_exists(
    ...
    from rocky import tasks
    from rocky.github_client import IssueInfo, RepoInfo
...
async def test_triage_issue_fails_open_when_timeline_fetch_errors(
    ...
    from rocky import tasks
    from rocky.github_client import GitHubError, IssueInfo, RepoInfo
