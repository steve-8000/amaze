...
These tests swap `rocky.worker.RpcClient` for a recording fake so we can
...
from rocky import worker
from rocky.config import Settings
from rocky.git_ops import DirtyState
...
def _patch_worker(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("rocky.worker.RpcClient", _FakeRpcClient)
    monkeypatch.setattr("rocky.worker._AGENT_HOME_STAGE", tmp_path / "missing-agent-home-stage")
    monkeypatch.setattr("rocky.worker.host_tools.build", lambda _b: ())
    monkeypatch.setattr(
        "rocky.worker.persona.system_append",
        lambda *, repo, issue, workspace: "SYS",
    )
    monkeypatch.setattr(
        "rocky.worker.persona.seed_phases",
...
async def test_run_rpc_omits_continue_when_session_empty(
    ...
    assert client_kwargs["env"]["ROCKY_REPLAY_TOKEN"] == ""
    assert client_kwargs["env"]["ROCKY_GH_PROXY_HMAC_KEY"] == ""
...
async def test_run_rpc_uses_workspace_xdg_dirs_for_slot_without_chown(
    ...
    monkeypatch.setattr("rocky.sandbox.os.chown", lambda path, uid, gid: chown_calls.append((Path(path), uid, gid)))
...
async def test_run_rpc_arms_hard_timeout_timer(
    ...
    monkeypatch.setattr("rocky.worker.threading.Timer", FakeTimer)
...
async def test_run_rpc_hard_timeout_stops_client_and_fails(
    ...
    monkeypatch.setattr("rocky.worker.threading.Timer", FiringTimer)
    inputs, bindings = _make_inputs(tmp_path, settings, session_has_jsonl=False)
    loop = asyncio.new_event_loop()
    try:
    ...
    # `_closed_error` unset (amaze_rpc bug), so the worker would hang otherwise.
    assert len(fake.mark_closed_calls) == 1
    from amaze_rpc import RpcProcessExitError
...
async def test_run_rpc_cancel_hook_stops_and_marks_closed(
    ...
    an upstream amaze_rpc bug where `stop()` does not set `_closed_error`, leaving
    `_wait_for_agent_end` blocked until timeout."""
    captured: list = []
    monkeypatch.setattr("rocky.worker.register_cancel_hook", lambda hook: captured.append(hook))
    monkeypatch.setattr("rocky.worker.unregister_cancel_hook", lambda: None)
...
async def test_run_rpc_sends_reminder_when_pr_class_quits_early(tmp_path: Path, settings: Settings) -> None:
    ...
    try:
        ...
        loop.close()
    fake = _FakeRpcClient.instances[0]
    # kickoff + 2 reminders (default ROCKY_TASK_COMPLETION_MAX_REMINDERS=2)
