...
import httpx
import pytest
from amaze_rpc import HostToolContext, RpcCommandError

from rocky import host_tools
from rocky.db import Database
from rocky.github_client import GitHubClient, IssueInfo, RepoInfo
from rocky.host_tools import AbortController, ToolBindings, build
from rocky.sandbox import LocalGitTransport, Workspace
...
def _bindings(
    ...
    bindings = ToolBindings(
        ...
        loop=loop,
        author_name="rocky-bot",
        author_email="rocky-bot@example.invalid",
        slot_uid=slot_uid,
    )
    db.upsert_issue(
...
def test_repo_command_env_scrubs_secrets_and_uses_workspace_cache(
    ...
    monkeypatch.setenv("ROCKY_GH_PROXY_HMAC_KEY", "secret-proxy")
    ...
    assert env["ROCKY_GH_PROXY_HMAC_KEY"] == ""
    assert env["BUN_INSTALL_CACHE_DIR"] == str(bindings.workspace.root / ".amaze-xdg" / "cache" / "bun-install")
    assert env["XDG_CACHE_HOME"] == str(bindings.workspace.root / ".amaze-xdg" / "cache")
    assert env["TMPDIR"] == str(bindings.workspace.root / ".amaze-tmp")
    ...
    assert (bindings.workspace.root / ".amaze-tmp").is_dir()
...
def test_run_repo_command_uses_slot_identity_kwargs(
    ...
    assert kwargs["env"]["BUN_INSTALL_CACHE_DIR"].endswith("/.amaze-xdg/cache/bun-install")
...
def test_guarded_push_branch_rev_parse_runs_via_repo_command_and_passes_slot_uid(
    ...
    from rocky.git_ops import PushResult
    ...
    def fake_run_repo_command(
        ...
        if command[:3] == ["git", "log", "--format=%H%x09%ae%x09%an"]:
            return subprocess.CompletedProcess(
                command,
                0,
                "abc123\trocky-bot@example.invalid\trocky-bot\n",
                "",
            )
...
def test_gh_post_comment_happy_path(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(201, json={"id": 999, "user": {"login": "rocky-bot"}, "body": "hi", "created_at": "t"})
...
def test_gh_post_comment_defaults_to_inbound_pr_thread(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["url"] = str(request.url)
        return httpx.Response(201, json={"id": 7, "user": {"login": "rocky-bot"}, "body": "hi", "created_at": "t"})
...
def test_repro_record_chowns_to_slot_when_root(db: Database, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("rocky.host_tools.os.chown", lambda path, uid, gid: chowns.append((Path(path), uid, gid)))
...
def test_mark_unable_posts_comment_and_abandons(db: Database, tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        captured["body"] = json.loads(request.content)
        return httpx.Response(201, json={"id": 77, "user": {"login": "rocky-bot"}, "body": "x", "created_at": "t"})
...
def test_gh_push_branch_rejects_wrong_identity(db: Database, tmp_path: Path) -> None:
    ...
    from rocky.sandbox import SandboxManager
    ...
    try:
        ...
        assert "rocky-bot <rocky-bot@example.invalid>" in msg
...
def test_gh_push_branch_rejects_invalid_identity_scan_range(db: Database, tmp_path: Path) -> None:
    ...
    env = os.environ | {
        ...
        "GIT_AUTHOR_EMAIL": "rocky-bot@example.invalid",
        "GIT_COMMITTER_NAME": "rocky-bot",
        "GIT_COMMITTER_EMAIL": "rocky-bot@example.invalid",
    }
    subprocess.run(["git", "init", "--initial-branch=main", str(seed)], check=True, capture_output=True)
    (seed / "README.md").write_text("init\n")
    for cmd in (
        ...
        [
            ...
            str(seed),
            "-c",
            "user.email=rocky-bot@example.invalid",
            "-c",
            "user.name=rocky-bot",
            "commit",
            "-m",
...
def test_gh_push_branch_runs_fix_and_check_before_pushing(
    ...
    assert lines[0].startswith("rocky-bot <rocky-bot@example.invalid> style: bun run fix"), lines
...
def test_gh_open_pr_runs_fix_then_check_and_commits_fixup(
    ...
    lines = log.stdout.strip().splitlines()
    assert lines[0] == "rocky-bot|rocky-bot@example.invalid|style: bun run fix"
...
def _stub_settings(*, enabled: bool = True, hours: float = 4.0):
    ...
    from rocky.config import Settings
...
def _question_handler(captured: dict[str, Any]):
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            201,
            json={"id": 4242, "user": {"login": "rocky-bot"}, "body": "x", "created_at": "t"},
        )
