...
from rocky.cancellation import clear_current_event, set_current_event
from rocky.config import Settings
from rocky.db import Database, EventRow
from rocky.github_backend import GitHubBackend
from rocky.sandbox import GitTransport, SandboxManager, _reap_slot
from rocky.slot_pool import SlotPool
...
class WorkerPool:
    ...
    async def start(self) -> None:
        ...
        self._workers.append(asyncio.create_task(self._dispatch_loop(), name="rocky-dispatch"))
        ...
        if self.sandbox.natives_cache is not None and self.settings.natives_cache_gc_interval_seconds > 0:
            self._workers.append(asyncio.create_task(self._natives_cache_gc_loop(), name="rocky-natives-gc"))
    ...
    async def _dispatch_loop(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                task = asyncio.create_task(self._run_event(row), name=f"rocky-event-{row.delivery_id[:8]}")
