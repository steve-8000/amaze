...
from rocky import host_tools, persona, pragmas
from rocky.cancellation import register_cancel_hook, unregister_cancel_hook
from rocky.config import Settings
from rocky.db import Database, issue_key
from rocky.git_ops import DirtyState, inspect_dirty_state
from rocky.github_backend import GitHubBackend
from rocky.github_client import CommentInfo, IssueInfo, RepoInfo
from rocky.host_tools import AbortController, ToolBindings, _git_identity_env
from rocky.natives_cache import NativesCache
from rocky.natives_cache import compute_key as natives_compute_key
from rocky.sandbox import GitTransport, Workspace, _prepare_slot_runtime_env, _safe_directory_env
...
_SCRUBBED_ENV_KEYS: tuple[str, ...] = (
    ...
    # `bash` tool could otherwise `printenv` them out of rocky's env.
    "GITHUB_TOKEN",
    "GITHUB_WEBHOOK_SECRET",
    "ROCKY_REPLAY_TOKEN",
    "ROCKY_GH_PROXY_HMAC_KEY",
)
...
def _build_extra_env(settings: Settings) -> dict[str, str]:
    ...
    `amaze_rpc` merges this dict on top of `os.environ`, so overlaying empty
...
def _run_rpc_blocking(
    ...
    *,
    ...
    with RpcClient(
        executable=settings.amaze_command,
        ...
        # it's idempotent.
        #
        # NOTE: amaze_rpc.RpcClient.stop() has a bug where it sets `_stopping=True`
