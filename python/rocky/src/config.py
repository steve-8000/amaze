"""Env-driven configuration for rocky."""
...
class Settings(BaseSettings):
    ...
    bot_login: str = Field(..., alias="ROCKY_BOT_LOGIN")
    git_author_name: str | None = Field(None, alias="ROCKY_GIT_AUTHOR_NAME")
    git_author_email: str = Field(..., alias="ROCKY_GIT_AUTHOR_EMAIL")
    repo_allowlist_raw: str = Field("", alias="ROCKY_REPO_ALLOWLIST")
    ...
    gh_proxy_url: str | None = Field(None, alias="ROCKY_GH_PROXY_URL")
    gh_proxy_hmac_key: SecretStr | None = Field(None, alias="ROCKY_GH_PROXY_HMAC_KEY")
    # Bind address for `python -m rocky.proxy serve`. Internal-only by
    # default; gh-proxy never exposes a host port.
    gh_proxy_bind_host: str = Field("0.0.0.0", alias="ROCKY_GH_PROXY_BIND_HOST")
    gh_proxy_bind_port: int = Field(8081, alias="ROCKY_GH_PROXY_BIND_PORT")
    ...
    gh_proxy_max_body_bytes: int = Field(1 << 20, alias="ROCKY_GH_PROXY_MAX_BODY_BYTES")
    ...
    gh_proxy_git_timeout_seconds: float = Field(60.0, alias="ROCKY_GH_PROXY_GIT_TIMEOUT_SECONDS")

    # Model selection
    model: str = Field("anthropic/claude-sonnet-4-6", alias="ROCKY_MODEL")
    provider: str | None = Field(None, alias="ROCKY_PROVIDER")
    thinking_level: ThinkingLevel = Field("high", alias="ROCKY_THINKING")

    # Runtime
    max_concurrency: int = Field(8, alias="ROCKY_MAX_CONCURRENCY")
    task_timeout_seconds: float = Field(2400.0, alias="ROCKY_TASK_TIMEOUT_SECONDS")
    task_timeout_hard_grace_seconds: float = Field(60.0, alias="ROCKY_TASK_TIMEOUT_HARD_GRACE_SECONDS")
    request_timeout_seconds: float = Field(120.0, alias="ROCKY_REQUEST_TIMEOUT_SECONDS")
    ...
    # Set to 0 to disable.
    task_completion_max_reminders: int = Field(2, alias="ROCKY_TASK_COMPLETION_MAX_REMINDERS")
    amaze_command: str = Field("amaze", alias="ROCKY_AMAZE_COMMAND")
    ...
    log_dir: Path = Field(Path("./data/logs"), alias="ROCKY_LOG_DIR")

    # Server
    bind_host: str = Field("0.0.0.0", alias="ROCKY_BIND_HOST")
    bind_port: int = Field(8080, alias="ROCKY_BIND_PORT")

    # Dev-only replay header value; if empty, /replay is disabled
    replay_token: SecretStr | None = Field(None, alias="ROCKY_REPLAY_TOKEN")
    ...
    rate_limit_window_seconds: float = Field(3600.0, alias="ROCKY_RATE_LIMIT_WINDOW_SECONDS")
    rate_limit_default: int = Field(3, alias="ROCKY_RATE_LIMIT_DEFAULT")
    rate_limit_contributor: int = Field(10, alias="ROCKY_RATE_LIMIT_CONTRIBUTOR")
    rate_limit_unlimited_raw: str = Field("", alias="ROCKY_RATE_LIMIT_UNLIMITED")
    ...
    maintainer_logins_raw: str = Field("", alias="ROCKY_MAINTAINER_LOGINS")
    ...
    reviewer_bots_raw: str = Field("", alias="ROCKY_REVIEWER_BOTS")
    ...
    question_autoclose_enabled: bool = Field(True, alias="ROCKY_QUESTION_AUTOCLOSE_ENABLED")
    question_autoclose_hours: float = Field(4.0, alias="ROCKY_QUESTION_AUTOCLOSE_HOURS")
    question_autoclose_scan_seconds: float = Field(60.0, alias="ROCKY_QUESTION_AUTOCLOSE_SCAN_SECONDS")

    # natives build-output cache. Hardlinks pre-built
    ...
    natives_cache_enabled: bool = Field(True, alias="ROCKY_NATIVES_CACHE_ENABLED")
    natives_cache_root: Path = Field(Path("/data/cache/natives"), alias="ROCKY_NATIVES_CACHE_ROOT")
    natives_cache_max_entries_per_repo: int = Field(8, alias="ROCKY_NATIVES_CACHE_MAX_ENTRIES_PER_REPO")
    natives_cache_max_bytes: int = Field(4 * 1024**3, alias="ROCKY_NATIVES_CACHE_MAX_BYTES")
    natives_cache_gc_interval_seconds: float = Field(3600.0, alias="ROCKY_NATIVES_CACHE_GC_INTERVAL_SECONDS")
    ...
    def _require_bot_login(cls, value: str) -> str:
        cleaned = value.strip()
        if not cleaned:
            raise ValueError("ROCKY_BOT_LOGIN must be a non-empty GitHub login")
        return cleaned
    ...
    def _validate_proxy_or_pat(self) -> Settings:
        ...
        if has_token and has_url:
            raise ValueError(
                "GITHUB_TOKEN and ROCKY_GH_PROXY_URL are mutually exclusive — "
                "set ONE to choose between direct-PAT and gh-proxy modes."
            )
        if has_url != has_key:
            raise ValueError(
                "ROCKY_GH_PROXY_URL and ROCKY_GH_PROXY_HMAC_KEY must both be set together (or both empty)."
            )
    ...
    def model_pool(self) -> tuple[str, ...]:
        """ROCKY_MODEL may be a single id or a comma-separated list; this
    ...
    def resolved_author_name(self) -> str:
        """Falls back to bot_login if ROCKY_GIT_AUTHOR_NAME isn't set."""
...
class _ProxyEnvLoader(BaseSettings):
    """Minimal env loader for `python -m rocky.proxy serve`.
    ...
    gh_proxy_hmac_key: SecretStr = Field(..., alias="ROCKY_GH_PROXY_HMAC_KEY")
    gh_proxy_bind_host: str = Field("0.0.0.0", alias="ROCKY_GH_PROXY_BIND_HOST")
    gh_proxy_bind_port: int = Field(8081, alias="ROCKY_GH_PROXY_BIND_PORT")
    workspace_root: Path = Field(Path("./data/workspaces"), alias="ROCKY_WORKSPACE_ROOT")
    log_dir: Path = Field(Path("./data/logs"), alias="ROCKY_LOG_DIR")
