...
from rocky.proxy_client import GitHubProxyClient, ProxyGitTransport
from rocky.queue import WorkerPool
...
def _require_proxy_mode(cfg: Settings) -> tuple[str, bytes]:
    if cfg.github_token is not None:
        raise SystemExit(
            "rocky orchestrator refuses to start with GITHUB_TOKEN set in env. "
            "The PAT must live only in the gh-proxy container."
        )
    if cfg.gh_proxy_url is None or cfg.gh_proxy_hmac_key is None:
        raise SystemExit(
            "rocky orchestrator requires ROCKY_GH_PROXY_URL and "
            "ROCKY_GH_PROXY_HMAC_KEY (run gh-proxy in a sibling container)."
        )
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="rocky", version="0.1.0", lifespan=lifespan)
    ...
    async def replay(
        ...
        if x_rocky_token != cfg.replay_token.get_secret_value():
    ...
    def _require_trigger_token(cfg: Settings, token: str | None) -> None:
        if cfg.replay_token is None:
            raise HTTPException(404, "trigger disabled (set ROCKY_REPLAY_TOKEN to enable)")
    ...
    async def api_github_issues(
        ...
        bag = request.app.state.bag
        cfg: Settings = bag["settings"]
        _require_trigger_token(cfg, x_rocky_token)
    ...
    async def api_trigger(
        ...
        if mode == "triage":
            ...
            if not cfg.allows(repo_full):
                raise HTTPException(403, f"{repo_full} not in ROCKY_REPO_ALLOWLIST")
            try:
