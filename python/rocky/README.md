...
Self-hosted GitHub triage bot. Drives [`amaze --mode rpc`](https://github.com/can1357/amaze)
...
- `question` → one comment, suffixed with a 👎-to-keep-open prompt; if the
  issue author doesn't react 👎 within `ROCKY_QUESTION_AUTOCLOSE_HOURS`
...
Follow-up issue comments and PR review comments resume the same amaze session
...
- **rocky** — FastAPI + sqlite event queue + `WorkerPool` running `amaze` in
...
- **gh-proxy** — sibling on an `internal: true` network. Holds `GITHUB_TOKEN`,
  verifies HMAC-signed requests from rocky, executes REST + `git push`.
...
→ `worker.run_task` spawns `amaze --mode rpc` with `cwd=worktree`,
persistent `session_dir`, model randomly drawn from `ROCKY_MODEL` (CSV).

The agent uses amaze's built-in tools (`read`/`edit`/`bash`/`lsp`, scoped to
...
`~/.amaze/agent/models.container.yml` points at (mounted into the container as `models.yml`; kept under a separate filename on the host so the host amaze doesn't route through the gateway). rocky lives inside the amaze
monorepo at `python/rocky/`; both the docker build context and the
`/work/amaze` bind mount default to the parent monorepo (`../..`). Override
`AMAZE_ROOT` only if you want a different amaze checkout backing the build
and runtime.

Bot account needs **Write** on every repo in `ROCKY_REPO_ALLOWLIST`. A
...
cp .env.example .env
$EDITOR .env
openssl rand -hex 32              # ROCKY_GH_PROXY_HMAC_KEY
openssl rand -hex 32              # GITHUB_WEBHOOK_SECRET

bun run amaze:image                  # build amaze/amaze:dev (one-time / on amaze change)
bun run rocky:build && bun run rocky:up
...
comment out `ROCKY_GH_PROXY_URL` / `ROCKY_GH_PROXY_HMAC_KEY` and set
`GITHUB_TOKEN`. The two modes are mutually exclusive (`config.py`
rejects a `.env` setting both).

Build invalidation is bounded: editing rocky Python touches only the
runtime layer; editing amaze source rebuilds `amaze/amaze:dev`, which
rocky's `Dockerfile.rocky` extends via `FROM ${AMAZE_BASE}`.

### Public URL

rocky does not ship a tunnel. Cloudflare, smee, ngrok are all fine. The
...
The container entrypoint is `python -m rocky serve`. Other commands run
inside the running container:

```bash
docker compose exec rocky rocky triage  owner/repo#123   # synthesize an issues.opened and wait
docker compose exec rocky rocky replay  <delivery_id>    # re-enqueue a stored event and wait
docker compose exec rocky rocky status                   # dump issues table
docker compose exec rocky rocky cleanup owner/repo#123   # force workspace removal, state=abandoned
```

`bun run rocky:…` shortcuts in the root `package.json` cover the common
lifecycle commands (`rocky:dev`, `rocky:build`, `rocky:up`, `rocky:down`,
`rocky:logs`, `rocky:restart`, `rocky:reset`).

## Tests

```bash
pytest -x tests/                              # unit suite, no network
ROCKY_INTEGRATION=1 pytest -x tests/test_worker_smoke.py
```

The integration test spawns a real `amaze --mode rpc` against an
`httpx.MockTransport` GitHub and a local bare repo, so it needs `amaze` on
...
- gh-proxy has no host port. The `rocky_internal` network is
...
- Agent subprocess env is scrubbed of `GITHUB_TOKEN` /
  `ROCKY_GH_PROXY_HMAC_KEY` / friends via `worker._SCRUBBED_ENV_KEYS`.
...
- Pre-push gates (`gh_push_branch`): branch matches the workspace
  ...
  `origin/<default>..HEAD` carries `ROCKY_GIT_AUTHOR_NAME` +
  `ROCKY_GIT_AUTHOR_EMAIL`.
...
- **Crash recovery.** On startup, `db.reset_stuck_running()` flips
  ...
  `ROCKY_SHUTDOWN_DRAIN_TIMEOUT_SECONDS` (25s) +
  `ROCKY_SHUTDOWN_KILL_TIMEOUT_SECONDS` (5s); compose
...
| Container exits with `AMAZE_ROOT … missing` | `/work/amaze` mount empty inside the container; on the host either run `docker compose` from `python/rocky/` so `AMAZE_ROOT` defaults to `../..`, or export `AMAZE_ROOT` to a valid amaze checkout. |
| `git push: Authentication required` | Bot PAT lacks push, or `ROCKY_BOT_LOGIN` ≠ PAT's account. |
| `refusing to push: commit author identity mismatch` | Some commit not authored as `ROCKY_GIT_AUTHOR_*`. The error lists the offending shas; `git commit --amend --reset-author --no-edit`. |
...
| `Failed to load pi_natives` | Wrong arch / missing native. `bun run amaze:image` then `bun run rocky:build`. |
| `No API key found for <provider>` | `~/.amaze/agent/models.container.yml` mount missing or provider id mismatch with `ROCKY_MODEL`. |
...
src/
  ...
  worker.py          synchronous amaze RPC driver, prompt assembly, env scrubbing
