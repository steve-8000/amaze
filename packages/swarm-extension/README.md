...
Multi-agent orchestration for amaze-agent. Define agent workflows in YAML — pipelines, parallel fan-outs, sequential chains, or any DAG — and run them unattended until completion.

Each agent is a full amaze-agent subagent with access to every tool: bash, python, read, write, edit, grep, find, fetch, web_search, browser. The orchestrator manages lifecycle and ordering; agents communicate through the shared workspace filesystem.
...
amaze-swarm path/to/swarm.yaml

# Background — survives terminal close:
nohup amaze-swarm path/to/swarm.yaml \
...
### Inside amaze-agent (TUI)

Register the extension in your config (`~/.amaze/config.json` or `.amaze/config.json`):
...
Each agent is a full amaze-agent session. It can:
...
Any model configured in amaze works. Set a swarm default and optionally override per agent:
...
src/swarm/
  ...
  executor.ts         Spawns agents via amaze-agent's runSubprocess
