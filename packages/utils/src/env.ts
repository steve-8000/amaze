 ...
 *
 * All users should import this env module (import { $env } from "@amaze/utils")
 ...
 * `AMAZE_COMPILED` env var (set by the build script's `--define`) is checked
 * first for cheap fast-path detection.
 */
export function isCompiledBinary(): boolean {
	if (Bun.env.AMAZE_COMPILED) return true;
