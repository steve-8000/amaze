...
import { Process, ProcessStatus } from "@amaze/natives";
...
function buildSpawnEnv(shell: string): Record<string, string> {
	const noCI = $env.AMAZE_BASH_NO_CI || $env.CLAUDE_BASH_NO_CI;
	return {
 ...
 * Supports AMAZE_BASH_NO_LOGIN and CLAUDE_BASH_NO_LOGIN to skip -l.
 */
function getShellArgs(): string[] {
	const noLogin = $env.AMAZE_BASH_NO_LOGIN || $env.CLAUDE_BASH_NO_LOGIN;
...
function getShellPrefix(): string | undefined {
	return $env.AMAZE_SHELL_PREFIX || $env.CLAUDE_CODE_SHELL_PREFIX;
}
