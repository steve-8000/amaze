...
- Added package-level TypeScript exports and publication config so consumers can import `@steve-z8k/snapcompact` with typed access to snapcompact APIs
- Published `@steve-z8k/snapcompact` as the reusable snapcompact compaction package, including bitmap-frame rendering helpers, archive helpers, and the local `snapcompactCompact()` strategy.
