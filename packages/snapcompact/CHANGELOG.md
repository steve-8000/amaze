...
- Added package-level TypeScript exports and publication config so consumers can import `@amaze/snapcompact` with typed access to snapcompact APIs
- Published `@amaze/snapcompact` as the reusable snapcompact compaction package, including bitmap-frame rendering helpers, archive helpers, and the local `snapcompactCompact()` strategy.
