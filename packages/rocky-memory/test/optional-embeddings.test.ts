...
describe("optional embeddings", () => {
	it("falls back cleanly when embeddings are disabled", async () => {
		await withEnv({ ROCKY_MEMORY_NO_EMBEDDINGS: "1" }, async () => {
	...
	it("uses a fake provider and caches single-query embeddings", async () => {
		await withEnv({ ROCKY_MEMORY_NO_EMBEDDINGS: undefined }, async () => {
			let calls = 0;
			setEmbeddingProviderForTests({
	...
	it("calls an OpenAI-compatible custom embeddings endpoint without requiring an API key", async () => {
		...
		const server = Bun.serve({
			...
			fetch: async request => {
				...
				expect(request.headers.get("http-referer")).toBe("https://amaze/");
		...
		try {
			await withEnv(
				{
					ROCKY_MEMORY_NO_EMBEDDINGS: undefined,
					ROCKY_MEMORY_EMBEDDING_MODEL: "openai/text-embedding-3-small",
					ROCKY_MEMORY_EMBEDDING_API_URL: server.url.toString().replace(/\/+$/, ""),
					ROCKY_MEMORY_EMBEDDING_API_KEY: undefined,
	...
	it("lets constructor-scoped noEmbeddings override enabled providers", async () => {
		...
		const memory = new RockyMemory({ noEmbeddings: true });
		try {
			const result = await withRockyMemoryRuntimeOptions(memory.runtimeOptions, () => embed(["hello"]));
	...
	it("uses a constructor-scoped embedding provider", async () => {
		...
		try {
			const result = await withRockyMemoryRuntimeOptions(memory.runtimeOptions, () => embedQuery("cache me"));
		...
		await withEnv(
			{
				...
				BUN_ENV: undefined,
				ROCKY_MEMORY_NO_EMBEDDINGS: undefined,
				ROCKY_MEMORY_EMBEDDING_MODEL: "BAAI/bge-small-en-v1.5",
				ROCKY_MEMORY_EMBEDDING_API_URL: undefined,
