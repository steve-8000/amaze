...
afterEach(() => {
	if (ORIGINAL_BLOB_DIR === undefined) {
		delete process.env.ROCKY_MEMORY_BLOB_DIR;
	} else {
		process.env.ROCKY_MEMORY_BLOB_DIR = ORIGINAL_BLOB_DIR;
	}
});

function useTempBlobDir(): string {
	const dir = mkdtempSync(join(tmpdir(), "rockyMemory-blobs-"));
	process.env.ROCKY_MEMORY_BLOB_DIR = join(dir, "blobs");
	return process.env.ROCKY_MEMORY_BLOB_DIR;
}
