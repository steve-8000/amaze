...
import { defaultTripleDbPath, TripleStore } from "@steve-z8k/pi-rocky-memory/core/triples";

const originalHome = process.env.HOME;
const originalDataDir = process.env.ROCKY_MEMORY_DATA_DIR;
const roots: string[] = [];

function tempRoot(): string {
	const root = mkdtempSync(join(tmpdir(), "rockyMemory-ts-triples-"));
	roots.push(root);
	return root;
}
...
afterEach(() => {
	...
	if (originalDataDir === undefined) delete process.env.ROCKY_MEMORY_DATA_DIR;
	else process.env.ROCKY_MEMORY_DATA_DIR = originalDataDir;
	while (roots.length > 0) rmSync(roots.pop() as string, { recursive: true, force: true });
});

describe("TripleStore default data-directory handling", () => {
	it("keeps triples.db beside the configured RockyMemory data directory", () => {
		const root = tempRoot();
		...
		try {
			...
			expect(existsSync(join(home, ".hermes", "rockyMemory", "data", "triples.db"))).toBe(false);
		} finally {
			store.close();
		}
	});

	it("copies an existing legacy triples database into ROCKY_MEMORY_DATA_DIR", () => {
		const root = tempRoot();
		const home = join(root, "home");
		const dataDir = join(root, "configured-data");
		const legacyDb = join(home, ".hermes", "rockyMemory", "data", "triples.db");
		process.env.HOME = home;
		process.env.ROCKY_MEMORY_DATA_DIR = dataDir;
