...
import { cmdRecall, cmdRemember, cmdStats, runCli } from "@steve-z8k/pi-rocky-memory/cli";
import { BeamMemory } from "@steve-z8k/pi-rocky-memory/core/beam";

function tempRoot(): string {
	return mkdtempSync(join(tmpdir(), "rockyMemory-ts-cli-"));
}
...
describe("CLI command handlers", () => {
	...
	it("reports usage, parse, operation, and unknown-command errors without tracebacks", async () => {
		...
		try {
			...
			expect(usageIo.stderr).toContain("Usage: rockyMemory store <content> [source] [importance]");
			...
			expect(unknownIo.stderr).toContain("Run 'rockyMemory --help' for usage.");
