...
function tempDb(): string {
	const root = mkdtempSync(join(tmpdir(), "rockyMemory-identity-parity-"));
	roots.push(root);
...
describe("identity memory parity", () => {
	...
	it("stores author and channel identity on remember and defaults channel to session", () => {
		const dbPath = tempDb();
		const identified = new RockyMemory({
			dbPath,
			sessionId: "session-a",
			authorId: "abdias",
			authorType: "human",
		...
		const anonymous = new RockyMemory({ dbPath, sessionId: "session-b" });
		try {
	...
	it("reports working stats through identity filters", () => {
		const dbPath = tempDb();
		const a = new RockyMemory({ dbPath, sessionId: "a1", authorId: "abdias", channelId: "team" });
		const b = new RockyMemory({ dbPath, sessionId: "b1", authorId: "sarah", channelId: "team" });
		try {
