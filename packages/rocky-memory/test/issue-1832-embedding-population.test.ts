...
function withFakeMemory<T>(fn: (memory: RockyMemory, calls: () => number) => Promise<T>): Promise<T> {
...
/**
 * Re-enter the per-RockyMemory runtime-options scope when reaching into `memory.beam`
 * directly (only `RockyMemory.remember`/`recall`/etc. enter it automatically).
 */
function inScope<T>(memory: RockyMemory, fn: () => T): T {
	return withRockyMemoryRuntimeOptions(memory.runtimeOptions, fn);
}

function readEmbeddings(memory: RockyMemory): EmbeddingRow[] {
	return memory.conn
...
describe("issue #1832 — embedding write/read coverage", () => {
	...
	it("flushes pending embeddings before close so short-lived `rockyMemory store` owners persist them", async () => {
		...
		const { provider, calls } = fakeProvider();
		const dbPath = `${tmpdir()}/rockyMemory-1833-${randomBytes(6).toString("hex")}.db`;
		const captured: string[] = [];
		const stdout = { write: (s: string) => captured.push(s) };
		const stderr = { write: (s: string) => captured.push(s) };
		const runtimeOptions: ResolvedRockyMemoryRuntimeOptions = {
			embeddings: { provider },
		};

		await withRockyMemoryRuntimeOptions(runtimeOptions, async () => {
	...
	it("scopes embedQuery() cache per provider so two RockyMemory runtimes do not cross-contaminate", async () => {
		// Repro for #1833 review comment: `embedQuery()` caches by query text only, so a
		// second `RockyMemory` in the same process with a different provider would read back
		// the first runtime's vector and score against it. Each RockyMemory here exposes a
		...
		const alpha = new RockyMemory({
			db: new Database(":memory:"),
			embeddings: { provider: alphaProvider },
		});
		const beta = new RockyMemory({
			db: new Database(":memory:"),
