...
describe("BankManager", () => {
	it("creates, lists, renames, stats, and deletes isolated bank directories", () => {
		const root = mkdtempSync(join(tmpdir(), "rockyMemory-banks-"));
		try {
			...
			expect(manager.getBankDbPath("default")).toBe(join(root, "rockyMemory.db"));
			expect(manager.getBankDbPath("work")).toBe(join(root, "banks", "work", "rockyMemory.db"));
			expect(manager.getBankStats("work").db_size_bytes).toBeGreaterThanOrEqual(0);
			const renamed = manager.renameBank("work", "project_a");
			expect(renamed).toBe(join(root, "banks", "project_a", "rockyMemory.db"));
	...
	it("validates names and protects default deletion", () => {
		...
		try {
			...
			expect(manager.getBankDbPath("")).toBe(join(root, "rockyMemory.db"));
