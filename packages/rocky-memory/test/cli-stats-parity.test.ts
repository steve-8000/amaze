...
beforeEach(() => {
	root = mkdtempSync(join(tmpdir(), "rockyMemory-ts-cli-stats-parity-"));
...
describe("CLI stats parity", () => {
	it("prints real working, episodic, triple, bank, and database counts", async () => {
		const dbPath = join(root, "rockyMemory.db");
	...
	it("stats commands read the configured data directory, not the default home path", async () => {
		...
		expect(existsSync(join(customDataDir, "rockyMemory.db"))).toBe(true);
		...
		expect(statsIo.stdout).toContain(join(customDataDir, "rockyMemory.db"));
	});
});

describe("rockyMemory-stats diagnostic behavior parity", () => {
	...
	it("diagnostics initialize missing databases gracefully and report zero counts", () => {
		const dbPath = join(root, "empty", "rockyMemory.db");
