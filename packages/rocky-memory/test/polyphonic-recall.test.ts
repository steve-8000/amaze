...
const previousPolyphonic = process.env.ROCKY_MEMORY_POLYPHONIC_RECALL;

afterEach(() => {
	if (previousPolyphonic === undefined) delete process.env.ROCKY_MEMORY_POLYPHONIC_RECALL;
	else process.env.ROCKY_MEMORY_POLYPHONIC_RECALL = previousPolyphonic;
	delete process.env.ROCKY_MEMORY_VOICE_VECTOR;
	delete process.env.ROCKY_MEMORY_VOICE_GRAPH;
	delete process.env.ROCKY_MEMORY_VOICE_FACT;
	delete process.env.ROCKY_MEMORY_VOICE_TEMPORAL;
});

describe("PolyphonicRecallEngine", () => {
	it("reads the polyphonic recall gate per call", () => {
		delete process.env.ROCKY_MEMORY_POLYPHONIC_RECALL;
		expect(polyphonicRecallIsEnabled()).toBe(false);
		process.env.ROCKY_MEMORY_POLYPHONIC_RECALL = "0";
		expect(polyphonicRecallIsEnabled()).toBe(false);
		process.env.ROCKY_MEMORY_POLYPHONIC_RECALL = "1";
	...
	it("honors per-voice gates without producing fake-success results", () => {
		...
		try {
			const engine = seedPolyphonicFixture(beam);
			process.env.ROCKY_MEMORY_VOICE_VECTOR = "0";
			process.env.ROCKY_MEMORY_VOICE_GRAPH = "0";
			process.env.ROCKY_MEMORY_VOICE_TEMPORAL = "0";
			const results = engine.recall("Alice recent", [1, 0], 10);
	...
	it("filters vector and temporal voices to beam-session or global memories", () => {
		...
		try {
			...
			process.env.ROCKY_MEMORY_VOICE_GRAPH = "0";
			process.env.ROCKY_MEMORY_VOICE_FACT = "0";
			process.env.ROCKY_MEMORY_VOICE_TEMPORAL = "0";
