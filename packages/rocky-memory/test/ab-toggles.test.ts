...
const roots: string[] = [];
const toggleNames = [
	"ROCKY_MEMORY_VOICE_VECTOR",
	"ROCKY_MEMORY_VOICE_GRAPH",
	"ROCKY_MEMORY_VOICE_FACT",
	"ROCKY_MEMORY_VOICE_TEMPORAL",
] as const;
const savedEnv: Partial<Record<(typeof toggleNames)[number], string>> = {};

function tempDb(): string {
	const root = mkdtempSync(join(tmpdir(), "rockyMemory-ab-toggle-"));
	roots.push(root);
	return join(root, "rockyMemory.db");
}
...
describe("A/B polyphonic voice toggles", () => {
	it("treats falsy values as disabled with whitespace and case normalization", () => {
		...
		for (const value of falsyValues) {
			withEngine(engine => {
				process.env.ROCKY_MEMORY_VOICE_VECTOR = value;
				process.env.ROCKY_MEMORY_VOICE_GRAPH = value;
				process.env.ROCKY_MEMORY_VOICE_FACT = value;
				process.env.ROCKY_MEMORY_VOICE_TEMPORAL = value;
