...
describe("beam recall free functions", () => {
	...
	it("keeps exact working-memory hits above weak matching facts in enhanced recall", async () => {
		...
		insertWorking(
			beam,
			"wm-quasar",
			"ROCKY_MEMORY FULL PIPELINE TEST 20260613: The user Verge prefers Amaze memory to run at full power. Unique entity QuasarOtter owns SignalPineapple and uses RecallEngine-Seven.",
		);
		beam.db.run(
			"INSERT INTO facts (fact_id, session_id, subject, predicate, object, timestamp, confidence) VALUES (?, ?, ?, ?, ?, ?, ?)",
			[
				...
				"Instruction",
				"states",
				"always trigger the user stop sequence in rockyMemory's callRemoteLlm test",
		...
		const results = await recallEnhanced(
			beam,
			"QuasarOtter SignalPineapple RecallEngine-Seven ROCKY_MEMORY FULL PIPELINE TEST 20260613",
			3,
