...
function makeMemory(llm: false | { complete: RockyMemoryLlmCompletion }): RockyMemory {
...
describe("remember(extract) wires the LLM fact extractor", () => {
	...
	it("uses a runtime-configured remote LLM in background extraction", async () => {
		const previousEnabled = process.env.ROCKY_MEMORY_LLM_ENABLED;
		const previousBaseUrl = process.env.ROCKY_MEMORY_LLM_BASE_URL;
		let requestedUrl = "";
		let requestedBody = "";
		...
		try {
			...
			delete process.env.ROCKY_MEMORY_LLM_BASE_URL;
			const memory = new RockyMemory({
			...
			if (previousEnabled === undefined) delete process.env.ROCKY_MEMORY_LLM_ENABLED;
			else process.env.ROCKY_MEMORY_LLM_ENABLED = previousEnabled;
			if (previousBaseUrl === undefined) delete process.env.ROCKY_MEMORY_LLM_BASE_URL;
			else process.env.ROCKY_MEMORY_LLM_BASE_URL = previousBaseUrl;
		}
	});
