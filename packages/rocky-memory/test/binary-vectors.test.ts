...
import {
	...
	quantizeInt8,
} from "@steve-z8k/pi-rocky-memory/core/binary-vectors";
...
describe("binary vector helpers", () => {
	...
	it("normalizes ROCKY_MEMORY_VEC_TYPE with Python-compatible fallback", () => {
		expect(getVecType({ ROCKY_MEMORY_VEC_TYPE: "bit" })).toBe("bit");
		expect(getVecType({ ROCKY_MEMORY_VEC_TYPE: "int8" })).toBe("int8");
		expect(getVecType({ ROCKY_MEMORY_VEC_TYPE: "float32" })).toBe("float32");
		expect(getVecType({ ROCKY_MEMORY_VEC_TYPE: "bogus" })).toBe("float32");
