...
afterEach(() => {
	if (previousProactive === undefined) delete process.env.ROCKY_MEMORY_PROACTIVE_LINKING;
	else process.env.ROCKY_MEMORY_PROACTIVE_LINKING = previousProactive;
...
describe("proactive memory linking", () => {
	...
	it("honors host configuration when the environment variable is unset", () => {
		delete process.env.ROCKY_MEMORY_PROACTIVE_LINKING;
	...
	it("keeps host configuration scoped to each RockyMemory instance", () => {
		delete process.env.ROCKY_MEMORY_PROACTIVE_LINKING;
		const enabled = new RockyMemory({
			sessionId: "proactive-rockyMemory-on",
			dbPath: ":memory:",
			proactiveLinking: true,
		});
		configureRecallFeatures({ proactiveLinking: false });
		const disabled = new RockyMemory({
			sessionId: "proactive-rockyMemory-off",
			dbPath: ":memory:",
			proactiveLinking: false,
		});
		try {
	...
	it("lets the environment variable override instance configuration", () => {
		process.env.ROCKY_MEMORY_PROACTIVE_LINKING = "0";
