...
function tempRoot(): string {
	const root = mkdtempSync(join(tmpdir(), "rockyMemory-c25-delta-"));
	roots.push(root);
	return root;
}

function seededMemory(): { memory: RockyMemory; root: string } {
	const root = tempRoot();
	const memory = new RockyMemory({ sessionId: "s1", dbPath: join(root, "rockyMemory.db") });
