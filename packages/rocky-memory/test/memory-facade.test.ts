...
function tempRoot(): string {
	const root = mkdtempSync(join(tmpdir(), "rockyMemory-memory-facade-"));
	roots.push(root);
	return root;
}
...
function useTempDataDir(): string {
	const root = tempRoot();
	previousDataDir = process.env.ROCKY_MEMORY_DATA_DIR;
	process.env.ROCKY_MEMORY_DATA_DIR = root;
	return root;
}
...
afterEach(() => {
	...
	if (previousDataDir === undefined) {
		...
		process.env.ROCKY_MEMORY_DATA_DIR = previousDataDir;
	}
...
describe("RockyMemory facade", () => {
	...
	it("accepts an already-open Database handle for memory, annotations, and episodic graph writes", () => {
		const previousProactiveLinking = process.env.ROCKY_MEMORY_PROACTIVE_LINKING;
		process.env.ROCKY_MEMORY_PROACTIVE_LINKING = "1";
		const db = openDatabase(":memory:");
		const memory = new RockyMemory({ db, sessionId: "external-db" });
		try {
			...
			memory.close();
			db.close();
			if (previousProactiveLinking === undefined) delete process.env.ROCKY_MEMORY_PROACTIVE_LINKING;
			else process.env.ROCKY_MEMORY_PROACTIVE_LINKING = previousProactiveLinking;
		}
	});

	it("stores duplicate-content batch items with distinct ids", () => {
		const memory = new RockyMemory({ dbPath: join(tempRoot(), "rockyMemory.db"), sessionId: "batch" });
		try {
	...
	it("preserves legacy and Python-compatible aliases", async () => {
		const memory = new RockyMemory({
			dbPath: join(tempRoot(), "rockyMemory.db"),
			session_id: "aliases",
		});
		try {
