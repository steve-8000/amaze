...
beforeEach(() => {
	root = mkdtempSync(join(tmpdir(), "rockyMemory-ts-cli-errors-parity-"));
...
describe("CLI usage and operation failure parity", () => {
	it("reports missing required arguments as usage errors without tracebacks", async () => {
		for (const [args, expected] of [
			[["store"], "Usage: rockyMemory store <content> [source] [importance]"],
			[["recall"], "Usage: rockyMemory recall <query> [top_k]"],
			[["update", "missing-id"], "Usage: rockyMemory update <memory_id> <new_content> [importance]"],
			[["delete"], "Usage: rockyMemory delete <memory_id>"],
			[["import"], "Usage: rockyMemory import <file.json>"],
			[["export"], "Usage: rockyMemory export <file.json>"],
			[["bank"], "Usage: rockyMemory bank <list|create|delete> [name]"],
		] as const) {
			const io = capture();
	...
	it("reports parse errors and unknown commands without tracebacks", async () => {
		...
		expect(unknown.stderr).toContain("Run 'rockyMemory --help' for usage.");
	...
	it("reports import file and JSON validation errors without tracebacks", async () => {
		...
		for (const payload of ["[]", '"not an export"']) {
			...
			expect(io.stdout).toBe("");
			expect(io.stderr).toContain("Import file must contain a RockyMemory export object");
			expect(io.stderr).not.toContain("Traceback");
		}
	...
	it("bank validation errors are user-facing", async () => {
		for (const [args, expected, code] of [
			...
			[["bank", "create"], "Usage: rockyMemory bank create <name>", 2],
			[["bank", "delete"], "Usage: rockyMemory bank delete <name>", 2],
