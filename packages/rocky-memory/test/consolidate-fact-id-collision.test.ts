...
import { computeFactId, VeracityConsolidator } from "@steve-z8k/pi-rocky-memory/core/veracity-consolidation";
...
function withDb<T>(fn: (path: string, cons: VeracityConsolidator) => T): T {
	const dir = mkdtempSync(join(tmpdir(), "rockyMemory-veracity-"));
