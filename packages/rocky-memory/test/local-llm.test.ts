...
describe("local LLM TypeScript port", () => {
	it("reports remote availability and calls OpenAI-compatible HTTP", async () => {
		process.env.ROCKY_MEMORY_LLM_BASE_URL = "http://local-llm/v1";
		process.env.ROCKY_MEMORY_LLM_API_KEY = "sk-test";
		process.env.ROCKY_MEMORY_LLM_MODEL = "test-model";
		let auth = "";
		let model = "";
	...
	it("uses host backend before remote and skips remote on host miss", async () => {
		process.env.ROCKY_MEMORY_LLM_ENABLED = "true";
		process.env.ROCKY_MEMORY_HOST_LLM_ENABLED = "true";
		process.env.ROCKY_MEMORY_LLM_BASE_URL = "http://remote/v1";
		let calls = 0;
	...
	it("renders host sleep prompt override without chat-template tokens", () => {
		process.env.ROCKY_MEMORY_SLEEP_PROMPT = "Write in German. Source={source}. Memories:\n{memories}";
	...
	it("expands chunk budget when host backend will handle calls", () => {
		process.env.ROCKY_MEMORY_LLM_ENABLED = "true";
		process.env.ROCKY_MEMORY_HOST_LLM_ENABLED = "true";
		process.env.ROCKY_MEMORY_HOST_LLM_N_CTX = "32000";
		process.env.ROCKY_MEMORY_LLM_N_CTX = "2048";
	...
	it("uses a constructor-scoped completion function instead of remote URL settings", async () => {
		process.env.ROCKY_MEMORY_LLM_ENABLED = "true";
		process.env.ROCKY_MEMORY_LLM_BASE_URL = "http://remote.example/v1";
		let fetchCalls = 0;
		const fetchMock: FetchImpl = async () => {
			fetchCalls += 1;
		...
		try {
			const text = await withRockyMemoryRuntimeOptions(memory.runtimeOptions, () =>
	...
	it("uses a constructor-scoped pi-ai Model instance", async () => {
		...
		const memory = new RockyMemory({ llm: model });
		try {
			const text = await withRockyMemoryRuntimeOptions(memory.runtimeOptions, () => complete("hello"));
	...
	it("lets llm:false override remote environment defaults", async () => {
		...
		const memory = new RockyMemory({ llm: false });
		try {
			const text = await withRockyMemoryRuntimeOptions(memory.runtimeOptions, () =>
				complete("hello", 0.3, { fetch: fetchMock }),
			);
