...
export const DEFAULT_DATA_DIR = join(homedir(), ".hermes", "rockyMemory", "data");
export const DEFAULT_DB_FILENAME = "rockyMemory.db";
export const FASTEMBED_CACHE_DIR = join(homedir(), ".hermes", "cache", "fastembed");
export const MODEL_CACHE_DIR = join(homedir(), ".hermes", "rockyMemory", "models");
...
export function dataDir(env: Env = process.env): string {
	return envOptionalString("ROCKY_MEMORY_DATA_DIR", env) ?? DEFAULT_DATA_DIR;
}
...
export function beamOptimizationsEnabled(env: Env = process.env): boolean {
	return envTruthy("ROCKY_MEMORY_BEAM_OPTIMIZATIONS", env);
}

export function embeddingModel(env: Env = process.env): string {
	return envString("ROCKY_MEMORY_EMBEDDING_MODEL", DEFAULT_EMBEDDING_MODEL, env);
}

export function embeddingDim(env: Env = process.env): number {
	const explicit = envInt("ROCKY_MEMORY_EMBEDDING_DIM", NaN, env);
...
export function embeddingsViaApi(env: Env = process.env): boolean {
	return envTruthy("ROCKY_MEMORY_EMBEDDINGS_VIA_API", env);
}

export function embeddingsDisabled(env: Env = process.env): boolean {
	return envString("ROCKY_MEMORY_NO_EMBEDDINGS", "", env) !== "";
}
...
export function embeddingMaxInputChars(env: Env = process.env): number {
	return Math.max(0, envInt("ROCKY_MEMORY_EMBEDDING_MAX_INPUT_CHARS", 8192, env));
}
...
export function isApiEmbeddingModel(model = embeddingModel(), env: Env = process.env): boolean {
	if (model.startsWith("openai/") || model.includes("text-embedding") || model.startsWith("text-embedding"))
		return true;
	const baseUrl = envString("ROCKY_MEMORY_EMBEDDING_API_URL", envString("OPENROUTER_BASE_URL", "", env), env);
...
export function workingMemoryMaxItems(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_WM_MAX_ITEMS", 10000, env);
}

export function workingMemoryTtlHours(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_WM_TTL_HOURS", 24, env);
}

export function episodicRecallLimit(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_EP_LIMIT", 50000, env);
}

export function maxEpisodeChars(env: Env = process.env): number {
	return Math.max(1, envInt("ROCKY_MEMORY_MAX_EPISODE_CHARS", 100000, env));
}

export function sleepBatchSize(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_SLEEP_BATCH", 5000, env);
}

export function scratchpadMaxItems(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_SP_MAX", 1000, env);
}

export function recencyHalflifeHours(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_RECENCY_HALFLIFE", 168, env);
}

export function tier2Days(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_TIER2_DAYS", 30, env);
}

export function tier3Days(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_TIER3_DAYS", 180, env);
}

export function tier1Weight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_TIER1_WEIGHT", 1.0, env);
}

export function tier2Weight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_TIER2_WEIGHT", 0.5, env);
}

export function tier3Weight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_TIER3_WEIGHT", 0.25, env);
}

export function degradeBatchSize(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_DEGRADE_BATCH", 100, env);
}

export function smartCompressEnabled(env: Env = process.env): boolean {
	return !envDisabled("ROCKY_MEMORY_SMART_COMPRESS", env);
}

export function tier3MaxChars(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_TIER3_MAX_CHARS", 300, env);
}

export function statedWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_STATED_WEIGHT", VERACITY_WEIGHT_DEFAULTS.stated, env);
}

export function inferredWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_INFERRED_WEIGHT", VERACITY_WEIGHT_DEFAULTS.inferred, env);
}

export function toolWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_TOOL_WEIGHT", VERACITY_WEIGHT_DEFAULTS.tool, env);
}

export function importedWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_IMPORTED_WEIGHT", VERACITY_WEIGHT_DEFAULTS.imported, env);
}

export function unknownWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_UNKNOWN_WEIGHT", VERACITY_WEIGHT_DEFAULTS.unknown, env);
}

export function veracityWeightOverrides(env: Env = process.env): string[] {
	const names = [
		"ROCKY_MEMORY_STATED_WEIGHT",
		"ROCKY_MEMORY_INFERRED_WEIGHT",
		"ROCKY_MEMORY_TOOL_WEIGHT",
		"ROCKY_MEMORY_IMPORTED_WEIGHT",
		"ROCKY_MEMORY_UNKNOWN_WEIGHT",
	];
...
export function vecType(env: Env = process.env): VecType {
	return envOneOf("ROCKY_MEMORY_VEC_TYPE", ["float32", "int8", "bit"] as const, "int8", env);
}

export function vectorWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_VEC_WEIGHT", 0.5, env);
}

export function ftsWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_FTS_WEIGHT", 0.3, env);
}

export function importanceWeight(env: Env = process.env): number {
	return envFloat("ROCKY_MEMORY_IMPORTANCE_WEIGHT", 0.2, env);
}
...
export function autoMigrateEnabled(env: Env = process.env): boolean {
	return envString("ROCKY_MEMORY_AUTO_MIGRATE", "1", env) !== "0";
}
 ...
 * (e.g. the coding-agent `rockyMemory.polyphonicRecall` / `rockyMemory.enhancedRecall` /
 * `rockyMemory.proactiveLinking` settings) lands here; the `ROCKY_MEMORY_POLYPHONIC_RECALL` /
 * `ROCKY_MEMORY_ENHANCED_RECALL` / `ROCKY_MEMORY_PROACTIVE_LINKING` environment variables still
...
export function llmBaseUrl(env: Env = process.env): string {
	return envString("ROCKY_MEMORY_LLM_BASE_URL", "", env).replace(/\/+$/, "");
}

export function llmApiKey(env: Env = process.env): string {
	return envString("ROCKY_MEMORY_LLM_API_KEY", "", env);
}

export function llmModel(env: Env = process.env): string {
	return envString("ROCKY_MEMORY_LLM_MODEL", "", env);
}

export function hostLlmEnabled(env: Env = process.env): boolean {
	return envBool("ROCKY_MEMORY_HOST_LLM_ENABLED", false, env);
}

export function hostLlmProvider(env: Env = process.env): string | undefined {
	return envOptionalString("ROCKY_MEMORY_HOST_LLM_PROVIDER", env);
}

export function hostLlmModel(env: Env = process.env): string | undefined {
	return envOptionalString("ROCKY_MEMORY_HOST_LLM_MODEL", env);
}

export function hostLlmContext(env: Env = process.env): number {
	return envInt("ROCKY_MEMORY_HOST_LLM_N_CTX", 32000, env);
}

export function sleepPrompt(env: Env = process.env): string {
	return envString("ROCKY_MEMORY_SLEEP_PROMPT", "", env).trim();
}
