...
function resolveBank(args: ToolArguments): string {
	return stringArg(args, "bank") || process.env.ROCKY_MEMORY_MCP_BANK || "default";
}
...
function createBeam(args: ToolArguments, bank = resolveBank(args)): BeamMemory {
	const sessionId = process.env.ROCKY_MEMORY_SESSION_ID || `mcp_${bank}`;
	return new BeamMemory({
		sessionId,
		dbPath: bankDbPath(bank),
		authorId: optionalStringArg(args, "author_id") ?? process.env.ROCKY_MEMORY_AUTHOR_ID ?? null,
		authorType: optionalStringArg(args, "author_type") ?? process.env.ROCKY_MEMORY_AUTHOR_TYPE ?? null,
		channelId: optionalStringArg(args, "channel_id") ?? process.env.ROCKY_MEMORY_CHANNEL_ID ?? sessionId,
	});
}

function sharedBeam(): BeamMemory {
	const configured = process.env.ROCKY_MEMORY_SHARED_SURFACE_DB;
...
const TOOL_HANDLERS: Record<string, Handler> = {
	"rocky-memory_remember": handleRemember,
	"rocky-memory_recall": handleRecall,
	"rocky-memory_shared_remember": handleSharedRemember,
	"rocky-memory_shared_recall": handleSharedRecall,
	"rocky-memory_shared_forget": handleSharedForget,
	"rocky-memory_shared_stats": () => handleSharedStats(),
	"rocky-memory_sleep": handleSleep,
	"rocky-memory_stats": handleStats,
	"rocky-memory_get_stats": handleStats,
	"rocky-memory_invalidate": handleInvalidate,
	"rocky-memory_validate": handleValidate,
	"rocky-memory_get": handleGet,
	"rocky-memory_triple_add": handleTripleAdd,
	"rocky-memory_triple_query": handleTripleQuery,
	"rocky-memory_scratchpad_write": handleScratchpadWrite,
	"rocky-memory_scratchpad_read": handleScratchpadRead,
	"rocky-memory_scratchpad_clear": handleScratchpadClear,
	"rocky-memory_export": handleExport,
	"rocky-memory_update": handleUpdate,
	"rocky-memory_forget": handleForget,
	"rocky-memory_import": handleImport,
	"rocky-memory_diagnose": handleDiagnose,
	"rocky-memory_graph_query": handleGraphQuery,
	"rocky-memory_graph_link": handleGraphLink,
};
