...
import {
	...
	isPiAiModel,
	type RockyMemoryLlmCompleteOptions,
	type RockyMemoryLlmCompletion,
} from "./runtime-options";

const ENV_MODEL_REPO = process.env.ROCKY_MEMORY_LLM_REPO ?? "";
export interface RemoteLlmOptions {
	fetch?: FetchImpl;
}

const ENV_MODEL_FILE = process.env.ROCKY_MEMORY_LLM_FILE ?? "";
...
function activeLlmOptions() {
	return getRockyMemoryRuntimeOptions()?.llm;
}

function activeCustomCompletion(): RockyMemoryLlmCompletion | undefined {
...
function llmEnabled(): boolean {
	...
	return envBool("ROCKY_MEMORY_LLM_ENABLED", true);
}
...
function llmMaxTokens(): number {
	...
	return envInt("ROCKY_MEMORY_LLM_MAX_TOKENS", 2048);
}

function llmContextTokens(): number {
	return envInt("ROCKY_MEMORY_LLM_N_CTX", 2048);
}
...
function hostLlmEnabled(): boolean {
	...
	return envBool("ROCKY_MEMORY_HOST_LLM_ENABLED", false);
}

function hostLlmContextTokens(): number {
	return envInt("ROCKY_MEMORY_HOST_LLM_N_CTX", 32000);
}
...
function llmBaseUrl(): string {
	...
	return stripTrailingSlash(env("ROCKY_MEMORY_LLM_BASE_URL"));
}
...
function llmModelName(): string {
	...
	return env("ROCKY_MEMORY_LLM_MODEL") || "local";
}
...
function llmApiKey(): ApiKey {
	...
	return env("ROCKY_MEMORY_LLM_API_KEY");
}

function sleepPrompt(): string {
	return env("ROCKY_MEMORY_SLEEP_PROMPT").trim();
}
...
function formatSleepPrompt(memories: readonly string[], source = ""): string | null {
	const override = getRockyMemoryRuntimeOptions()?.llm?.consolidationPrompt;
...
async function tryHostLlm(prompt: string, maxTokens: number, temperature: number): Promise<[boolean, string | null]> {
	...
	const raw = await callHostLlm(prompt, {
		...
		temperature,
		timeout: 15,
		provider: env("ROCKY_MEMORY_HOST_LLM_PROVIDER").trim() || null,
		model: env("ROCKY_MEMORY_HOST_LLM_MODEL").trim() || null,
	});
...
export async function callRemoteLlm(
	...
	try {
		...
		const response = await withAuth(llmApiKey(), async key => {
			...
			if (res.status === 401) {
				throw new ProviderHttpError("rockyMemory remote LLM request unauthorized (401)", 401, {
					headers: res.headers,
				});
			}
			return res;
...
async function summarizeChunk(
	...
	if (llmEnabled() && llmBaseUrl() !== "" && !envBool("ROCKY_MEMORY_FORCE_LOCAL", false)) {
