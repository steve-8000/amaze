...
import { getRockyMemoryRuntimeOptions } from "./runtime-options";
...
export const EXTRACTION_PROMPT_TEMPLATE =
	env("ROCKY_MEMORY_EXTRACTION_PROMPT") ||
	`You are an expert structured memory extractor for RockyMemory v3.0+ MEMORIA tables.
...
export function buildExtractionPrompt(text: string, detectedLang = "en"): string {
	const template = getRockyMemoryRuntimeOptions()?.llm?.extractionPrompt ?? EXTRACTION_PROMPT_TEMPLATE;
