...
export const SHMR_BATCH_SIZE = Number.parseInt(process.env.ROCKY_MEMORY_SHMR_BATCH_SIZE ?? "50", 10);
export const SHMR_MAX_ITERATIONS = Number.parseInt(process.env.ROCKY_MEMORY_SHMR_MAX_ITERATIONS ?? "3", 10);
export const SHMR_SIMILARITY_THRESHOLD = Number.parseFloat(
	process.env.ROCKY_MEMORY_SHMR_SIMILARITY_THRESHOLD ?? "0.70",
);
export const SHMR_HARMONY_THRESHOLD = Number.parseFloat(process.env.ROCKY_MEMORY_SHMR_HARMONY_THRESHOLD ?? "0.60");
export const SHMR_MIN_CLUSTER_SIZE = Number.parseInt(process.env.ROCKY_MEMORY_SHMR_MIN_CLUSTER_SIZE ?? "2", 10);
...
export async function embedBatch(texts: readonly string[]): Promise<Vector[]> {
	...
	try {
		...
		logger.debug("rockyMemory shmr embedding failed; using hash fallback", { error: String(error) });
	}
