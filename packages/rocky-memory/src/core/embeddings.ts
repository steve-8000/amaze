...
// (configured via `withRockyMemoryRuntimeOptions`) gets a stable integer id so the cache
// scope reflects the runtime's actual embedding source. Two RockyMemory instances in the
...
function activeEmbeddingOptions() {
	return getRockyMemoryRuntimeOptions()?.embeddings;
}

/**
 * Compose the per-query cache key. Includes the active provider's identity, the
 * resolved model name, and the API base URL so two `RockyMemory` instances in the same
...
export function embeddingsDisabled(): boolean {
	...
	return $flag("ROCKY_MEMORY_NO_EMBEDDINGS");
}
 ...
 * `ROCKY_MEMORY_EMBEDDING_MAX_INPUT_CHARS`, then the bundled `8192` default. `0`
...
function effectiveMaxInputChars(): number {
	...
	const envValue = Number.parseInt($env.ROCKY_MEMORY_EMBEDDING_MAX_INPUT_CHARS ?? "", 10);
...
function capInputs(texts: readonly string[]): readonly string[] {
	...
	logger[rockyMemoryDebugEnabled() ? "warn" : "debug"]("rockyMemory: embedding input truncated", {
...
function embeddingApiKey(): ApiKey {
	...
	return $env.ROCKY_MEMORY_EMBEDDING_API_KEY || $env.OPENROUTER_API_KEY || $env.OPENAI_API_KEY || "";
}
...
function embeddingBaseUrl(): string {
	...
	return $env.ROCKY_MEMORY_EMBEDDING_API_URL || $env.OPENROUTER_BASE_URL || "https://openrouter.ai/api/v1";
}
...
function defaultModel(): string {
	...
	return $env.ROCKY_MEMORY_EMBEDDING_MODEL || "BAAI/bge-small-en-v1.5";
}

/**
 * Resolve the embedding model name for the currently active runtime scope.
 *
 * Reads (in order): the active provider's `model` from `withRockyMemoryRuntimeOptions`,
 * the `ROCKY_MEMORY_EMBEDDING_MODEL` env var, then the bundled fastembed default. Stored
...
export function isApiModel(modelName: string): boolean {
	...
	const active = activeEmbeddingOptions();
	const baseUrl = active?.apiUrl ?? ($env.ROCKY_MEMORY_EMBEDDING_API_URL || $env.OPENROUTER_BASE_URL);
	if (baseUrl !== undefined && baseUrl !== "" && !hostMatchesUrl(baseUrl, "openrouter")) {
		return true;
	}
	return $flag("ROCKY_MEMORY_EMBEDDINGS_VIA_API");
}
...
export function embeddingDimFor(modelName: string): number {
	const override = Number.parseInt($env.ROCKY_MEMORY_EMBEDDING_DIM ?? "", 10);
...
async function getLocalModel(): Promise<LocalEmbeddingModel | null> {
	...
	try {
		...
		logger[rockyMemoryDebugEnabled() ? "warn" : "debug"]("rockyMemory: local embedding model failed to load", {
			model: modelName,
			error: String(error),
		});
...
async function embedApi(texts: readonly string[]): Promise<EmbeddingMatrix | null> {
	...
	try {
		...
		const response = await withAuth(apiKey, async key => {
			...
			if (res.status === 401) {
				throw new ProviderHttpError("rockyMemory embedding request unauthorized (401)", 401, {
					headers: res.headers,
				});
			}
			return res;
		...
		logger.debug("rockyMemory embedding request failed", { status: extractHttpStatusFromError(error) });
		return null;
	}
}
...
export async function embed(texts: readonly string[]): Promise<EmbeddingMatrix | null> {
	...
	try {
		...
		logger[rockyMemoryDebugEnabled() ? "warn" : "debug"]("rockyMemory: local embedding failed", {
