...
export const DEFAULT_PLUGIN_DIR = join(homedir(), ".hermes", "rockyMemory", "plugins");

export type PluginConfig = Record<string, unknown>;
export type MemoryDict = Record<string, unknown>;

export class RockyMemoryPlugin {
	...
	constructor(config: PluginConfig = {}) {
		if (new.target === RockyMemoryPlugin) throw new TypeError("RockyMemoryPlugin is abstract");
		this.config = config;
		const ctor = this.constructor as typeof RockyMemoryPlugin;
		this.name =
...
export class LoggingPlugin extends RockyMemoryPlugin {
...
export class MetricsPlugin extends RockyMemoryPlugin {
...
export class FilterPlugin extends RockyMemoryPlugin {
...
export class CompressionPlugin extends RockyMemoryPlugin {
...
export type PluginConstructor<T extends RockyMemoryPlugin = RockyMemoryPlugin> = new (config?: PluginConfig) => T;

export class PluginManager {
	private readonly registry = new Map<string, PluginConstructor>();
	private readonly instances = new Map<string, RockyMemoryPlugin>();
	...
	registerPlugin(name: string, pluginClass: PluginConstructor): void {
		if (typeof pluginClass !== "function" || !(pluginClass.prototype instanceof RockyMemoryPlugin)) {
			throw new TypeError("pluginClass must be a RockyMemoryPlugin subclass");
		}
		if (this.registry.has(name)) throw new ValueError(`Plugin '${name}' is already registered`);
		this.registry.set(name, pluginClass);
	}
	loadPlugin(name: string, config: PluginConfig = {}): RockyMemoryPlugin {
	...
	getPlugin(name: string): RockyMemoryPlugin | null {
	...
	loadAll(configs: Record<string, PluginConfig> = {}): RockyMemoryPlugin[] {
		const loaded: RockyMemoryPlugin[] = [];
