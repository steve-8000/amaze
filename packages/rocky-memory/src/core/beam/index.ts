...
function autoMigrateAnnotations(db: Database, dbPath: string | undefined): void {
	...
	if (process.env.ROCKY_MEMORY_AUTO_MIGRATE === "0") {
		const row = db
			.query(
			...
			.get() as { count: number };
		console.warn(
			`ROCKY_MEMORY_AUTO_MIGRATE=0: ${row.count} annotation rows pending; run scripts/migrate_triplestore_split.py manually.`,
		);
		return;
	}
