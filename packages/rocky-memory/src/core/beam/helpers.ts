...
export function normalizeWeights(
	...
	let vw = Math.max(0, vecWeight ?? envNumber("ROCKY_MEMORY_VEC_WEIGHT", DEFAULT_WEIGHTS[0]));
	let fw = Math.max(0, ftsWeight ?? envNumber("ROCKY_MEMORY_FTS_WEIGHT", DEFAULT_WEIGHTS[1]));
	let iw = Math.max(0, importanceWeight ?? envNumber("ROCKY_MEMORY_IMPORTANCE_WEIGHT", DEFAULT_WEIGHTS[2]));
...
export function workingMemoryVecSearch(
	...
	if (queryEmbedding.length === 0) return [];
	try {
		const limit = process.env.ROCKY_MEMORY_BEAM_MODE ? 500_000 : 50_000;
		const rows = db
			.query(`
...
async function runEmbedding(beam: BeamMemoryState, items: readonly EmbedItem[]): Promise<void> {
	try {
		...
		logger[rockyMemoryDebugEnabled() ? "warn" : "debug"]("rockyMemory: background embedding failed", {
 ...
 * set by `RockyMemory.#withRuntimeOptions` has already exited by the time the task runs.
 */
export function scheduleEmbedding(beam: BeamMemoryState, items: readonly EmbedItem[]): void {
	...
	const task = withRockyMemoryRuntimeOptions(runtimeOptions, () => runEmbedding(beam, cleaned));
