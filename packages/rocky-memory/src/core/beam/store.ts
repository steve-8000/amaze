...
import { getRockyMemoryRuntimeOptions, withRockyMemoryRuntimeOptions } from "../runtime-options";
...
const SCRATCHPAD_MAX_ITEMS = Number.parseInt(process.env.ROCKY_MEMORY_SP_MAX ?? "1000", 10);
...
function proactiveLinkingAllowed(beam: BeamMemoryState): boolean {
	const override = process.env.ROCKY_MEMORY_PROACTIVE_LINKING;
 ...
 * `RockyMemory.#withRuntimeOptions` has already exited by the time the task runs.
 */
function scheduleFactExtraction(beam: BeamMemoryState, memoryId: string, content: string): void {
	if (content.trim() === "") return;
	const runtimeOptions = getRockyMemoryRuntimeOptions();
	const task = withRockyMemoryRuntimeOptions(runtimeOptions, () => runFactExtraction(beam, memoryId, content));
 ...
 * `ROCKY_MEMORY_NO_EMBEDDINGS` env, or an unresolved (empty) active model — so a
...
export function reconcileEmbeddingModel(beam: BeamMemoryState): void {
	...
	if (mismatch) {
		...
		logger.info("rockyMemory: embedding model changed, rebuilding", {
	...
	logger.info("rockyMemory: resuming interrupted embedding rebuild", { to: active, count: missing.length });
	rebuild(missing);
}
