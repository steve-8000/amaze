...
function checkpointRoot(host: MemoryHost): string {
	...
	return path === undefined || path === ":memory:"
		? join(process.cwd(), ".rockyMemory-sync")
...
export class DeltaSync {
	...
	constructor(
		readonly rockyMemory: MemoryHost,
		checkpointDir?: string,
	) {
		this.db = databaseOf(rockyMemory);
		this.checkpointDir = checkpointDir ?? checkpointRoot(rockyMemory);
