...
import {
	...
	type RockyMemoryEmbeddingRuntimeOptions,
	type RockyMemoryLlmCompletion,
	type RockyMemoryLlmRuntimeOptions,
	resolveEmbeddingProvider,
	withRockyMemoryRuntimeOptions,
} from "./runtime-options";

export interface RockyMemoryOptions {
	...
	readonly embeddings?: false | RockyMemoryEmbeddingRuntimeOptions;
	readonly llmEnabled?: boolean;
	readonly llmBaseUrl?: string;
	readonly llmApiKey?: ApiKey;
	readonly llmModel?: string | Model<Api>;
	readonly llm?: false | RockyMemoryLlmRuntimeOptions | Model<Api> | RockyMemoryLlmCompletion;
...
function hasOwn(options: RockyMemoryOptions, key: keyof RockyMemoryOptions): boolean {
	return Object.hasOwn(options, key);
}

function resolveRuntimeOptions(options: RockyMemoryOptions): ResolvedRockyMemoryRuntimeOptions | undefined {
	...
	let llm: ResolvedRockyMemoryRuntimeOptions["llm"];
...
let defaultInstance: RockyMemory | null = null;
...
function resolveDbPath(options: RockyMemoryOptions, bank: string): string | undefined {
...
function defaultFor(bank: string | null | undefined = null): RockyMemory {
	...
	if (defaultInstance === null || defaultInstance.bank !== targetBank) {
		...
		defaultInstance = new RockyMemory({ bank: targetBank });
	}
	return defaultInstance;
}

export class RockyMemory {
	readonly sessionId: string;
	...
	readonly db: Database;
	readonly runtimeOptions?: ResolvedRockyMemoryRuntimeOptions;
	#ownsDb: boolean;
	#closed = false;

	constructor(options: RockyMemoryOptions = {}) {
	...
	#withRuntimeOptions<T>(fn: () => T): T {
		return withRockyMemoryRuntimeOptions(this.runtimeOptions, fn);
	}
}
...
export function getDefaultInstance(bank: string | null = null): RockyMemory {
