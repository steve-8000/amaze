...
function resolveDbPath(context?: CliContext): string {
	return context?.dbPath ?? (context?.dataDir ? join(context.dataDir, "rockyMemory.db") : configuredDbPath());
}
...
async function withMemory<T>(context: CliContext | undefined, fn: (memory: BeamMemory) => T | Promise<T>): Promise<T> {
	...
	try {
		...
		// short-lived owners (e.g. `rocky-memory store …` / `rocky-memory sleep …`) don't
...
export const cmdExport: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory export <file.json>");
...
export const cmdImport: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory import <file.json>");
	...
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed))
		fail("Import file must contain a rocky-memory export object", 1);
	return withMemory(context, memory => {
...
export const cmdRemember: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory store <content> [source] [importance]");
...
export const cmdRecall: CommandHandler = async (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory recall <query> [top_k]");
	const query = args[0] ?? "";
...
export const cmdUpdate: CommandHandler = (args, context) => {
	if (args.length < 2) usage("Usage: rocky-memory update <memory_id> <new_content> [importance]");
...
export const cmdDelete: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory delete <memory_id>");
	const memoryId = args[0] ?? "";
	return withMemory(context, memory => {
...
export const cmdScratchpad: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory scratchpad <read|write|clear> [content]");
	const subcmd = args[0];
	return withMemory(context, memory => {
		if (subcmd === "read") {
		...
		if (subcmd === "write") {
			if (args.length < 2) usage("Usage: rocky-memory scratchpad write <content>");
...
export const cmdBank: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: rocky-memory bank <list|create|delete> [name]");
	...
	try {
		...
		if (subcmd === "create") {
			if (args.length < 2) fail("Usage: rocky-memory bank create <name>");
			const name = args[1] ?? "";
		...
		if (subcmd === "delete") {
			if (args.length < 2) fail("Usage: rocky-memory bank delete <name>");
			const name = args[1] ?? "";
...
export function printHelp(context?: CliContext): void {
	out(context, "rocky-memory - Local AI Memory System\n");
	out(context, "Usage: rocky-memory <command> [args]\n");
...
export async function runCli(args: readonly string[] = Bun.argv.slice(2), context?: CliContext): Promise<number> {
	...
	if (!handler) {
		...
		err(context, "Run 'rocky-memory --help' for usage.");
		return 2;
	}
	try {
