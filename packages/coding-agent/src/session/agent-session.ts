...
import todoContinuationPrompt from "../prompts/system/todo-continuation.md" with { type: "text" };
...
function buildSessionMetadata(
	...
	if (provider === "anthropic") {
		...
		if (typeof accountUuid === "string" && accountUuid.length > 0) {
			...
			userId.device_id = crypto.createHash("sha256").update(`amaze-device-id-v1:${accountUuid}`).digest("hex");
		}
	}
...
export class AgentSession {
	...
	enqueueCustomMessageDisplay(text: string, mode: "steer" | "followUp"): string {
		const tag = `amaze-cmd-${Date.now()}-${++this.#customDisplayTagCounter}`;
	...
	async #checkTodoCompletion(): Promise<void> {
		...
		const completedCount = phases.flatMap(phase => phase.tasks).filter(task => task.status === "completed").length;
		const totalCount = completedCount + incomplete.length;
		const remainingTodoList = incompleteByPhase
			.map(phase => `- ${phase.name}\n${phase.tasks.map(task => `  - [${task.status}] ${task.content}`).join("\n")}`)
			.join("\n");
		const reminder = prompt.render(todoContinuationPrompt, {
			completedCount,
			totalCount,
