...
function resolveConfiguredEditMode(rawEditMode: string): EditMode | undefined {
	...
	if (!editMode) {
		throw new Error(`Invalid AMAZE_EDIT_VARIANT: ${rawEditMode}`);
	}
...
function resolveAllowFuzzy(session: ToolSession, rawValue: string): boolean {
	switch (rawValue) {
		...
		default:
			throw new Error(`Invalid AMAZE_EDIT_FUZZY: ${rawValue}`);
	}
}
...
function resolveFuzzyThreshold(session: ToolSession, rawValue: string): number {
	...
	if (Number.isNaN(threshold) || threshold < 0 || threshold > 1) {
		throw new Error(`Invalid AMAZE_EDIT_FUZZY_THRESHOLD: ${rawValue}`);
	}
...
export class EditTool implements AgentTool<TInput> {
	...
	readonly summary = "Apply surgical file edits using the configured edit mode";
	...
	constructor(private readonly session: ToolSession) {
		const {
			AMAZE_EDIT_FUZZY: editFuzzy = "auto",
			AMAZE_EDIT_FUZZY_THRESHOLD: editFuzzyThreshold = "auto",
			AMAZE_EDIT_VARIANT: envEditVariant = "auto",
		} = Bun.env;
