/**
 * Amaze extension-package sub-discovery provider.
 *
 ...
 * `rules/`, `prompts/`, and `.mcp.json` — are picked up by amaze's standard
 * discovery surfaces. The native `amaze` provider in `builtin.ts` only walks
 * `.amaze/` and `~/.amaze/agent/`, so without this provider those sub-trees are
 * silently ignored.
 *
 * Provider priority is set below the native `amaze` provider (100) so an
 * extension package never shadows the user's own `.amaze/` configuration on
 * dedup.
 *
 * @see ./amaze-extension-roots.ts
 * @see ../../docs/extension-loading.md
 */
import * as path from "node:path";
import { logger, parseFrontmatter, tryParseJson } from "@steve-z8k/pi-utils";
...
import { type AmazeExtensionRoot, listAmazeExtensionRoots } from "./amaze-extension-roots";
...
const DISPLAY_NAME = "Amaze Extension Packages";
const DESCRIPTION =
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	const roots = await listAmazeExtensionRoots(ctx);
...
async function loadHooks(ctx: LoadContext): Promise<LoadResult<Hook>> {
	const roots = await listAmazeExtensionRoots(ctx);
	const tasks: Array<{ root: AmazeExtensionRoot; hookType: "pre" | "post" }> = [];
...
async function loadMCPServers(ctx: LoadContext): Promise<LoadResult<MCPServer>> {
	...
	const tasks: Array<{ root: AmazeExtensionRoot; mcpPath: string }> = [];
	for (const root of roots) {
	...
	for (let i = 0; i < tasks.length; i++) {
		...
		if (!parsed) {
			warnings.push(`[amaze-plugins] Invalid JSON in ${mcpPath}`);
			logger.warn(`[amaze-plugins] Invalid JSON in ${mcpPath}`);
			continue;
		}
		...
		for (const [serverName, serverCfg] of Object.entries(servers)) {
			...
			if (typeof cfg.command !== "string" && typeof cfg.url !== "string") {
				warnings.push(`[amaze-plugins] Skipping MCP server "${serverName}" in ${mcpPath}: missing command or url`);
				continue;
			}
			items.push({
				name: serverName,
