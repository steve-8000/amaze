 ...
 * `prompts/`, `.mcp.json`) are wired into discovery by `amaze-plugins.ts`.
 *
 * CLI-provided paths are injected via {@link injectAmazeExtensionCliRoots}
 * before discovery runs; settings paths are read lazily from
 * `<scope>/settings.json` in {@link listAmazeExtensionRoots} to mirror what
 * `loadExtensionModules` already does.
 *
 * @see ./amaze-plugins.ts
 * @see ./builtin.ts `loadExtensionModules`
 */
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { getAgentDir, isEnoent, logger, tryParseJson } from "@steve-z8k/pi-utils";
...
export interface AmazeExtensionRoot {
 ...
 * the registered set; {@link clearAmazeExtensionCliRoots} resets for tests.
 */
export function injectAmazeExtensionCliRoots(paths: readonly string[], home: string, cwd: string): void {
...
export function clearAmazeExtensionCliRoots(): void {
	injectedCliRoots = [];
}

/** Inspect currently-injected CLI roots (read-only). Exposed for diagnostics + tests. */
export function getInjectedAmazeExtensionCliRoots(): readonly AmazeExtensionRoot[] {
...
function scopeDirs(ctx: LoadContext): ScopeDirs {
	return {
		project: path.join(ctx.cwd, ".amaze"),
		user: getAgentDir(),
	};
}
 ...
 * are dropped):
 *
 * 1. CLI roots injected via {@link injectAmazeExtensionCliRoots}
 * 2. Project `<cwd>/.amaze/settings.json#extensions`
 * 3. User `~/.amaze/agent/settings.json#extensions`
 * 4. Enabled plugins installed under `<plugins>/node_modules/` (e.g. via
 *    `amaze install <pkg>` / `amaze plugin install` / `amaze plugin link`)
 *
 ...
 */
export async function listAmazeExtensionRoots(ctx: LoadContext): Promise<AmazeExtensionRoot[]> {
	...
	const roots: AmazeExtensionRoot[] = [];
 ...
 *
 * Marketplace and `amaze plugin link` installs write to the plugin manager's
