...
const DISPLAY_NAME = "Amaze";
const DESCRIPTION = "Native Amaze configuration from ~/.amaze and .amaze/";
const PRIORITY = 100;
...
async function getConfigDirs(ctx: LoadContext): Promise<Array<{ dir: string; level: "user" | "project" }>> {
	...
	// profile's agent dir (~/.amaze/profiles/<name>/agent), like sessions and MCP.
...
async function loadSkills(ctx: LoadContext): Promise<LoadResult<Skill>> {
	// Walk up from cwd finding .amaze/skills/ in ancestors (closest first)
	...
	// User-level scan from ~/.amaze/agent/skills/
...
async function loadRules(ctx: LoadContext): Promise<LoadResult<Rule>> {
	...
	// https://amaze/docs/context-files as the file that gets "re-injected near
	// the current turn so they keep hold across long conversations".
	// User scope:    ~/.amaze/agent/RULES.md
	// Project scope: nearest .amaze/RULES.md walking up from cwd to repoRoot
...
registerProvider<ContextFile>(contextFileCapability.id, {
	id: PROVIDER_ID,
	displayName: DISPLAY_NAME,
	description: "Load AGENTS.md from .amaze/ directories",
	priority: PRIORITY,
