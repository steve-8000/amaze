/**
 * Protocol handler for amaze:// URLs.
 *
 * Serves statically embedded documentation files bundled at build time.
 *
 * URL forms:
 * - amaze:// - Lists all available documentation files
 * - amaze://<file>.md - Reads a specific documentation file
 */
...
/**
 * Handler for amaze:// URLs.
 *
 * Resolves documentation file names to their content, or lists available docs.
 */
export class AmazeProtocolHandler implements ProtocolHandler {
	readonly scheme = "amaze";
	...
	async #listDocs(url: InternalUrl): Promise<InternalResource> {
		...
		const listing = EMBEDDED_DOC_FILENAMES.map(f => `- [${f}](amaze://${f})`).join("\n");
	...
	async #readDoc(filename: string, url: InternalUrl): Promise<InternalResource> {
		...
		if (path.isAbsolute(filename)) {
			throw new Error("Absolute paths are not allowed in amaze:// URLs");
		}

		const normalized = path.posix.normalize(filename.replaceAll("\\", "/"));
		if (normalized === ".." || normalized.startsWith("../") || normalized.includes("/../")) {
			throw new Error("Path traversal (..) is not allowed in amaze:// URLs");
		}
		...
		if (content === undefined) {
			...
			const suffix =
				suggestions.length > 0
					...
					: "\nUse amaze:// to list available files.";
