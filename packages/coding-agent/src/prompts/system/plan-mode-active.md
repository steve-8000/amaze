...
<procedure>
1. **Understand** — focus on the request and the code behind it. Launch parallel `finder` subagents (via `task`) when scope spans areas; give each a distinct focus (existing implementations, related components, test patterns). Hunt for reusable code before proposing new.
