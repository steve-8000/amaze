...
5. **Verify after every phase before launching the next.** Run the appropriate gate: `bun check` for types and package-scoped `bun test` for behavior. If a phase introduced breakage, dispatch fix-up subagents *before* moving on. NEVER declare a phase done on a red tree.
...
- Wrapping a single trivial edit (e.g. removing one redundant config line) in a `coder`/`fixer` task with full Goal/Constraints scaffolding — just make the edit inline.
