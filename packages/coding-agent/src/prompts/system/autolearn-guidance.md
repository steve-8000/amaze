...
You can grow a library of reusable **managed skills** with the `manage_skill` tool. Managed skills are Markdown files in Circle's canonical skill store and are surfaced to future sessions like any other skill.
...
- `manage_skill`/`capture_lesson` write only through the Circle skill backend. They do not write Amaze's legacy `~/.amaze/agent/managed-skills` store.
