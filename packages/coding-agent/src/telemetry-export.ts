 ...
 * spawns `amaze` as a child process) had no way to collect amaze's internal traces.
 *
 ...
 * consuming process configures the destination entirely through env; amaze stays
...
async function registerProvider(): Promise<void> {
	...
	const tracerProvider = new NodeTracerProvider({
		resource: resourceFromAttributes({
			"service.name": process.env.OTEL_SERVICE_NAME ?? "amaze-agent",
		}),
