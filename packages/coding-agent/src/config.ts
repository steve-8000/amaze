 ...
 * project with amaze's own assets and was the source of issue #1423 (the host
 * project's `CHANGELOG.md` rendered as amaze's startup changelog).
 */
...
/**
 * Path to amaze's own `CHANGELOG.md`, or `undefined` when the package directory
 ...
 * User-level: ~/.amaze/agent, ~/.claude, ~/.codex, ~/.gemini
 * Project-level: .amaze, .claude, .codex, .gemini
 */
...
export interface ConfigDirEntry {
	path: string;
	source: string; // e.g., ".amaze", ".claude"
	level: "user" | "project";
}

export interface GetConfigDirsOptions {
	/** Include user-level directories (~/.amaze/agent/...). Default: true */
	user?: boolean;
	/** Include project-level directories (.amaze/...). Default: true */
	project?: boolean;
 ...
 * // → [{ path: "~/.amaze/agent/commands", source: ".amaze", level: "user" }, ...]
 *
 * @example
 ...
 * Returns one entry per config base (.amaze, .claude) - the nearest one found.
