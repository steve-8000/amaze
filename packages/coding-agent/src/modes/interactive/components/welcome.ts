...
// --- tui/theme compat shim (local theme framework diverged from upstream) ---
const BOX_ROUND = {
	topLeft: "╭",
	topRight: "╮",
	bottomLeft: "╰",
...
const BOX_SHARP = { teeUp: "┴" } as const;
...
const padding = (n: number): string => " ".repeat(Math.max(0, n));
function styledSymbol(key: string, color: ThemeColor): string {
	return theme.fg(color, SYMBOLS[key] ?? "");
}
...
export class WelcomeComponent implements Component {
	...
	render(termWidth: number): string[] {
		...
		const separator = ` ${theme.fg("dim", BOX_ROUND.horizontal.repeat(separatorWidth))}`;
		...
		if (this.recentSessions.length === 0) {
			...
			const bulletPrefix = ` ${SYMBOLS["md.bullet"]} `;
		...
		const lspLines = this.lspServers.map((server) => {
			const icon =
				server.status === "ready"
					? styledSymbol("status.success", "success")
					: server.status === "connecting"
						? styledSymbol("status.pending", "muted")
						: styledSymbol("status.error", "error");
		...
		const hChar = BOX_ROUND.horizontal;
		const h = theme.fg("dim", hChar);
		const v = theme.fg("dim", BOX_ROUND.vertical);
		const tl = theme.fg("dim", BOX_ROUND.topLeft);
		const tr = theme.fg("dim", BOX_ROUND.topRight);
		const bl = theme.fg("dim", BOX_ROUND.bottomLeft);
		const br = theme.fg("dim", BOX_ROUND.bottomRight);
		...
		// Bottom border
		if (showRightColumn) {
			lines.push(bl + h.repeat(leftCol) + theme.fg("dim", BOX_SHARP.teeUp) + h.repeat(rightCol) + br);
		} else {
...
function gradientLogo(lines: readonly string[], phase = 0): string[] {
	...
	const colorAt = supportsTrueColor()
		? (t: number): string => {
