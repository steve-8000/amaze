...
let BUILTIN_THEME_PATHS: Record<string, string> = {};
...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		const register = (themePath: string, fallbackName: string) => {
			try {
				const json = JSON.parse(fs.readFileSync(themePath, "utf-8")) as ThemeJson;
				const name = json.name || fallbackName;
				if (!themes[name]) {
					themes[name] = json;
					paths[name] = themePath;
				}
			} catch {
		...
		// Core themes first (dark/light take precedence over any duplicate name).
		register(path.join(themesDir, "dark.json"), "dark");
		register(path.join(themesDir, "light.json"), "light");
		...
		if (fs.existsSync(defaultsDir)) {
			for (const file of fs.readdirSync(defaultsDir)) {
				if (file.endsWith(".json")) {
					register(path.join(defaultsDir, file), file.replace(/\.json$/, ""));
				}
			}
		}

		BUILTIN_THEMES = themes;
		BUILTIN_THEME_PATHS = paths;
	}
...
export function getAvailableThemesWithPaths(): ThemeInfo[] {
	...
	for (const name of Object.keys(getBuiltinThemes())) {
		addTheme({ name, path: BUILTIN_THEME_PATHS[name] ?? path.join(themesDir, `${name}.json`) });
	}
...
export function getDefaultTheme(): string {
	// amaze ships a single fixed theme.
	return "erid";
}
...
export function initTheme(themeName?: string, enableWatcher: boolean = false): void {
	...
	try {
		...
		// Theme is invalid - fall back to the fixed amaze theme silently
