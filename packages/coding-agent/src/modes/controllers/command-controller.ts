...
import { CompactionCancelledError, type CompactionOutcome } from "@amaze/agent-core/compaction";
import {
	getEnvApiKey,
	getProviderDetails,
	...
	async handleMemoryCommand(text: string): Promise<void> {
		...
		if (action === "search") {
			...
			showMarkdownPanel(
				...
				runRockeySearch(agentDir, this.ctx.sessionManager.getCwd(), this.ctx.settings, query),
			);
			return;
		}
		...
		if (action === "import") {
			if (backend.id !== "rockey") return this.ctx.showError("Rockey backend is not active for this session.");
			try {
				this.ctx.showStatus(await runRockeyImport(agentDir, this.ctx.sessionManager.getCwd()));
			} catch (error) {
				this.ctx.showError(`Rockey import failed: ${error instanceof Error ? error.message : String(error)}`);
			}
			return;
		}
		...
		if (action === "doctor") {
			...
			showMarkdownPanel(
				this.ctx,
				"Rockey Doctor",
				runRockeyDoctor(agentDir, this.ctx.settings, this.ctx.session.modelRegistry),
			);
			return;
		}

		if (action === "reindex-sessions") {
			if (backend.id !== "rockey") return this.ctx.showError("Rockey backend is not active for this session.");
			try {
				this.ctx.showStatus(await runRockeySessionReindex(agentDir));
			} catch (error) {
				this.ctx.showError(
					`Rockey session reindex failed: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
			return;
		}
