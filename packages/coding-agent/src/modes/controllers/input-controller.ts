...
import { type AgentMessage, ThinkingLevel } from "@amaze/agent-core";
import type { AutocompleteProvider, SlashCommand } from "@amaze/tui";
import { $env, sanitizeText } from "@amaze/utils";
...
export class InputController {
	...
	setupEditorSubmitHandler(): void {
		this.ctx.editor.onSubmit = async (text: string) => {
			...
			if (!hasUserMessages && !this.ctx.sessionManager.getSessionName() && !$env.AMAZE_NO_TITLE) {
	...
	async #invokeSkillCommand(text: string, streamingBehavior: "steer" | "followUp"): Promise<boolean> {
		...
		try {
			...
			void bumpSkillUse(this.ctx.settings.getAgentDir(), details.name);
			if (this.ctx.session.isStreaming) {
				this.ctx.updatePendingMessagesDisplay();
				this.ctx.ui.requestRender();
			}
