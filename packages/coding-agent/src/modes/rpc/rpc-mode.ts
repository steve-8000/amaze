...
import { $env, readJsonl, Snowflake } from "@amaze/utils";
import type {
	ExtensionUIContext,
...
function shouldEmitRpcTitles(): boolean {
	const raw = $env.AMAZE_RPC_EMIT_TITLE;
	if (!raw) return false;
...
export async function runRpcMode(
	...
	// may write there.
	process.env.AMAZE_NOTIFICATIONS = "off";
	...
	class RpcExtensionUIContext implements ExtensionUIContext {
		...
		setTitle(title: string): void {
			// Title updates are low-value noise for most RPC hosts; opt in via AMAZE_RPC_EMIT_TITLE=1.
