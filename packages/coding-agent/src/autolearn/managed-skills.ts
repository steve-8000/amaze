 ...
 * isolated directory (`~/.amaze/agent/managed-skills`) separate from
 * user-authored skills (`~/.amaze/agent/skills`). They are discovered and
...
import { getAgentDir, isEnoent } from "@steve-z8k/pi-utils";
import { YAML } from "bun";

/** Provider id stamped on discovered managed skills (distinguishes them from authored). */
export const MANAGED_SKILLS_PROVIDER_ID = "amaze-managed";
...
/** Resolve the isolated managed-skills directory (`~/.amaze/agent/managed-skills`). */
