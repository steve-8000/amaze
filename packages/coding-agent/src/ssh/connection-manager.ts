...
async function probeHostInfo(host: SSHConnectionTarget): Promise<SSHHostInfo> {
	...
	if (os === "windows" && host.compat !== false) {
		const bashProbe = await runSshCaptureSync(await buildRemoteCommand(host, 'bash -lc "echo AMAZE_BASH_OK"'));
		if (bashProbe.exitCode === 0 && bashProbe.stdout.includes("AMAZE_BASH_OK")) {
			compatShell = "bash";
		} else {
			const shProbe = await runSshCaptureSync(await buildRemoteCommand(host, 'sh -lc "echo AMAZE_SH_OK"'));
			if (shProbe.exitCode === 0 && shProbe.stdout.includes("AMAZE_SH_OK")) {
				compatShell = "sh";
			}
		}
