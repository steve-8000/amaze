/**
 * `amaze install <target>` — top-level convenience over `amaze plugin install` /
 * `amaze plugin link`.
 *
 * The docs (amaze/docs/extension-authoring) advertise
 *
 *   amaze install ./my-extension
 *
...
/**
 * Heuristic used to decide whether `amaze install <target>` should `link` a
...
export default class Install extends Command {
	...
	async run(): Promise<void> {
		...
		if (targets.length === 0) {
			process.stderr.write("Usage: amaze install <path | npm-spec | name@marketplace> [...]\n");
			process.exit(1);
		}
