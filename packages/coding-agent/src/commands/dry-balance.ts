...
export default class DryBalance extends Command {
	...
	static flags = {
		model: Flags.string({ description: "Model selector (same syntax as --model on amaze)" }),
	...
	static examples = [
		...
		"# Dry-run a specific model\n  amaze dry-balance anthropic/claude-sonnet-4-5",
		"# Larger run with bounded concurrency\n  amaze dry-balance --model openai-codex/gpt-5-codex --count 1000 --concurrency 64",
		"# Benchmark every OAuth account in parallel\n  amaze dry-balance --bench",
		"# Machine-readable output\n  amaze dry-balance --json",
	];
