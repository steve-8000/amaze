...
export default class Agents extends Command {
	...
	static flags = {
		...
		user: Flags.boolean({ description: "Write to ~/.amaze/agent/agents (default)" }),
		project: Flags.boolean({ description: "Write to ./.amaze/agents" }),
	};

	static examples = [
		"# Export bundled agents into user config (default)\n  amaze agents unpack",
		"# Export bundled agents into project config\n  amaze agents unpack --project",
		"# Overwrite existing local agent files\n  amaze agents unpack --project --force",
		"# Export into a custom directory\n  amaze agents unpack --dir ./tmp/agents --json",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(Agents);
		if (!args.action) {
			renderCommandHelp("amaze", "agents", Agents);
			return;
		}
