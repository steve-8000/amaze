 ...
 *
 * Override at runtime with the `AMAZE_HL_SEP` env var (e.g.
 * `AMAZE_HL_SEP=">"`, `AMAZE_HL_SEP="\\"`). The value is read once at module load;
...
export const HL_EDIT_SEP = (() => {
	const sep = process.env.AMAZE_HL_SEP?.trim();
