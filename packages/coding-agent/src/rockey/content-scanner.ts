...
export function scanRockeyContent(content: string): string | null {
	...
	for (const { pattern, id } of MEMORY_THREAT_PATTERNS) {
		if (pattern.test(content)) {
			return `Blocked: content matches threat pattern '${id}'. Memory entries may be surfaced through search or prompt recall and must not contain injection or exfiltration payloads.`;
		}
	}
