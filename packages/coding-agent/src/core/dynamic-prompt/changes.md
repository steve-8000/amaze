...
- `build.ts`: Replaced the `"You are a helpful assistant."` opener with a amaze identity section. Added five new reusable sections to the assembled prompt: identity, parallel tool calls, exploration discipline, verification rigor (V1/V2/V3 tiers), and style. Added an optional `tuningSection` field for per-model addenda.
...
- `build.ts`: If upstream rewrites the assembly. Resolution: keep the amaze identity and the new section calls.
...
- All three `test/suite/prompt-presets-*.test.ts` files: Updated assertions to match the new preset names and the amaze-neutral identity.

### Why

- amaze is a neutral coding agent. Persona-named presets collapsed identity into specific personas and made model selection hard to reason about. Naming presets after the model family they target makes the link from `--model` to active preset obvious.
