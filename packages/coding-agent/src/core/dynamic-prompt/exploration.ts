...
- If the location is unknown, call \`context_engine\` once with the task; it will handle internal search, FastContext ranking, and bounded reads.
...
- Do not re-read the same file, overlapping ranges, or the same evidence block unless a new unresolved fact requires it.

Do not manually fan out \`index_status\` + \`search_query\` from the main model; Xenonite Core owns that policy internally.`
		: "";
	return `## Exploration

Use tools whenever they materially improve correctness. Memory of file contents is unreliable; when repository context is needed, use the highest-level repository context tool available before claiming or editing code/text.

Stop searching when the selected context answers the core question, a bounded read proves the needed fact, or \`context_engine\` says \`shouldReadMore\` is false. Launch another read only when synthesis surfaced a concrete missing fact, never as a "just to be sure" sweep. Never repeat the same read against the same file or overlapping range unless the previous result left a specific fact unresolved.${xenoniteFirst}`;
}
