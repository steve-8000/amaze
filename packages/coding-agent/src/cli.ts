 ...
 */
import { type CliConfig, type CommandEntry, run } from "@amaze/utils/cli";
...
const commands: CommandEntry[] = [
	...
	{ name: "evolve", load: () => import("./commands/evolve").then(m => m.default) },
	{ name: "metrics", load: () => import("./commands/metrics").then(m => m.default) },
	{ name: "observe", load: () => import("./commands/observe").then(m => m.default) },
...
async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@amaze/utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker } = await import("@amaze/stats");
