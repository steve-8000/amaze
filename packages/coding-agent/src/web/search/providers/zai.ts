 ...
 */
import { type ApiKey, type AuthStorage, type FetchImpl, getEnvApiKey, withAuth } from "@steve-z8k/pi-ai";
...
export async function searchZai(params: ZaiSearchParams): Promise<SearchResponse> {
	...
	const rawResult = await withAuth(keyOrResolver, key => callZaiSearch(key, params), {
		...
		missingKeyMessage: "Z.AI credentials not found. Set ZAI_API_KEY or login with 'amaze /login zai'.",
	});
