...
export const SETTINGS_SCHEMA = {
	...
	// Env (`AMAZE_AUTH_BROKER_URL` / `AMAZE_AUTH_BROKER_TOKEN`) takes precedence so
	...
	"prompt.mainContextMode": {
		type: "enum",
		values: ["full", "compact"] as const,
		default: "compact",
		ui: {
			tab: "context",
			label: "Main Prompt Context",
			description: "How much project context the top-level orchestrator loads at startup",
			options: [
				{
					value: "compact",
					label: "Compact",
					description: "Keep the nearest context file, hide workspace tree and skill listings",
				},
				{
					value: "full",
					label: "Full",
					description: "Load full project context into the main orchestrator prompt",
				},
			],
		},
	},
	...
	"prompt.cache.orchestratorRetention": {
		...
		ui: {
			tab: "context",
			label: "Orchestrator Prompt Cache",
			description:
				"Prompt-cache retention for the top-level orchestrator. Default follows the provider/global policy (currently long unless AMAZE_CACHE_RETENTION=short).",
			options: [
				{
					value: "default",
				...
				{ value: "long", label: "Long", description: "Prefer long-lived prompt cache entries" },
				{ value: "short", label: "Short", description: "Prefer lower cache-write premium" },
				{ value: "none", label: "None", description: "Disable provider prompt caching" },
			],
		},
	},

	"prompt.cache.subagentRetention": {
		type: "enum",
		values: ["default", "long", "short", "none"] as const,
		default: "short",
		ui: {
			...
			options: [
				...
				{ value: "long", label: "Long", description: "Use long-lived entries for reusable subagent runs" },
				{
					value: "default",
					label: "Provider default",
					description: "Use the provider/global default retention policy",
	...
	"compaction.idleThresholdTokens": {
		...
		ui: {
			...
			options: [
				{ value: "100000", label: "100K tokens", description: "Default" },
	...
	"dev.autoqaPush.endpoint": {
		type: "string",
		// Bundled QA collector — runs `/work/pi-www/autoqa` behind qa.amaze.sh.
		// Override via `PI_AUTO_QA_PUSH_URL` or `dev.autoqaPush.endpoint`
		// in `config.yml` to point at a self-hosted instance.
		default: "https://qa.amaze.sh/v1/grievances" as const,
		ui: {
			tab: "tools",
			label: "Auto QA Push Endpoint",
			description: "Full URL that receives the JSON payload (default ships to https://qa.amaze.sh/v1/grievances)",
		},
	},
