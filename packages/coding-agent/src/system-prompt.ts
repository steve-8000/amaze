...
export type ProjectContextMode = "full" | "compact";
...
export async function buildSystemPrompt(options: BuildSystemPromptOptions = {}): Promise<BuildSystemPromptResult> {
	...
	const agentsMdFiles = Array.from(new Set(workspaceTree.agentsMdFiles))
		.sort()
		.slice(0, projectContextMode === "compact" ? Math.min(AGENTS_MD_LIMIT, 20) : AGENTS_MD_LIMIT);
	...
	const effectiveContextFiles = projectContextMode === "compact" ? contextFiles.slice(-1) : contextFiles;
	const effectiveSkills = projectContextMode === "compact" ? [] : filteredSkills;
	const effectiveWorkspaceTree =
	...
	const data = {
		...
		workspaceTree: effectiveWorkspaceTree,
