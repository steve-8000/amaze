...
} from "@amaze/ai/providers/openai-codex-responses";
 ...
 *
 * Broker mode: when `AMAZE_AUTH_BROKER_URL` is set, credentials are pulled from
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		// Discover and load custom tools from .amaze/tools/, .claude/tools/, etc.
		...
		const intentField =
			settings.get("tools.intentTracing") || $flag("AMAZE_INTENT_TRACING") ? INTENT_FIELD : undefined;
		...
		agent = new Agent({
			...
			cacheRetention: promptCachePolicy.cacheRetention,
