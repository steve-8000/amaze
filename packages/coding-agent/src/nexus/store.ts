...
`)
	...
	/**
	 * Active memory entries whose embedding column is null. Pipeline workers
	 * pick up this list, batch-embed via the embedding client, and write back
	 * with `addEmbedding`. The cap is small by design — embeddings backfill
	 * runs as a background-style budgeted job.
	 */
	listMissingEmbeddings(limit: number): Array<{ id: string; content: string }> {
		const rows = this.#dbInstance
			.prepare("SELECT id, content FROM memory_items WHERE status = 'active' AND embedding IS NULL ORDER BY updated_at DESC LIMIT ?")
			.all(Math.max(1, Math.min(500, limit))) as Array<{ id: string; content: string }>;
		return rows;
	}
