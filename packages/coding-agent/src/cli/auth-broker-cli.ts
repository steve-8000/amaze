/**
 * `amaze auth-broker` command handlers.
 *
 * Sub-verbs:
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// skip the rotating config-dir log file the TUI default would have used.
...
async function runLogin(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!providerArg) {
		throw new Error(`Usage: ${APP_NAME} auth-broker login <provider> [--via=user@host]`);
	}
...
async function runLocalLogin(provider: OAuthProvider): Promise<void> {
	// Spawn the ai CLI in-process — it handles the per-provider OAuth dance
	...
	if (exitCode !== 0) {
		throw new Error(`ai login exited with code ${exitCode}`);
	}
}
...
async function runLogout(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	const providerArg = flags.provider;
	if (!providerArg) {
		throw new Error(`Usage: ${APP_NAME} auth-broker logout <provider>`);
	}
...
/**
 * Maps the `type` field of a CLIProxyAPI credential JSON to the amaze provider id.
...
async function loadImportPlan(
	...
	for (const file of files) {
		...
		if (!provider) {
			skipped.push({
				file,
				reason: `cannot determine amaze provider from type=${json.type ?? "?"} (pass --provider to override)`,
			});
			continue;
		}
...
async function runMigrate(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!brokerConfig) {
		throw new Error(
			"AMAZE_AUTH_BROKER_URL must be set (or `auth.broker.url` in config.yml). `migrate` uploads local credentials to a configured broker.",
		);
	}
...
async function runStatus(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	if (!cfg) {
		const message = "No auth-broker configured (set AMAZE_AUTH_BROKER_URL to enable).";
