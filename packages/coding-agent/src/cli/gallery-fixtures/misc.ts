...
export const miscFixtures: Record<string, GalleryFixture> = {
	...
	ssh: {
		...
		args: {
			host: "deploy@web-01",
			command: "systemctl status amaze-api --no-pager | head -n 12",
			cwd: "/srv/amaze",
			timeout: 60,
		},
		result: {
			content: [
				{
					type: "text",
					text: [
						"● amaze-api.service - Amaze Agent API",
						"     Loaded: loaded (/etc/systemd/system/amaze-api.service; enabled)",
						"     Active: active (running) since Sat 2026-06-06 09:14:02 UTC; 3h 21min ago",
						"   Main PID: 4812 (bun)",
						"      Tasks: 17 (limit: 4915)",
						"     Memory: 142.6M",
						"        CPU: 38.214s",
						"     CGroup: /system.slice/amaze-api.service",
	...
	report_tool_issue: {
		...
		args: {
			tool: "bash",
			report: "Command preview rendered stale arguments during streaming",
		},
