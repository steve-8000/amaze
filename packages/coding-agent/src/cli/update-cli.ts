...
const REPO = "can1357/amaze";
const PACKAGE = "@amaze/coding-agent";
...
function isPathInDirectory(filePath: string, directoryPath: string): boolean {
	...
	// PATH-resolved app path can refer to the same directory through different
...
function resolveUpdateMethod(amazePath: string, bunBinDir: string | undefined): "bun" | "binary" {
	if (!bunBinDir) return "binary";
	return isPathInDirectory(amazePath, bunBinDir) ? "bun" : "binary";
}

export function resolveUpdateMethodForTest(amazePath: string, bunBinDir: string | undefined): "bun" | "binary" {
	return resolveUpdateMethod(amazePath, bunBinDir);
}
async function resolveUpdateTarget(): Promise<UpdateTarget> {
	...
	if (amazePath) {
		const method = resolveUpdateMethod(amazePath, bunBinDir);
		if (method === "bun") return { method };
		return { method, path: amazePath };
	}
...
/**
 * Resolve the path that the app command maps to in the user's PATH.
 */
...
/**
 * Run the resolved app binary and check if it reports the expected version.
 */
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(chalk.yellow(`You may need to reinstall ${APP_NAME}.`));
}
...
export async function runUpdateCommand(opts: { force: boolean; check: boolean }): Promise<void> {
	...
	// Choose update method based on the prioritized app binary in PATH
	try {
