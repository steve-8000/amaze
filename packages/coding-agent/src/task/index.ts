...
export function resolveAgentThinkingLevelOverride(
	agentName: string,
	settings: import("../config/settings").Settings,
	fallback: AgentDefinition["thinkingLevel"],
...
export class TaskTool implements AgentTool<TaskToolSchemaInstance, TaskToolDetails, Theme> {
	...
	async #executeSync(
		...
		const thinkingLevelOverride = resolveAgentThinkingLevelOverride(
			agentName,
		...
		try {
				...
				try {
					...
					if (mergeMode === "branch" && result.exitCode === 0) {
						try {
							...
							const branchName = `amaze/task/${task.id}`;
			...
			const summaries = results.map(r => {
				...
				const yieldItems = r.extractedToolData?.yield as Array<{ memoryNote?: string }> | undefined;
				const memoryNote = yieldItems?.map(item => item.memoryNote?.trim()).find(note => note && note.length > 0);
				let preview = output;
				let truncated = false;
