...
import { $env, getProjectDir, logger, normalizePathForComparison, postmortem, VERSION } from "@amaze/utils";
...
async function checkForNewVersion(currentVersion: string): Promise<string | undefined> {
	...
	try {
		const response = await fetch("https://registry.npmjs.org/@amaze/coding-agent/latest");
...
export async function runRootCommand(
	...
	const smolModel = parsedArgs.smol ?? $env.AMAZE_SMOL_MODEL;
	const slowModel = parsedArgs.slow ?? $env.AMAZE_SLOW_MODEL;
