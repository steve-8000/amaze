...
import { XSearchDeepTool, XSearchTool } from "./x-search";
...
/** Tool type (AgentTool from @amaze/ai) */
...
export async function createTools(session: ToolSession, toolNames?: string[]): Promise<Tool[]> {
	...
	const isToolAllowed = (name: string) => {
		...
		if (name === "todo_write" || name === "todo_read") return !includeYield && session.settings.get("todo.enabled");
