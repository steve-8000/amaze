...
import type { ImageContent, TextContent } from "@amaze/ai";
import { glob, type SummaryResult, summarizeCode } from "@amaze/natives";
import type { Component } from "@amaze/tui";
import { Text } from "@amaze/tui";
import { getRemoteDir, logger, prompt, readImageMetadata, untilAborted } from "@amaze/utils";
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	async execute(
		...
		const semicolon = readPath.indexOf(";");
		if (semicolon >= 0 && !readPath.includes("?")) {
			const next = readPath.slice(semicolon + 1).trimStart();
			if (next.includes("/") || next.includes("\\") || next.includes("://")) {
				throw new ToolError("Read accepts exactly one target per call. Call read once per file/URL.");
			}
		}
