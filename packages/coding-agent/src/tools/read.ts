...
import { getAutoGeneratedMarker } from "./auto-generated-guard";
import {
	type ConflictEntry,
	type ConflictScope,
...
export class ReadTool implements AgentTool<typeof readSchema, ReadToolDetails> {
	...
	async execute(
		...
		// Non-fatal notice for auto-generated files: reading them is legitimate, but the agent
		// should not paginate through (potentially huge, e.g. 15k-line) generated output 300 lines
		// at a time. Surface the marker so it reads the source/generator config instead. Unlike
		// assertEditableFile this never throws — read stays unblocked. Marker lookup is LRU-cached.
		const autoGeneratedMarker = await getAutoGeneratedMarker(absolutePath).catch(() => undefined);
		if (autoGeneratedMarker) {
			const notice = `[Auto-generated file (marker: "${autoGeneratedMarker}"). Avoid paginating the whole file; edit the source/generator config instead.]`;
			const firstText = content.find((c): c is TextContent => c.type === "text");
			if (firstText) {
				firstText.text = `${notice}\n${firstText.text}`;
			} else {
