#!/usr/bin/env python3
"""Amaze built-in CUA daemon.

Speaks newline-delimited JSON-RPC on stdin/stdout. Startup emits:
{type:"ready", version, cuaAvailable, cuaVersion, cuaImportError}.

The daemon starts even when the Python `cua` package is unavailable so the
TypeScript tool can report an actionable install error.
"""
...
class Daemon:
	...
	async def handle_screenshot(self, params: dict[str, Any]) -> dict[str, Any]:
		...
		return {"png_b64": base64.b64encode(png_bytes).decode("ascii"), "width": int(width or 0), "height": int(height or 0)}
	...
	async def run(self) -> None:
		...
		_emit({"type": "ready", "version": DAEMON_VERSION, "cuaAvailable": _CUA_AVAILABLE, "cuaVersion": _CUA_VERSION, "cuaImportError": _CUA_IMPORT_ERROR})
		...
		while not self._stop.is_set():
			...
			try:
				...
				_emit({"id": 0, "error": {"code": -32700, "message": f"Parse error: {error}"}})
				continue
	...
	async def _handle_request(self, request_id: int, request: dict[str, Any]) -> None:
		try:
			...
			_emit({"id": request_id, "error": {"code": -32603, "message": f"{type(error).__name__}: {error}", "data": traceback.format_exc()}})
