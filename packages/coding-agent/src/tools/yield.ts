...
import type { TSchema } from "@amaze/ai/types";
import {
...
function wrapYieldParameters(dataSchema: Record<string, unknown>): Record<string, unknown> {
	return {
		...
		properties: {
			...
			memory_note: {
				type: "string",
				description: "optional one-sentence note for the orchestrator about durable information worth saving",
			},
		},
		required: ["result"],
	};
...
export class YieldTool implements AgentTool<TSchema, YieldDetails> {
	...
	async execute(
		...
		const memoryNoteRaw = raw.memory_note;
		const memoryNote =
			typeof memoryNoteRaw === "string" && memoryNoteRaw.trim().length > 0 ? memoryNoteRaw.trim() : undefined;
		...
		return {
			...
			details: { data, status, error: errorMessage, memoryNote },
		};
	}
}
...
subprocessToolRegistry.register<YieldDetails>("yield", {
	extractData: event => {
		...
		return {
			...
			memoryNote: typeof record.memoryNote === "string" ? record.memoryNote : undefined,
		};
	},
