...
import { type AstReplaceChange, type AstReplaceFileChange, astEdit } from "@amaze/natives";
import type { Component } from "@amaze/tui";
import { Text } from "@amaze/tui";
import { $envpos, prompt, untilAborted } from "@amaze/utils";
...
export class AstEditTool implements AgentTool<typeof astEditSchema, AstEditToolDetails> {
	...
	async execute(
		...
		return untilAborted(signal, async () => {
			...
			const maxFiles = $envpos("AMAZE_MAX_AST_FILES", 1000);
