 ...
 *
 * Enabled by default; gated behind AMAZE_AUTO_QA=1 / `dev.autoqa` so a user
 ...
 * bundled endpoint (`dev.autoqaPush.endpoint`, default `qa.amaze.sh`). Each
 * insert schedules a background flush that POSTs pending rows and deletes
 * them on HTTP 2xx. `AMAZE_AUTO_QA_PUSH=1` forces push in non-interactive
...
import { $env, $flag, getAgentDir, getInstallId, logger, VERSION } from "@amaze/utils";
...
export function isAutoQaEnabled(settings?: Settings): boolean {
	return $flag("AMAZE_AUTO_QA") || !!settings?.get("dev.autoqa");
}
 ...
 * `~/.amaze/agent/autoqa.db`. Idempotently runs schema creation, the
 * `pushed`-column migration, and index setup so every consumer — tool
 * execute path, manual `amaze grievances push`, future debug scripts —
 ...
 *
 * Exported because the `amaze grievances` CLI handlers need the migrated
...
/**
 * Optional per-flush controls. Used by `amaze grievances push` to surface
...
function resolvePushConfig(settings: Settings | undefined, bypassConsent: boolean): PushConfig | null {
	...
	// covers explicit user-driven pushes (`amaze grievances push`) where the
	// user clearly intends to ship regardless of dialog state. The
	// `AMAZE_AUTO_QA_PUSH` env flag stays as a CI/headless override too.
	if (!bypassConsent) {
		const consented = settings?.get("dev.autoqa.consent") === "granted";
		if (!consented && !$flag("AMAZE_AUTO_QA_PUSH")) return null;
	}

	const endpoint = envOverrideString("AMAZE_AUTO_QA_PUSH_URL") ?? settings?.get("dev.autoqaPush.endpoint");
	if (!endpoint || endpoint.trim().length === 0) return null;

	const token = envOverrideString("AMAZE_AUTO_QA_PUSH_TOKEN") ?? settings?.get("dev.autoqaPush.token");
...
async function performFlush(db: Database, config: PushConfig, options: FlushOptions = {}): Promise<FlushResult> {
	...
	for (;;) {
		...
		const body = JSON.stringify({
			agent: { name: "amaze", version: VERSION },
...
export function createReportToolIssueTool(session: ToolSession, activeBuiltinNames: readonly string[] = []): AgentTool {
	...
	return {
		...
		async execute(_toolCallId, rawParams) {
			...
			// at ~/.amaze/agent/autoqa.db regardless of consent — they always
			// own their local data and can inspect or wipe it via `amaze grievances`.
			...
			try {
				...
				if (db) {
					...
					//      `amaze grievances push` or a future consent change.
