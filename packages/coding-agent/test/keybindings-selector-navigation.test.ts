...
import { ExtensionList } from "@steve-z8k/pi-coding-agent/modes/components/extensions/extension-list";
import type { Extension } from "@steve-z8k/pi-coding-agent/modes/components/extensions/types";
import { HistorySearchComponent } from "@steve-z8k/pi-coding-agent/modes/components/history-search";
...
import type { SessionTreeNode } from "@steve-z8k/pi-coding-agent/session/session-entries";
...
async function createHistoryStorage(prompts: string[]): Promise<HistoryStorage> {
	const dir = TempDir.createSync("@amaze-history-nav-");
	tempDirs.push(dir);
	HistoryStorage.resetInstance();
