...
import { AcpAgent } from "@steve-z8k/pi-coding-agent/modes/acp/acp-agent";
import { ACP_TERMINAL_AUTH_FLAG, prepareAcpTerminalAuthArgs } from "@steve-z8k/pi-coding-agent/modes/acp/terminal-auth";
import type { AgentSession } from "@steve-z8k/pi-coding-agent/session/agent-session";
import { SessionManager } from "@steve-z8k/pi-coding-agent/session/session-manager";
import { getConfigRootDir, setAgentDir, VERSION } from "@steve-z8k/pi-utils";
...
async function createAgent(): Promise<AcpAgent> {
	const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "amaze-acp-init-"));
