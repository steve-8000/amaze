...
import { generateThemeVars } from "@amaze/pi-coding-agent/export/html";
import { defaultThemes } from "@amaze/pi-coding-agent/modes/theme/defaults";
import { getResolvedThemeColors, getThemeByName, isLightTheme } from "@amaze/pi-coding-agent/modes/theme/theme";
import { getAgentDir, getCustomThemesDir, setAgentDir } from "@amaze/pi-utils";
...
describe("getResolvedThemeColors HTML export defaults", () => {
	...
	it("uses light text when a light-status custom theme derives dark export surfaces from userMessageBg", async () => {
		...
		tempAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "amaze-theme-export-"));
