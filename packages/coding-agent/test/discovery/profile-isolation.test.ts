/**
 * Regression: Amaze-native user-level config discovery must follow the active
 * profile. A profile relocates the agent directory to ~/.amaze/profiles/<name>/agent;
 * the native provider used to read user config (commands, skills, rules, etc.)
 * from the literal home (~/.amaze/agent) via `ctx.home`, leaking the default
 ...
 * (ctx.home + ".amaze/agent") points at the tempdir decoys below; without the fix
...
import { type Skill, skillCapability } from "@steve-z8k/pi-coding-agent/capability/skill";
...
describe("native user-level config discovery follows the active profile", () => {
	...
	beforeEach(async () => {
		...
		tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "amaze-profile-iso-home-"));
		projectDir = await fs.mkdtemp(path.join(os.tmpdir(), "amaze-profile-iso-project-"));
		profileAgentDir = await fs.mkdtemp(path.join(os.tmpdir(), "amaze-profile-iso-agent-"));
