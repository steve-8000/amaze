 ...
 *
 * The native `amaze` discovery provider only walks `.amaze/` and `~/.amaze/agent/`.
 * Extension packages registered via `extensions:` in settings or
 * `--extension` on the CLI ship their own `skills/`, `hooks/`, `tools/`,
 * `commands/`, `rules/`, `prompts/`, and `.mcp.json`. The `amaze-plugins`
 * provider (`src/discovery/amaze-plugins.ts`) is what wires those sub-trees
...
import { hookCapability } from "@steve-z8k/pi-coding-agent/capability/hook";
import { mcpCapability } from "@steve-z8k/pi-coding-agent/capability/mcp";
import { promptCapability } from "@steve-z8k/pi-coding-agent/capability/prompt";
import { ruleCapability } from "@steve-z8k/pi-coding-agent/capability/rule";
import { skillCapability } from "@steve-z8k/pi-coding-agent/capability/skill";
import { slashCommandCapability } from "@steve-z8k/pi-coding-agent/capability/slash-command";
import { toolCapability } from "@steve-z8k/pi-coding-agent/capability/tool";
import type { LoadContext, Provider } from "@steve-z8k/pi-coding-agent/capability/types";
// Register all discovery providers as a side effect.
import "@steve-z8k/pi-coding-agent/discovery";
import {
	clearAmazeExtensionCliRoots,
	injectAmazeExtensionCliRoots,
} from "@steve-z8k/pi-coding-agent/discovery/amaze-extension-roots";
import { getConfigRootDir, setAgentDir } from "@steve-z8k/pi-utils";

const PROVIDER_ID = "amaze-plugins";
...
function buildExtensionPackage(packageDir: string): void {
	writeFile(
		...
		JSON.stringify({ name: path.basename(packageDir), amaze: { extensions: ["./src/main.ts"] } }),
	);
...
beforeEach(() => {
	clearCache();
	clearAmazeExtensionCliRoots();
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-plugins-"));
...
test("project settings.json#extensions surfaces every sub-directory", async () => {
	writeFile(path.join(project, ".amaze", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("user settings.json#extensions also feeds sub-discovery", async () => {
	writeFile(path.join(home, ".amaze", "agent", "settings.json"), JSON.stringify({ extensions: [ext] }));
...
test("`--extension` CLI injection is wired through the same provider", async () => {
	...
	injectAmazeExtensionCliRoots([ext], home, project);
...
test("file-extension entrypoints contribute zero sub-surface (the file has no siblings to scan)", async () => {
	...
	writeFile(path.join(project, ".amaze", "settings.json"), JSON.stringify({ extensions: [standaloneFile] }));
...
test("relative paths in settings resolve against the project cwd", async () => {
	...
	writeFile(path.join(project, ".amaze", "settings.json"), JSON.stringify({ extensions: [`./${relative}`] }));
...
test("installed plugins under `<plugins>/node_modules/` are surfaced (e.g. via `amaze plugin link`/`install`)", async () => {
	...
	const pluginsDir = path.join(home, ".amaze", "plugins");
	...
	writeFile(
		...
		JSON.stringify({ name: "amaze-plugins", dependencies: { "my-installed-ext": "1.0.0" } }),
	);
	// Plugin's own package.json must carry an `amaze`/`pi` manifest for the
	// loader to recognise it; the buildExtensionPackage fixture already wrote
	// one with `amaze.extensions`, which is sufficient.
...
test("disabled installed plugins do not contribute sub-discovery", async () => {
	...
	writeFile(
		path.join(pluginsDir, "package.json"),
		JSON.stringify({ name: "amaze-plugins", dependencies: { "my-disabled-ext": "1.0.0" } }),
	);
	writeFile(
		path.join(pluginsDir, "amaze-plugins.lock.json"),
...
test("linked plugins (only in lockfile, not in package.json#dependencies) are surfaced", async () => {
	// `amaze plugin link ./local-ext` creates a symlink under
	...
	// still find the package — otherwise the documented `amaze install
