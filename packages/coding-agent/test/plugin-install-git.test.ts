 ...
 *
 * Strategy: spy on the six `@steve-z8k/pi-utils` plugin-path getters so the
...
describe("PluginManager.install with git sources", () => {
	...
	beforeEach(async () => {
		tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "amaze-plugin-git-"));
	...
	test("installs from github: shorthand and resolves real package name from deps diff", async () => {
		...
		await Bun.write(
			pluginsPkgJson,
			JSON.stringify({ name: "amaze-plugins", private: true, dependencies: {} }, null, 2),
		);
