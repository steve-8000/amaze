...
import type { AgentTelemetryConfig, Tracer } from "@amaze/agent-core";
import { type AssistantMessage, Effort } from "@amaze/ai";
...
describe("runSubprocess yield reminders", () => {
	...
	it("disables eager task orchestration inside subagents", async () => {
		const session = createMockSession(({ emit }) => {
			emit({
				type: "tool_execution_end",
				toolCallId: "tool-subagent-settings",
				toolName: "yield",
				result: {
					content: [{ type: "text", text: "Result submitted." }],
					details: { status: "success", data: { ok: true } },
	...
	it("disables tool discovery inside subagents so generic workers keep the full tool set", async () => {
		const session = createMockSession(({ emit }) => {
			emit({
				type: "tool_execution_end",
				toolCallId: "tool-subagent-discovery-off",
				toolName: "yield",
				result: {
					content: [{ type: "text", text: "Result submitted." }],
					details: { status: "success", data: { ok: true } },
		...
		await runSubprocess({
			...baseOptions,
			id: "subagent-discovery-off",
			settings: parentSettings,
		});

		expect(parentSettings.get("tools.discoveryMode")).toBe("all");
		const call = createAgentSessionSpy.mock.calls[0]?.[0];
		if (!call?.settings) throw new Error("Expected subagent settings");
