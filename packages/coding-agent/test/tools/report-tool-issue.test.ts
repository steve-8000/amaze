...
import {
	...
	flushGrievances,
	isAutoQaEnabled,
} from "@steve-z8k/pi-coding-agent/tools/report-tool-issue";
...
describe("flushGrievances", () => {
	...
	it("posts pending rows with bearer header and marks them pushed=1 on 200", async () => {
		...
		expect(body.agent?.name).toBe("amaze");
		...
		// stays so users can browse what they've shipped via `amaze grievances`.
