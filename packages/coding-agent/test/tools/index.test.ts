...
import { type SettingPath, Settings } from "@amaze/coding-agent/config/settings";
import { createTools, HIDDEN_TOOLS, type ToolSession } from "@amaze/coding-agent/tools";
...
describe("createTools", () => {
	...
	it("still exposes eval when python kernel is unavailable (dispatches to js)", async () => {
		const session = createTestSession();
		vi.spyOn(await import("@amaze/coding-agent/eval/py/kernel"), "checkPythonKernelAvailability").mockResolvedValue({
			ok: false,
