...
describe("searchCodex model selection", () => {
	...
	afterEach(() => {
		...
		if (originalCodexSearchModel === undefined) {
			delete process.env.AMAZE_CODEX_WEB_SEARCH_MODEL;
		} else {
	...
	it("falls back to the default model when AMAZE_CODEX_WEB_SEARCH_MODEL is blank", async () => {
		process.env.AMAZE_CODEX_WEB_SEARCH_MODEL = "   ";
		using _hook = mockCodexFetch("gpt-5-codex-mini");
	...
	it("uses AMAZE_CODEX_WEB_SEARCH_MODEL when provided", async () => {
		process.env.AMAZE_CODEX_WEB_SEARCH_MODEL = "gpt-5.4-mini";
	...
	it("forces web_search tool choice and extracts markdown link citations when annotations are absent", async () => {
		process.env.AMAZE_CODEX_WEB_SEARCH_MODEL = "gpt-5.4";
