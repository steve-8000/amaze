...
import type { Tool, ToolCall } from "@amaze/ai/types";
import { enforceStrictSchema } from "@amaze/ai/utils/schema";
import { validateToolArguments } from "@amaze/ai/utils/validation";
...
describe("YieldTool", () => {
	...
	it("accepts an optional memory_note alongside success data", async () => {
		const tool = new YieldTool(createSession());
		const result = await tool.execute("call-memory-note", {
			result: { data: { ok: true } },
			memory_note: "User prefers concise changelog entries.",
		} as never);
		expect(result.details).toEqual({
			data: { ok: true },
			status: "success",
			error: undefined,
			memoryNote: "User prefers concise changelog entries.",
		});
	});
