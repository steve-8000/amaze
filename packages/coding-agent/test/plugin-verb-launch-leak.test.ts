/**
 * Regression test for #2935: the plugins docs advertise `amaze list` / `amaze remove`
 * as top-level commands, but only `amaze install` is registered. Before the fix,
 * `resolveCliArgv(["list"])` rewrote the bare verb to `["launch", "list"]`, so
 * `amaze list` silently started an interactive agent session with "list" as the
 * initial LLM prompt instead of managing plugins (the real command is
 * `amaze plugin list`). Same footgun for `amaze remove`.
 *
 * These tests pin the chosen bugfix: a bare, single-arg documented plugin verb
 * yields a helpful hint pointing at the real `amaze plugin <action>` command
...
describe("documented-but-unregistered plugin verbs do not leak to launch (#2935)", () => {
	test("bare `amaze list` hints at `amaze plugin list` instead of launching with 'list' as the prompt", () => {
		...
		expect("error" in result && result.error).toContain("amaze plugin list");
	});

	test("bare `amaze remove` hints at `amaze plugin uninstall` instead of launching with 'remove' as the prompt", () => {
		const result = resolveCliArgv(["remove"]);
		expect(result).not.toEqual({ argv: ["launch", "remove"] });
		expect(result).not.toHaveProperty("argv");
		expect(result).toHaveProperty("error");
		expect("error" in result && result.error).toContain("amaze plugin uninstall");
	});
