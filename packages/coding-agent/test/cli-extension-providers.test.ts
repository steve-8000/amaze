 ...
 *
 * One-shot CLIs (`amaze bench`, dry-balance) build a bare `ModelRegistry` that
 ...
 * `~/.amaze/agent/extensions/`) was invisible to model resolution and
 * `amaze bench <provider>/<model>` failed with "Model not found".
 *
 ...
 * passed explicitly so the test never touches the developer's real `~/.amaze`.
 */
...
import { getModelMatchPreferences, resolveCliModel } from "@steve-z8k/pi-coding-agent/config/model-resolver";
import { resetSettingsForTest, Settings } from "@steve-z8k/pi-coding-agent/config/settings";
import { loadCliExtensionProviders } from "@steve-z8k/pi-coding-agent/sdk";
