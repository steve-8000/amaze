...
import type { ScopedModel } from "@steve-z8k/pi-coding-agent/config/model-resolver";
import { buildModelScopeNotification } from "@steve-z8k/pi-coding-agent/main";
...
describe("buildModelScopeNotification", () => {
	it("does not emit startup model scope chrome while startup.quiet is enabled", () => {
		expect(buildModelScopeNotification([scopedModel("claude-sonnet-4-6")], true)).toBeNull();
	});

	it("emits the startup model scope banner when startup.quiet is disabled", () => {
		expect(buildModelScopeNotification([scopedModel("claude-sonnet-4-6")], false)).toEqual({
			kind: "info",
			message: "Model scope: claude-sonnet-4-6 (Ctrl+P to cycle)",
		});
	});
	it("includes thinking suffix only when explicitly scoped", () => {
		const withExplicit = {
			...scopedModel("claude-sonnet-4-6"),
			thinkingLevel: "high" as ThinkingLevel,
			explicitThinkingLevel: true,
		};
		expect(buildModelScopeNotification([withExplicit], false)).toEqual({
			kind: "info",
			message: "Model scope: claude-sonnet-4-6:high (Ctrl+P to cycle)",
		});
	});
