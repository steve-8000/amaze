...
import type { AgentToolContext } from "@amaze/agent-core";
import { AsyncJobManager } from "@amaze/coding-agent/async";
import { DEFAULT_BASH_INTERCEPTOR_RULES, Settings } from "@amaze/coding-agent/config/settings";
...
import { BashTool } from "@amaze/coding-agent/tools/bash";
import { FindTool } from "@amaze/coding-agent/tools/find";
import { JobTool } from "@amaze/coding-agent/tools/job";
import { wrapToolWithMetaNotice } from "@amaze/coding-agent/tools/output-meta";
import { ReadTool } from "@amaze/coding-agent/tools/read";
import { DEFAULT_FILE_LIMIT, MULTI_FILE_PER_FILE_MATCHES, SearchTool } from "@amaze/coding-agent/tools/search";
import { WriteTool } from "@amaze/coding-agent/tools/write";
import { $which, Snowflake } from "@amaze/utils";
...
describe("Coding Agent Tools", () => {
	...
	beforeEach(() => {
		...
		originalEditVariant = Bun.env.AMAZE_EDIT_VARIANT;
		Bun.env.AMAZE_EDIT_VARIANT = "replace";
	...
	afterEach(() => {
		...
		if (originalEditVariant === undefined) {
			delete Bun.env.AMAZE_EDIT_VARIANT;
		} else {
			Bun.env.AMAZE_EDIT_VARIANT = originalEditVariant;
		}
	...
	describe("read tool", () => {
		...
		it("rejects semicolon-separated path lists", async () => {
			...
			await expect(readTool.execute("test-call-multi-path", { path: `${firstFile};${secondFile}` })).rejects.toThrow(
				/Read accepts exactly one target per call/,
			);
		});
...
`,
	...
	describe("bash tool", () => {
		...
		it("should persist environment variables between commands", async () => {
			if (process.platform === "win32" || Bun.env.AMAZE_SHELL_PERSIST !== "1") {
				return;
			}

			await bashTool.execute("test-call-8-env-set", { command: "export AMAZE_TEST_VAR=hello" });
			const result = await bashTool.execute("test-call-8-env-get", { command: "echo $AMAZE_TEST_VAR" });
	...
	describe("search tool", () => {
		it("should include filename when searching a single file", async () => {
			...
			// AMAZE_EDIT_VARIANT=replace in beforeEach disables hashlines; expect line-number mode
