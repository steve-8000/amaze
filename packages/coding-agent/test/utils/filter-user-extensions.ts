...
import { getAgentDir, getConfigRootDir, getPluginsDir } from "@steve-z8k/pi-utils";
...
//    or `../`-relative entry pointing somewhere else under `~/.amaze/`), plus
//    the legacy non-XDG `~/.amaze/plugins` tree on hosts without XDG dirs.
// 2. `getAgentDir()` (`~/.amaze/agent` or `$PI_CODING_AGENT_DIR`)
...
// 3. `getPluginsDir()` (XDG-aware: `$XDG_DATA_HOME/amaze/plugins` or legacy)
//    Handles installed plugin extensions that live outside `~/.amaze` when
...
// symlinks `~/.amaze/agent/extensions/foo.ts` to a dotfiles repo would see the
