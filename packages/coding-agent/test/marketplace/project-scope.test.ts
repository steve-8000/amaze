...
describe("resolveActiveProjectRegistryPath", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-proj-scope-"));
	});
	...
	it("walk-up finds nearest .amaze/ directory", async () => {
		// Layout: tmpDir/.amaze/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must climb from cwd → sub → tmpDir and find .amaze/ there.
		fs.mkdirSync(path.join(tmpDir, ".amaze"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".amaze", "plugins", "installed_plugins.json"));
	});

	it("walk-up stops at the nearest .amaze/ — does not skip to a more distant one", async () => {
		// Layout: tmpDir/.amaze/   +   tmpDir/sub/.amaze/   +   tmpDir/sub/nested/  (cwd)
		// Resolver must stop at tmpDir/sub/.amaze/, not climb further to tmpDir/.amaze/.
		fs.mkdirSync(path.join(tmpDir, ".amaze"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".amaze"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".amaze", "plugins", "installed_plugins.json"));
	});

	it("falls back to .git root when no .amaze/ exists", async () => {
		// Layout: tmpDir/.git/   +   tmpDir/sub/  (cwd)
		// No .amaze/ anywhere → second pass finds .git/ at tmpDir.
	...
	it("returns null when neither .amaze/ nor .git/ found anywhere in the tree", async () => {
		// Start at the filesystem root — guaranteed to have no .amaze/ or .git/ ancestors.
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		// resolveActiveProjectRegistryPath must NOT return ~/.amaze/.../installed_plugins.json.
		...
		try {
			// Start from a tmpDir that has no .amaze/ or .git/ of its own.
			const result = await resolveActiveProjectRegistryPath(tmpDir);
			// Must not resolve to the home-dir Amaze registry.
			const homeAmazePath = path.join(homeDir, ".amaze", "plugins", "installed_plugins.json");
			expect(result).not.toBe(homeAmazePath);
		} finally {
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	/** Path where listClaudePluginRoots reads the user Amaze registry. */
	...
	beforeEach(() => {
		tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-shadow-home-"));
		tmpProject = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-shadow-proj-"));

		// Create .amaze/ in project so resolveActiveProjectRegistryPath finds it.
		fs.mkdirSync(path.join(tmpProject, ".amaze", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".amaze", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".amaze", "plugins", "installed_plugins.json");
	});
