/**
 * Vitest setup: quarantine AMAZE_CODING_AGENT_DIR so the test suite never
 * writes session JSONLs into the user's real `~/.amaze/agent/sessions/`.
 *
...
// Guarded so an explicit `AMAZE_CODING_AGENT_DIR=...` env (CI / opt-in) wins.
if (!process.env.AMAZE_CODING_AGENT_DIR) {
	const quarantineDir = join(
		tmpdir(),
		`amaze-vitest-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		"agent",
	);
	mkdirSync(quarantineDir, { recursive: true });
	process.env.AMAZE_CODING_AGENT_DIR = quarantineDir;
}
