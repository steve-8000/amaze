...
import { applyStartupCwd } from "@steve-z8k/pi-coding-agent/cli/startup-cwd";
import { getProjectDir, normalizePathForComparison, setProjectDir } from "@steve-z8k/pi-utils";
...
describe("parseArgs — --cwd flag", () => {
	...
	it("applies --cwd before session lookup callers read the project directory", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-cwd-launch-"));
		const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-cwd-target-"));
	...
	it("normalizes a relative --cwd target to the resolved absolute path", async () => {
		const launchDir = fs.mkdtempSync(path.join(os.tmpdir(), "amaze-cwd-rel-"));
