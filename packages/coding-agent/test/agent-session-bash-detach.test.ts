 ...
 * brush spawn). This test pulls the fix end-to-end through the Amaze coding
 * agent stack:
 *
 *   AgentSession.prompt
 *     → Agent.prompt (real)
 ...
 * SIGTTIN/SIGTTOU the Amaze host process.
 */
...
import { createMockModel, type MockResponse } from "@steve-z8k/pi-ai/providers/mock";
...
import { BashTool, type ToolSession } from "@steve-z8k/pi-coding-agent/tools";
...
describe("BashTool through AgentSession runs children in their own session (e2e)", () => {
	...
	beforeEach(async () => {
		...
		if (!model) throw new Error("expected claude-sonnet-4-6 to be bundled");
