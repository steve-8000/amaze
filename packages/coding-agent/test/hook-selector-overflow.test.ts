...
import { HookSelectorComponent } from "@steve-z8k/pi-coding-agent/modes/components/hook-selector";
...
describe("HookSelectorComponent", () => {
	...
	it("wraps outlined option text without omitting the tail", () => {
		const options = [
			"Option A: Move to Amaze-native only by migrating reusable shared AI instructions into .amaze/AGENTS.md, .amaze/rules, .amaze/skills, and .amaze/agents while deliberately not creating a root .github directory.",
			"Option B: Keep dual support by migrating canonical instructions into .amaze while also maintaining a root .github/copilot-instructions.md compatibility bridge for editors that do not understand Amaze resources yet.",
		];
		const component = new HookSelectorComponent(
		...
		expect(normalizedPlain).toContain("do not understand Amaze resources yet");
		for (const line of lines) {
			expect(visibleWidth(Bun.stripANSI(line))).toBeLessThanOrEqual(width);
		}
	});
		...
		const options = [
			{
				...
				description:
					"Authenticate via the provider keys and OAuth state already configured under ~/.amaze without opening a new browser-based setup flow.",
			},
			{
				label: "Set up Amaze Agent in terminal",
				description:
