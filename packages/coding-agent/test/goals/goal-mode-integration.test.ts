...
async function createGoalHarness(): Promise<GoalHarness> {
	...
	const tempDir = TempDir.createSync("@amaze-goal-mode-");
...
describe("InteractiveMode goal mode integration", () => {
	...
	it("refuses /plan while a goal is paused", async () => {
		await harness.mode.handleGoalModeCommand("Ship the release");
		vi.spyOn(harness.mode, "showHookSelector").mockResolvedValue("Pause");
		await harness.mode.handleGoalModeCommand();
		const showWarning = vi.spyOn(harness.mode, "showWarning");
		...
		expect(showWarning).toHaveBeenCalledWith("Resume or drop the paused goal before entering plan mode.");
		expect(harness.mode.planModeEnabled).toBe(false);
	});

	it("refuses /plan while a goal is budget-limited", async () => {
		await harness.mode.handleGoalModeCommand("Ship the release");
		await harness.mode.handleGoalModeCommand("budget 10");
		const state = harness.session.getGoalModeState();
		if (!state) throw new Error("expected active goal");
		state.goal.tokensUsed = 15;
		state.goal.status = "budget-limited";
		const showWarning = vi.spyOn(harness.mode, "showWarning");

		await harness.mode.handlePlanModeCommand();

		expect(showWarning).toHaveBeenCalledWith("Raise or clear the exhausted goal budget before entering plan mode.");
		expect(harness.mode.planModeEnabled).toBe(false);
	});
