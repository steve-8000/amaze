...
import { AuthStorage, Effort, getBundledModel, type Model } from "@amaze/ai";
...
import type { CustomTool } from "@amaze/coding-agent/extensibility/custom-tools/types";
...
describe("createAgentSession MCP discovery prompt gating", () => {
	...
	it("does not advertise MCP discovery when search_tool_bm25 is not active", async () => {
		...
		expect(session.systemPrompt.join("\n")).not.toContain("call `search_tool_bm25` to discover and activate it");
	});
	...
	it("advertises discovery guidance for builtin-only tools.discoveryMode all sessions", async () => {
		...
		expect(prompt).toContain("call `search_tool_bm25` to discover and activate it");
		expect(searchTool?.description).toContain("Total discoverable tools available:");
	});

	it("defaults top-level sessions to a small orchestrator tool set", async () => {
		const { session } = await createAgentSession({
			cwd: tempDir,
			agentDir: tempDir,
			modelRegistry,
	...
	it("prunes deactivated builtin discoveries so they can be rediscovered", async () => {
		...
		expect(await session.activateDiscoveredTools(["bash"])).toEqual(["bash"]);
		expect(session.getSelectedDiscoveredToolNames()).toContain("bash");

		await session.setActiveToolsByName(["read", "search_tool_bm25"]);

		expect(session.getActiveToolNames()).not.toContain("bash");
		expect(session.getSelectedDiscoveredToolNames()).not.toContain("bash");
		expect(await session.activateDiscoveredTools(["bash"])).toEqual(["bash"]);
