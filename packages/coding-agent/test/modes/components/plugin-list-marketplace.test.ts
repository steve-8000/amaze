...
import { PluginManager } from "@steve-z8k/pi-coding-agent/extensibility/plugins";
import {
...
} from "@steve-z8k/pi-coding-agent/modes/components/plugin-settings";
...
describe("PluginListComponent", () => {
	...
	it("empty-state mentions both npm and marketplace install commands", () => {
		...
		expect(text).toContain("amaze plugin install <package>");
		expect(text).toContain("amaze plugin install <name>@<marketplace>");
	});
...
describe("MarketplacePluginDetailComponent", () => {
	...
	it("shortens home-relative install paths to ~ before rendering", () => {
		...
		const installPath = `${home}/.amaze/cache/plugins/sample@mkt`;
		...
		expect(text).toContain("~/.amaze/cache/plugins/sample@mkt");
