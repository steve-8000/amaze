...
If you want `Shift+Enter` to keep working in tmux via that remap, add `ctrl+j` to your amaze `newLine` keybinding in `~/.amaze/agent/keybindings.json`:
...
On WSL, WezTerm may require a visible hardware cursor for IME candidate window positioning. If CJK IME candidates do not follow the text cursor, set `PI_HARDWARE_CURSOR=1` before running amaze or set `showHardwareCursor` to `true` in settings.
...
If you want the hardware cursor visible, set `PI_HARDWARE_CURSOR=1` before running amaze (disabled by default for compatibility).
