...
2. Resolves eval backend allowance via `AMAZE_PY` override (`getEvalBackendsFromEnv()`) or `eval.py` / `eval.js` settings.
...
5. Instantiates selected tools in parallel with `Promise.all`, records slow factory timings when `AMAZE_TIMING=1`, and wraps results with `wrapToolWithMetaNotice`.
...
- `resolveExtensionEntries(dir)` checks, in order:
  - `package.json` manifest (`amaze` or `pi`) with `extensions[]` entries,
...
- Uses `Settings.getShellConfig()` and `Shell` from `@amaze/natives`.
...
- `AMAZE_BLOCKED_AGENT` prevents self-recursive spawn of a specific agent.
...
- The `task.isolation.merge` setting controls how isolated changes are integrated back:
  ...
  - **branch**: each task commits to a temp branch (`amaze/task/<id>`) via `commitToBranch(...)`, then `mergeTaskBranches(...)` cherry-picks them sequentially onto HEAD. If `git apply` fails inside `commitToBranch`, the error is non-fatal — the agent result is preserved with a `merge failed` status.
...
- Eval availability is mode-driven (`AMAZE_PY`, `eval.py`, `eval.js`); eval falls back to JavaScript when Python is unavailable and JavaScript is enabled. The standalone `bash` tool is always available.
