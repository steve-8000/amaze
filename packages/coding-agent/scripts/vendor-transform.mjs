// Shared transform for vendoring code-yeongyu/pi-* extension packages into amaze builtins.
// Rewrites the standalone packages' published-API imports to amaze's in-tree equivalents:
//   - relative imports: drop the `.js` suffix the packages publish with, use amaze's `.ts`
//   - @steve-8000/amaze-ai|pi-tui  -> @steve-8000/amaze-ai|pi-tui (amaze's workspace names)
//   - {@mariozechner,@steve-8000}/pi-coding-agent -> depth-relative core/extensions/types.ts,
...
export function transformVendoredSource(content, targetFileAbs, builtinRoot) {
	...
	// Relative imports: published `.js` -> amaze `.ts`.
	out = out.replace(/(from\s+["'])(\.[^"']*?)\.js(["'])/g, "$1$2.ts$3");
	out = out.replace(/(import\s+["'])(\.[^"']*?)\.js(["'])/g, "$1$2.ts$3");

	// pi-ai / pi-tui workspace rename.
	out = out.replace(/(["'])@mariozechner\/pi-ai(["'])/g, "$1@steve-8000/amaze-ai$2");
	out = out.replace(/(["'])@mariozechner\/pi-tui(["'])/g, "$1@steve-8000/amaze-tui$2");

	// pi-coding-agent: split Theme (theme module) from the rest (extensions/types.ts).
	const codingAgentImport = /import\s+(type\s+)?\{([^}]*)\}\s+from\s+["'](?:@mariozechner|@steve-8000)\/pi-coding-agent["'];?/g;
