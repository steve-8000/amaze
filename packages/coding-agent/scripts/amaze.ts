/**
 * Bun `--preload` shim for the amaze dev launcher (`scripts/amaze`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the amaze CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@steve-z8k/pi-utils/dirs`).
 */
const launchCwd = process.env.AMAZE_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.AMAZE_LAUNCH_CWD;
	try {
