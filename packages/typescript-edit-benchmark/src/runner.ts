...
import type { AgentMessage, ResolvedThinkingLevel, ThinkingLevel } from "@amaze/agent-core";
import type { Model } from "@amaze/ai";
import { computeLineHash, formatSessionDumpText, RpcClient } from "@amaze/coding-agent";
...
const CLI_PATH = Bun.fileURLToPath(import.meta.resolve("@amaze/coding-agent/cli"));
...
interface BenchmarkClient {
	start(): Promise<void>;
	setThinkingLevel(level: import("@amaze/agent-core").ResolvedThinkingLevel): Promise<void>;
...
async function runSingleTask(
	...
	try {
		...
		if (config.editVariant !== undefined) process.env.AMAZE_EDIT_VARIANT = config.editVariant;
		if (config.editFuzzy !== undefined)
			process.env.AMAZE_EDIT_FUZZY = config.editFuzzy === "auto" ? "auto" : config.editFuzzy ? "1" : "0";
		if (config.editFuzzyThreshold !== undefined)
			process.env.AMAZE_EDIT_FUZZY_THRESHOLD =
				config.editFuzzyThreshold === "auto" ? "auto" : String(config.editFuzzyThreshold);
		process.env.AMAZE_NO_TITLE = "1";
