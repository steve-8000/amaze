...
```bash
npm install @steve-z8k/pi-ai
```

## Quick Start

```typescript
import { z, getModel, stream, complete, Context, Tool } from "@steve-z8k/pi-ai";
...
```typescript
import { z, Tool } from "@steve-z8k/pi-ai";
...
```typescript
import { stream, validateToolCall, Tool } from "@steve-z8k/pi-ai";
...
```typescript
import * as fs from "node:fs";
import { getModel, complete } from "@steve-z8k/pi-ai";
...
```typescript
import { getModel, streamSimple, completeSimple } from "@steve-z8k/pi-ai";
...
```typescript
import { getModel, stream } from "@steve-z8k/pi-ai";
...
```typescript
import { getProviders, getModels, getModel } from "@steve-z8k/pi-ai";
...
```typescript
import { Model, stream } from "@steve-z8k/pi-ai";
...
```typescript
import { getModel, complete, Context } from "@steve-z8k/pi-ai";
...
```typescript
import { Context, getModel, complete } from "@steve-z8k/pi-ai";
...
```typescript
import { getEnvApiKey } from "@steve-z8k/pi-ai";
...
Authenticate via the [`amaze`](https://amaze) coding-agent CLI, which drives this library's OAuth/API-key flows in-process and persists into `agent.db`:

```bash
amaze auth-broker login              # interactive provider selection
amaze auth-broker login anthropic    # login to a specific provider
amaze auth-broker login vllm         # store vLLM API key (or placeholder for local no-auth)
amaze auth-broker list               # list supported providers
amaze auth-broker logout             # interactive — pick a stored credential to remove
```
...
```typescript
import { loginGitHubCopilot } from "@steve-z8k/pi-ai";
import * as fs from "node:fs";
...
```typescript
import { getModel, complete, getOAuthApiKey } from "@steve-z8k/pi-ai";
import * as fs from "node:fs";
