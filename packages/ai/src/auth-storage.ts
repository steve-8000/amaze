...
import { getAgentDbPath, logger } from "@amaze/utils";
...
import { loginXaiOAuth } from "./utils/oauth/xai";
...
const CREDENTIAL_PROVIDER_ALIASES: ReadonlyMap<string, OAuthProviderId> = new Map([["xai", "xai-oauth"]]);
...
export type AuthStorageOptions = {
	 ...
	 * - coding-agent injects its resolveConfigValue (supports "!command" syntax via natives)
 ...
 * Does NOT support "!command" syntax (that requires natives).
 */
...
export class AuthStorage {
	...
	#credentialProvider(provider: string): string {
		return CREDENTIAL_PROVIDER_ALIASES.get(provider) ?? provider;
	}
	...
	#getCredentialsForProvider(provider: string): AuthCredential[] {
		return this.#getStoredCredentials(this.#credentialProvider(provider)).map(entry => entry.credential);
	}
	...
	async #resolveOAuthApiKey(
		...
		await Promise.all(
			candidates.map(async candidate => {
				...
				try {
					const credentialId = this.#getStoredCredentials(credentialProvider)[candidate.selection.index]?.id;
	...
	async #prepareOAuthCredentialForRequest(
		...
		const latestRows = this.#store.listAuthCredentials(credentialProvider);
		this.#setStoredCredentials(
			credentialProvider,
	...
	async #tryOAuthCredential(
		...
		try {
			...
			const customProvider = getOAuthProvider(credentialProvider as OAuthProviderId);
			if (customProvider) {
				const refreshedCredentials = await this.#refreshOAuthCredential(
					credentialProvider,
					selection.credential,
					this.#getStoredCredentials(credentialProvider)[selection.index]?.id,
					options?.signal,
				);
			...
			this.#replaceCredentialAt(credentialProvider, selection.index, updated);
			if ((checkUsage && !allowBlocked) || requiresProModel) {
			...
			if (isDefinitiveFailure) {
				...
				// updated.
				const credentialId = this.#getStoredCredentials(credentialProvider)[selection.index]?.id;
				if (credentialId !== undefined) {
					const latestRow = this.#store
						.listAuthCredentials(credentialProvider)
	...
	async invalidateCredentialMatching(provider: string, apiKey: string, signal?: AbortSignal): Promise<boolean> {
		...
		if (markSuspect) {
			...
			this.#tryDisableCredentialAtIfMatches(
				credentialProvider,
				matchedIndex,
				stored[matchedIndex].credential,
				"credential invalidated after provider authentication failure",
			);
		} else {
			await this.reload();
		}
