...
export { isRecord } from "@amaze/utils";
 ...
 *
 * Defaults to "long" (1h TTL on Anthropic) so long-lived agent sessions keep
 * their prompt-cache prefix warm. Cache write costs ~2x base input on Anthropic
 * (vs 1.25x for 5m), amortized over many reads at 90% discount. Sessions that
 * burn the full window in under 5 minutes pay slightly more; everything else
 * comes out ahead.
 *
 * Override per-request via `cacheRetention`, or globally via
 * `AMAZE_CACHE_RETENTION=short` to restore the previous 5m behavior.
 */
export function resolveCacheRetention(cacheRetention?: CacheRetention): CacheRetention {
	if (cacheRetention) return cacheRetention;
	if ($env.AMAZE_CACHE_RETENTION === "short") return "short";
	return "long";
}
