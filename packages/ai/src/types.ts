...
export interface Model<TApi extends Api = any> {
	...
	headers?: Record<string, string>;
	/**
	 * Streaming transport override. When `"amaze-native"`, `streamSimple` routes
	 * the request to the model's `baseUrl` via the auth-gateway's
	 * `POST /v1/amaze/stream` endpoint instead of dispatching the per-API
	 * provider client. The `baseUrl` must point at an `amaze auth-gateway`
	 * (or compatible) host; `headers.Authorization` (or `apiKey` resolved by
	 * the registry) carries the gateway bearer.
	 *
	 * Used by containerized amaze installs (e.g. rocky slots) to route every
