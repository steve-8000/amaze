...
/** Cap max output tokens at a value that matches AMAZE's other openai-responses defaults. */
			...
			fetchDynamicModels: () =>
				fetchOpenAICompatibleModels<Api>({
					...
					mapModel: (
						...
						// max_prompt_tokens is the prompt capacity (what AMAZE calls contextWindow).
...
const MODELS_DEV_PROVIDER_DESCRIPTORS_CORE: readonly ModelsDevProviderDescriptor[] = [
	...
	openAiCompletionsDescriptor("deepseek", "deepseek", "https://api.deepseek.com", {
		...
		compat: {
			// DeepSeek V4 only accepts `high`/`max`; map lower AMAZE levels upward so
