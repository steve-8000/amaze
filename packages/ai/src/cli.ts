...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @steve-8000/amaze-ai <command> [provider]
...
Examples:
  ...
  npx @steve-8000/amaze-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @steve-8000/amaze-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @steve-8000/amaze-ai --help' for usage`);
	process.exit(1);
}
