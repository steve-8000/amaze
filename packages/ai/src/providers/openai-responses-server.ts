/**
 * OpenAI Responses HTTP wire-format ↔ amaze Context bridge for the auth-gateway.
 *
 * Inbound: parses `POST /v1/responses` request bodies into a {@link ParsedRequest}.
 * Outbound: encodes amaze's {@link AssistantMessage} (and event stream) back into
...
export function parseRequest(body: unknown, headers?: Headers): ParsedRequest {
	...
	// `store` is a stateful-storage hint that amaze's gateway doesn't honour;
					...
					switch (ev.type) {
						...
						case "toolcall_end": {
							...
							if (cur.customWireName) {
								...
								// Standard JSON tool: arguments object on the amaze side, the
