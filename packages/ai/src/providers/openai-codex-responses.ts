...
const CODEX_DEBUG = $flag("AMAZE_CODEX_DEBUG");
...
function isCodexWebSocketEnvEnabled(): boolean {
	return $flag("AMAZE_CODEX_WEBSOCKET");
}

function getCodexWebSocketRetryBudget(): number {
	return parseCodexNonNegativeInteger($env.AMAZE_CODEX_WEBSOCKET_RETRY_BUDGET, CODEX_WEBSOCKET_RETRY_BUDGET);
}

function getCodexWebSocketRetryDelayMs(retry: number): number {
	const baseDelay = parseCodexPositiveInteger($env.AMAZE_CODEX_WEBSOCKET_RETRY_DELAY_MS, CODEX_RETRY_DELAY_MS);
...
function getCodexWebSocketIdleTimeoutMs(overrideMs?: number): number {
	return (
		overrideMs ??
		parseCodexPositiveInteger($env.AMAZE_CODEX_WEBSOCKET_IDLE_TIMEOUT_MS, CODEX_WEBSOCKET_IDLE_TIMEOUT_MS)
	);
}
...
function getCodexWebSocketFirstEventTimeoutMs(idleTimeoutMs: number, overrideMs?: number): number {
	return (
		...
		parseCodexPositiveInteger(
			$env.AMAZE_CODEX_WEBSOCKET_FIRST_EVENT_TIMEOUT_MS,
