			...
			for (const block of msg.content) {
				if (block.type === "thinking") {
					...
					if (isFreeformToolName(toolCall.name, context.tools)) {
						...
						// The Responses API requires function_call item ids to begin with
						// "fc_". Any other value (e.g. "custom" from a tool that was streamed
						// as a custom_tool_call but isn't freeform in this context) is invalid,
						// so omit the id rather than send a rejected one.
						const safeId = itemId?.startsWith("fc_") ? itemId : undefined;
						output.push({
							type: "function_call",
							id: safeId,
							call_id: callId,
