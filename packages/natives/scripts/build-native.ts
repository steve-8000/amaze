...
const rustDir = path.join(repoRoot, "crates/natives");
...
const canonicalAddonFilename = `amaze_natives.${targetPlatform}-${targetArch}${variantSuffix}.node`;
const canonicalAddonPath = path.join(nativeDir, canonicalAddonFilename);

console.log(`Building natives for ${targetPlatform}-${targetArch}${variantSuffix}${profileSuffix}…`);
